...
int ds4_dump_layer0_qkv_rope_oracle_json(const char *model_path, int token, FILE *fp) {
    if (!model_path || !fp || token < 0 || token >= DS4_N_VOCAB) return 1;
    ...
    float *kv_raw = xmalloc((size_t)kv_dim * sizeof(kv_raw[0]));
    float *qr_norm = xmalloc((size_t)q_rank * sizeof(qr_norm[0]));
    float *q = xmalloc((size_t)q_dim * sizeof(q[0]));
    float *kv = xmalloc((size_t)kv_dim * sizeof(kv[0]));
    ds4_gpu_tensor *cur_hc_tensor = NULL;
    ds4_gpu_tensor *flat_hc_tensor = NULL;
    ds4_gpu_tensor *hc_mix_tensor = NULL;
    ds4_gpu_tensor *hc_split_tensor = NULL;
    ...
    if (ok) {
        ok = ds4_gpu_matmul_q8_0_tensor(qr_tensor,
                                        model.map,
                                        model.size,
                                        layer->attn_q_a->abs_offset,
                                        DS4_N_EMBD,
    ...
    if (ok) {
        ok = ds4_gpu_matmul_q8_0_tensor(kv_raw_tensor,
                                        model.map,
                                        model.size,
                                        layer->attn_kv->abs_offset,
                                        DS4_N_EMBD,
    ...
    if (ok) {
        ok = ds4_gpu_matmul_q8_0_tensor(q_tensor,
                                        model.map,
                                        model.size,
                                        layer->attn_q_b->abs_offset,
                                        q_rank,
    ...
    if (ok) {
        ok = ds4_gpu_rope_tail_tensor(q_tensor,
                                      1,
                                      DS4_N_HEAD,
                                      DS4_N_HEAD_DIM,
                                      DS4_N_ROT,
    ...
    if (ok) {
        ok = ds4_gpu_rope_tail_tensor(kv_tensor,
                                      1,
                                      DS4_N_HEAD_KV,
                                      DS4_N_HEAD_DIM,
                                      DS4_N_ROT,
    ...
    if (ok) ok = ds4_gpu_synchronize() != 0;
    if (ok) {
        ok = ds4_gpu_tensor_read(attn_norm_tensor, 0, attn_norm, (uint64_t)DS4_N_EMBD * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(qr_tensor, 0, qr, q_rank * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(kv_raw_tensor, 0, kv_raw, kv_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(qr_norm_tensor, 0, qr_norm, q_rank * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(q_tensor, 0, q, q_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(kv_tensor, 0, kv, kv_dim * sizeof(float)) != 0;
    }
    if (!ok) {
        fprintf(stderr, "ds4: failed to run layer-0 QKV/RoPE GPU oracle\n");
        if (kv_tensor) ds4_gpu_tensor_free(kv_tensor);
        if (q_tensor) ds4_gpu_tensor_free(q_tensor);
        if (qr_norm_tensor) ds4_gpu_tensor_free(qr_norm_tensor);
        if (kv_raw_tensor) ds4_gpu_tensor_free(kv_raw_tensor);
        if (qr_tensor) ds4_gpu_tensor_free(qr_tensor);
        if (attn_norm_tensor) ds4_gpu_tensor_free(attn_norm_tensor);
        if (attn_cur_tensor) ds4_gpu_tensor_free(attn_cur_tensor);
        if (hc_split_tensor) ds4_gpu_tensor_free(hc_split_tensor);
        if (hc_mix_tensor) ds4_gpu_tensor_free(hc_mix_tensor);
    ...
    fputs("{\n", fp);
    fputs("  \"schema\": \"ds4.layer0_qkv_rope_oracle.v1\",\n", fp);
    fprintf(fp, "  \"case\": \"token%d_layer0_qkv_rope\",\n", token);
    fputs("  \"source\": \"current-c\",\n", fp);
    fputs("  \"model\": {\n", fp);
    fprintf(fp, "    \"mapped_size\": %" PRIu64 ",\n", model.size);
    fprintf(fp, "    \"tensor_count\": %" PRIu64 ",\n", model.n_tensors);
    ...
    fputs("  },\n", fp);
    fputs("  \"operation\": {\n", fp);
    fputs("    \"name\": \"current_c_gpu_layer0_qkv_rope_prefix\",\n", fp);
    fputs("    \"method\": \"ds4_gpu_embed_token_hc_tensor+ds4_gpu_rms_norm_plain_tensor+ds4_gpu_matmul_f16_tensor+ds4_gpu_hc_split_weighted_sum_norm_tensor+ds4_gpu_matmul_q8_0_tensor+ds4_gpu_dsv4_qkv_rms_norm_rows_tensor+ds4_gpu_head_rms_norm_tensor+ds4_gpu_rope_tail_tensor\",\n", fp);
    fprintf(fp, "    \"token\": %d,\n", token);
    fputs("    \"layer\": 0,\n", fp);
    fputs("    \"position\": 0,\n", fp);
    fprintf(fp, "    \"n_vocab\": %u,\n", (unsigned)DS4_N_VOCAB);
    ...
    layer0_attn_oracle_write_weight(fp, "attn_kv_a_norm", "base.layer.0.attn_kv_a_norm", layer->attn_kv_a_norm, false);
    fputs("  },\n", fp);
    fputs("  \"outputs\": {\n", fp);
    layer0_attn_oracle_write_output(fp, "attn_norm", attn_norm, DS4_N_EMBD, true);
    layer0_attn_oracle_write_output(fp, "qr", qr, q_rank, true);
    layer0_attn_oracle_write_output(fp, "kv_raw", kv_raw, kv_dim, true);
    layer0_attn_oracle_write_output(fp, "qr_norm", qr_norm, q_rank, true);
    layer0_attn_oracle_write_output(fp, "q", q, q_dim, true);
    layer0_attn_oracle_write_output(fp, "kv", kv, kv_dim, false);
    fputs("  }\n", fp);
    fputs("}\n", fp);
...
int ds4_dump_layer0_attn_output_oracle_json(const char *model_path, int token, FILE *fp) {
    if (!model_path || !fp || token < 0 || token >= DS4_N_VOCAB) return 1;
    ...
    float *kv = xmalloc((size_t)kv_dim * sizeof(kv[0]));
    float *raw_cache_row = xmalloc((size_t)kv_dim * sizeof(raw_cache_row[0]));
    float *heads = xmalloc((size_t)q_dim * sizeof(heads[0]));
    float *attn_low = xmalloc((size_t)low_dim * sizeof(attn_low[0]));
    float *attn_out = xmalloc((size_t)DS4_N_EMBD * sizeof(attn_out[0]));
    ...
    if (ok) {
        ok = ds4_gpu_kv_fp8_store_raw_tensor(kv_tensor,
                                             raw_cache_tensor,
                                             raw_cap,
                                             raw_row,
                                             DS4_N_HEAD_DIM,
    ...
    if (ok) {
        ok = ds4_gpu_attention_decode_heads_tensor(heads_tensor,
                                                   model.map,
                                                   model.size,
                                                   layer->attn_sinks->abs_offset,
                                                   q_tensor,
    ...
    if (ok) {
        ok = ds4_gpu_rope_tail_tensor(heads_tensor,
                                      1,
                                      DS4_N_HEAD,
                                      DS4_N_HEAD_DIM,
                                      DS4_N_ROT,
    ...
    if (ok) {
        ok = ds4_gpu_attention_output_low_q8_tensor(attn_low_tensor,
                                                    model.map,
                                                    model.size,
                                                    layer->attn_output_a->abs_offset,
                                                    group_dim,
    ...
    if (ok) {
        ok = ds4_gpu_matmul_q8_0_hc_expand_tensor(after_attn_hc_tensor,
                                                  attn_out_tensor,
                                                  model.map,
                                                  model.size,
                                                  layer->attn_output_b->abs_offset,
    ...
    if (ok) ok = ds4_gpu_synchronize() != 0;
    if (ok) {
        ok = ds4_gpu_tensor_read(kv_tensor, 0, kv, kv_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(raw_cache_tensor,
                                 (uint64_t)raw_row * kv_dim * sizeof(float),
                                 raw_cache_row,
                                 kv_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(heads_tensor, 0, heads, q_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(attn_low_tensor, 0, attn_low, low_dim * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(attn_out_tensor, 0, attn_out, (uint64_t)DS4_N_EMBD * sizeof(float)) != 0 &&
             ds4_gpu_tensor_read(after_attn_hc_tensor, 0, after_attn_hc, hc_dim * sizeof(float)) != 0;
    }
    if (!ok) {
        fprintf(stderr, "ds4: failed to run layer-0 attention-output GPU oracle\n");
        goto cleanup;
    }

    fputs("{\n", fp);
    fputs("  \"schema\": \"ds4.layer0_attn_output_oracle.v1\",\n", fp);
    fprintf(fp, "  \"case\": \"token%d_layer0_attn_output\",\n", token);
    fputs("  \"source\": \"current-c\",\n", fp);
    fputs("  \"model\": {\n", fp);
    fprintf(fp, "    \"mapped_size\": %" PRIu64 ",\n", model.size);
    fprintf(fp, "    \"tensor_count\": %" PRIu64 ",\n", model.n_tensors);
    ...
    fputs("  },\n", fp);
    fputs("  \"operation\": {\n", fp);
    fputs("    \"name\": \"current_c_gpu_layer0_attn_output_prefix\",\n", fp);
    fputs("    \"method\": \"qkv_rope_prefix+ds4_gpu_kv_fp8_store_raw_tensor+ds4_gpu_attention_decode_heads_tensor+ds4_gpu_rope_tail_tensor_inverse+ds4_gpu_attention_output_low_q8_tensor+ds4_gpu_matmul_q8_0_hc_expand_tensor\",\n", fp);
    fprintf(fp, "    \"token\": %d,\n", token);
    fputs("    \"layer\": 0,\n", fp);
    fputs("    \"position\": 0,\n", fp);
    ...
    layer0_attn_oracle_write_weight(fp, "attn_output_b", "base.layer.0.attn_output_b", layer->attn_output_b, false);
    fputs("  },\n", fp);
    fputs("  \"outputs\": {\n", fp);
    layer0_attn_oracle_write_output(fp, "kv", kv, kv_dim, true);
    layer0_attn_oracle_write_output(fp, "raw_cache_row", raw_cache_row, kv_dim, true);
    layer0_attn_oracle_write_output(fp, "heads", heads, q_dim, true);
    layer0_attn_oracle_write_output(fp, "attn_low", attn_low, low_dim, true);
    layer0_attn_oracle_write_output(fp, "attn_out", attn_out, DS4_N_EMBD, true);
    layer0_attn_oracle_write_output(fp, "after_attn_hc", after_attn_hc, hc_dim, false);
    fputs("  }\n", fp);
    fputs("}\n", fp);
    rc = ferror(fp) ? 1 : 0;
...
int ds4_dump_layer0_output_head_oracle_json(const char *model_path, int token, FILE *fp) {
    if (!model_path || !fp || token < 0 || token >= DS4_N_VOCAB) return 1;
    ...
    const uint64_t vocab_dim = weights.output->dim[1];
    after_ffn_hc = xmalloc((size_t)hc_dim * sizeof(after_ffn_hc[0]));
    ...
    if (ok) {
        ok = ds4_gpu_tensor_read(g.cur_hc, 0, after_ffn_hc, hc_dim * sizeof(float)) != 0 &&
    ...
    fputs("{\n", fp);
    fputs("  \"schema\": \"ds4.layer0_output_head_oracle.v1\",\n", fp);
    fputs("  \"case\": \"token0_layer0_output_head\",\n", fp);
    fputs("  \"source\": \"current-c\",\n", fp);
    fputs("  \"model\": {\n", fp);
    fprintf(fp, "    \"mapped_size\": %" PRIu64 ",\n", model.size);
    fprintf(fp, "    \"tensor_count\": %" PRIu64 ",\n", model.n_tensors);
    ...
    fputs("  },\n", fp);
    fputs("  \"operation\": {\n", fp);
    fputs("    \"name\": \"current_c_gpu_layer0_output_head\",\n", fp);
    fputs("    \"method\": \"metal_graph_encode_decode_layer+metal_graph_encode_output_head\",\n", fp);
    fprintf(fp, "    \"token\": %d,\n", token);
    fputs("    \"layer\": 0,\n", fp);
    fputs("    \"position\": 0,\n", fp);
    fputs("    \"decoded_layers\": 1,\n", fp);
    fprintf(fp, "    \"ctx_size\": %u,\n", ctx_size);
    fprintf(fp, "    \"prefill_cap\": %u,\n", prefill_cap);
    fprintf(fp, "    \"raw_cap\": %u,\n", raw_cap);
    fprintf(fp, "    \"raw_window\": %u,\n", raw_window);
    ...
    fputs("  },\n", fp);
    fputs("  \"outputs\": {\n", fp);
    layer0_attn_oracle_write_output(fp, "after_ffn_hc", after_ffn_hc, hc_dim, true);
