use ds4_engine::{
    context_memory_estimate, Backend, Engine, EngineOptions, RuntimeGraphRoute, SamplingOptions,
...
const HELP: &str = "\
Usage: ds4-session-runtime-rs [(-p PROMPT | --prompt-file FILE)] [options]\n\
\n\
M8.13b runtime boundary for seeded session-backed one-shot sampling.\n\
\n\
Options:\n\
  -m, --model FILE\n\
...
fn run() -> Result<i32, Box<dyn Error>> {
    ...
    if let Some(exit) = config
        .runtime_graph_route
        .fail_closed("ds4-session-runtime-rs")
    {
    ...
    let generation = engine.generate_sampled_text(
        &prompt,
        SamplingOptions {
    ...
    S: Into<String>,
{
    ...
    while i < argv.len() {
        ...
        match arg {
            ...
            "--seed" => {
                config.seed = parse_positive_u64(need_arg(&argv, &mut i, arg)?, arg)?;
                seed_seen = true;
            }
            "--backend" => {
                let value = need_arg(&argv, &mut i, arg)?;
    ...
    if !prompt_seen {
        return Err(exit(
            2,
            "",
            "ds4-rs: M8.13b session runtime requires -p or --prompt-file\n",
        ));
    }
    if !seed_seen {
        return Err(exit(
            2,
            "",
            "ds4-rs: M8.13b session runtime requires --seed\n",
        ));
    }
    if config.temperature <= 0.0 {
        return Err(exit(
            99,
            "",
            "ds4-rs: M8.13b session runtime is for non-greedy sampling; use M8.13a for --temp 0\n",
        ));
    }
    Ok(config)
}
...
fn parse_positive_u64(value: &str, opt: &str) -> Result<u64, RuntimeExit> {
