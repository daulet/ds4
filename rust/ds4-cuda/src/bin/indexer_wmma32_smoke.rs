...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{{\"milestone\":\"M14.2d2b2a\",\"device_name\":{:?},\"rust_kernel_toolchain\":true,\"wmma32_output_matches\":true,\"causal_mask_output_matches\":true,\"two_warp_tile_mapping_matches\":true,\"weighted_epilogue_matches\":true,\"nan_negative_clamp_matches\":true,\"invalid_shape_rejected\":true,\"owns_indexer_scores_wmma32_kernel\":{},\"owns_wmma64_and_wmma128_dispatch\":{},\"owns_specialized_topk_dispatch\":{},\"changes_default_route\":{}}}",
        substrate.device_name()?,
        M14_2D2B2A_SCOPE.owns_indexer_scores_wmma32_kernel,
        M14_2D2B2A_SCOPE.owns_wmma64_and_wmma128_dispatch,
        M14_2D2B2A_SCOPE.owns_specialized_topk_dispatch,
        M14_2D2B2A_SCOPE.changes_default_route,
    );
    Ok(())
}
...
fn indexer_scores_wmma32(
    ...
    {
        return Err(IndexerWmma32Error::InvalidShape);
    }
    module
        .indexer_scores_wmma32_kernel(
            stream,
            LaunchConfig {
                grid_dim: (
        ...
        .map_err(IndexerWmma32Error::Driver)
}

#[derive(Debug)]
enum IndexerWmma32Error {
    InvalidShape,
    Driver(DriverError),
}

impl fmt::Display for IndexerWmma32Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidShape => formatter.write_str("WMMA32 indexer score shape is invalid"),
            Self::Driver(error) => error.fmt(formatter),
        }
    }
}

impl std::error::Error for IndexerWmma32Error {
