...
use ds4_cuda::{
    select_indexer_score_kernel, substrate::CudaOxideSubstrate, IndexerScoreDispatchOptions,
    IndexerScoreKernel, M14_2D2B2C_SCOPE,
};
...
const WIDE_COMPONENTS: usize = 128;
const MMA_K: usize = 16;
const MMA_N: usize = 8;
const WMMA_WARPS: usize = 8;
const WMMA_THREADS: u32 = 256;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{{\"milestone\":\"M14.2d2b2c\",\"device_name\":{:?},\"rust_kernel_toolchain\":true,\"wmma128_output_matches\":true,\"causal_mask_output_matches\":true,\"eight_warp_tile_mapping_matches\":true,\"weighted_epilogue_matches\":true,\"nan_negative_clamp_matches\":true,\"invalid_shape_rejected\":true,\"score_dispatch_priority_matches\":true,\"owns_indexer_scores_wmma128_kernel\":{},\"owns_indexer_score_dispatch_policy\":{},\"owns_specialized_topk_dispatch\":{},\"changes_default_route\":{}}}",
        substrate.device_name()?,
        M14_2D2B2C_SCOPE.owns_indexer_scores_wmma128_kernel,
        M14_2D2B2C_SCOPE.owns_indexer_score_dispatch_policy,
        M14_2D2B2C_SCOPE.owns_specialized_topk_dispatch,
        M14_2D2B2C_SCOPE.changes_default_route,
    );
    Ok(())
}
...
fn dispatch_priority_matches_current_c() -> bool {
    ...
    select_indexer_score_kernel(base) == IndexerScoreKernel::Wmma128
...
fn indexer_scores_wmma128(
    ...
    {
        return Err(IndexerWmma128Error::InvalidShape);
    }
    module
        ...
        .map_err(IndexerWmma128Error::Driver)
}

#[derive(Debug)]
enum IndexerWmma128Error {
    InvalidShape,
    Driver(DriverError),
}

impl fmt::Display for IndexerWmma128Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidShape => formatter.write_str("WMMA128 indexer score shape is invalid"),
            Self::Driver(error) => error.fmt(formatter),
        }
    }
}

impl std::error::Error for IndexerWmma128Error {
