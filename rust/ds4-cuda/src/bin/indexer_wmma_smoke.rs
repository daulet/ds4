...
use ds4_cuda::{substrate::CudaOxideSubstrate, M14_2D2B1_SCOPE};
...
mod kernels {
    ...
    pub fn indexer_scores_wmma_kernel(
        ...
        static mut B_LO_TILE: SharedArray<f16, { MMA_K * MMA_N }, 16> = SharedArray::UNINIT;
        static mut B_HI_TILE: SharedArray<f16, { MMA_K * MMA_N }, 16> = SharedArray::UNINIT;
        static mut C_TILE: SharedArray<f32, { TILE_TOKENS * TILE_COMPONENTS }> =
            SharedArray::UNINIT;
        static mut ACC_TILE: SharedArray<f32, { TILE_TOKENS * TILE_COMPONENTS }> =
            SharedArray::UNINIT;

        let tile_c = thread::blockIdx_x() as usize * TILE_COMPONENTS;
        ...
        if causal != 0 {
            ...
            if tile_c as u32 >= max_visible {
                let mut i = tid;
                while i < TILE_TOKENS * TILE_COMPONENTS {
                    let row = i / TILE_COMPONENTS;
                    let col = i % TILE_COMPONENTS;
        ...
        while head < N_HEAD {
            ...
            while k0 < HEAD_DIM {
                ...
                let mut b_index = tid;
                while b_index < MMA_K * MMA_N {
                    let row = b_index / MMA_N;
                    let col = b_index % MMA_N;
                    let comp_lo = tile_c + col;
                    let comp_hi = tile_c + MMA_N + col;
                ...
                let a_ptr = unsafe { (&raw const A_TILE).cast::<f16>().add(a_row * MMA_K + a_col) }
                    .cast::<u8>();
                let b_lo_ptr =
                    unsafe { (&raw const B_LO_TILE).cast::<f16>().add(b_row * MMA_N) }.cast::<u8>();
                let b_hi_ptr =
                    unsafe { (&raw const B_HI_TILE).cast::<f16>().add(b_row * MMA_N) }.cast::<u8>();
            ...
            unsafe {
                C_TILE[group_id * TILE_COMPONENTS + col_base] = acc_lo.x();
                C_TILE[group_id * TILE_COMPONENTS + col_base + 1] = acc_lo.y();
                C_TILE[(group_id + 8) * TILE_COMPONENTS + col_base] = acc_lo.z();
                C_TILE[(group_id + 8) * TILE_COMPONENTS + col_base + 1] = acc_lo.w();
                C_TILE[group_id * TILE_COMPONENTS + MMA_N + col_base] = acc_hi.x();
                C_TILE[group_id * TILE_COMPONENTS + MMA_N + col_base + 1] = acc_hi.y();
                C_TILE[(group_id + 8) * TILE_COMPONENTS + MMA_N + col_base] = acc_hi.z();
                C_TILE[(group_id + 8) * TILE_COMPONENTS + MMA_N + col_base + 1] = acc_hi.w();
            }
            thread::sync_threads();

            let mut output_index = tid;
            while output_index < TILE_TOKENS * TILE_COMPONENTS {
                let row = output_index / TILE_COMPONENTS;
                let col = output_index % TILE_COMPONENTS;
                let token = tile_t + row;
                let comp = tile_c + col;
                if token < n_tokens as usize && comp < n_comp as usize {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "{{\"milestone\":\"M14.2d2b1\",\"device_name\":{:?},\"rust_kernel_toolchain\":true,\"base_wmma_output_matches\":true,\"causal_mask_output_matches\":true,\"two_mma_column_halves_match\":true,\"weighted_epilogue_matches\":true,\"nan_negative_clamp_matches\":true,\"invalid_shape_rejected\":true,\"owns_indexer_scores_wmma_kernel\":{},\"owns_widened_wmma_dispatch\":{},\"owns_specialized_topk_dispatch\":{},\"changes_default_route\":{}}}",
        substrate.device_name()?,
        M14_2D2B1_SCOPE.owns_indexer_scores_wmma_kernel,
        M14_2D2B1_SCOPE.owns_widened_wmma_dispatch,
        M14_2D2B1_SCOPE.owns_specialized_topk_dispatch,
        M14_2D2B1_SCOPE.changes_default_route,
    );
    Ok(())
}
...
fn indexer_scores_wmma(
    ...
    {
        return Err(IndexerWmmaError::InvalidShape);
    }
    module
        .indexer_scores_wmma_kernel(
            ...
            LaunchConfig {
                grid_dim: (
                    n_comp.div_ceil(TILE_COMPONENTS as u32),
        ...
        .map_err(IndexerWmmaError::Driver)
}

#[derive(Debug)]
enum IndexerWmmaError {
    InvalidShape,
    Driver(DriverError),
}

impl fmt::Display for IndexerWmmaError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidShape => formatter.write_str("WMMA indexer score shape is invalid"),
            Self::Driver(error) => error.fmt(formatter),
        }
    }
}

impl std::error::Error for IndexerWmmaError {
