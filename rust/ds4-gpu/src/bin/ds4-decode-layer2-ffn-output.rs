...
const SCHEMA: &str = "ds4.decode_layer2_ffn_output.v1";
const CASE: &str = "token0_layer2_ffn_output";
const TOKEN: u32 = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer2-ffn-output: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        Output::F32(read_tensor_output("after_layer1_hc", &cur_hc, hc_dim)?),
        ...
        Output::F32(read_tensor_output("layer2_heads", &heads, q_dim)?),
        Output::F32(read_tensor_output("layer2_attn_low", &attn_low, low_dim)?),
        Output::F32(read_tensor_output(
            "layer2_attn_out",
            &attn_out,
            u64::from(N_EMBD),
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer2-ffn-output --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer2_ffn_output\",");
    ...
    write_weight(
        "layer2_attn_output_a",
        "layer2.attn_output_a",
        &layer2.attn_output_a,
        true,
    );
    write_weight(
        "layer2_attn_output_b",
        "layer2.attn_output_b",
        &layer2.attn_output_b,
        true,
    );
    write_weight(
    ...
    write_weight("layer2_ffn_norm", "layer2.ffn_norm", &layer2.ffn_norm, true);
    write_weight(
        "layer2_ffn_gate_inp",
        "layer2.ffn_gate_inp",
        &layer2.ffn_gate_inp,
    ...
    if let Some(tensor) = &layer2.ffn_exp_probs_b {
        write_weight(
            "layer2_ffn_exp_probs_b",
            "layer2.ffn_exp_probs_b",
            tensor,
            true,
        );
    }
    if let Some(tensor) = &layer2.ffn_gate_tid2eid {
        write_weight(
            "layer2_ffn_gate_tid2eid",
            "layer2.ffn_gate_tid2eid",
            tensor,
            true,
        );
    }
    write_weight(
        "layer2_ffn_gate_exps",
        "layer2.ffn_gate_exps",
        &layer2.ffn_gate_exps,
        true,
    );
    write_weight(
    ...
    write_weight(
        "layer2_ffn_down_exps",
        "layer2.ffn_down_exps",
        &layer2.ffn_down_exps,
        true,
    );
    write_weight(
        "layer2_ffn_gate_shexp",
        "layer2.ffn_gate_shexp",
        &layer2.ffn_gate_shexp,
        true,
    );
    write_weight(
    ...
    write_weight(
        "layer2_ffn_down_shexp",
        "layer2.ffn_down_shexp",
        &layer2.ffn_down_shexp,
        false,
    );
    println!("  }},");
