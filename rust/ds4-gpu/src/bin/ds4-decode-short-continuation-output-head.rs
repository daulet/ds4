...
const SCHEMA: &str = "ds4.decode_short_continuation_output_head.v1";
const CASE: &str = "tokens0_21_short_continuation_output_head";
const SEQUENCE_LEN: u32 = 22;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-short-continuation-output-head: {err}");
        std::process::exit(1);
    }
}
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-short-continuation-output-head --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"rust_gpu_short_continuation_output_head\",");
    println!("    \"method\": \"decode_backend_execute_layer_x22x43+default_split_flush+compressed_row_quantize+output_head\",");
