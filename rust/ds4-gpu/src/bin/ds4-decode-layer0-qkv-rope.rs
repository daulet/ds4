...
use ds4_gpu::graph_plan::{
    HC_EPS, N_EMBD, N_HC, N_HC_SINKHORN_ITER, N_HEAD, N_HEAD_DIM, N_HEAD_KV, N_LORA_Q, N_ROT,
    N_VOCAB, RMS_EPS, ROPE_FREQ_BASE, ROPE_YARN_BETA_FAST, ROPE_YARN_BETA_SLOW,
};
...
const SCHEMA: &str = "ds4.decode_layer0_qkv_rope.v1";
const CASE: &str = "token0_layer0_qkv_rope";
const TOKEN: u32 = 0;
const LAYER: usize = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer0-qkv-rope: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        ...
        read_tensor_output("qr", &qr, q_rank)?,
        read_tensor_output("kv_raw", &kv_raw, kv_dim)?,
        read_tensor_output("qr_norm", &qr_norm, q_rank)?,
        read_tensor_output("q", &q, q_dim)?,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer0-qkv-rope --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer0_qkv_rope_prefix\",");
    println!("    \"method\": \"embed_token_hc+rms_norm_plain+matmul_f16+hc_split_weighted_sum_norm+matmul_q8_0+dsv4_qkv_rms_norm_rows+head_rms_norm+rope_tail\",");
