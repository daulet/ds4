...
const SCHEMA: &str = "ds4.decode_layer2_attn_output.v1";
const CASE: &str = "token0_layer2_attn_output";
const TOKEN: u32 = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer2-attn-output: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        ...
        read_tensor_output("layer2_heads", &heads, q_dim)?,
        read_tensor_output("layer2_attn_low", &attn_low, low_dim)?,
        read_tensor_output("layer2_attn_out", &attn_out, u64::from(N_EMBD))?,
        read_tensor_output("layer2_after_attn_hc", &after_attn_hc, hc_dim)?,
    ];
    write_report(
        &gguf,
        &weights,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer2-attn-output --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer2_attn_output\",");
    println!("    \"method\": \"dense_layer0+swap+dense_layer1+swap+layer2_qkv_rope+kv_fp8_store_raw+attn_compressor_update+indexer_compressor_update+attention_decode_heads+inverse_rope+attention_output_low_q8+matmul_q8_0_hc_expand\",");
