...
use ds4_gpu::graph_plan::{
    GraphPlan, HC_EPS, N_EMBD, N_HC, N_HC_SINKHORN_ITER, N_HEAD, N_HEAD_DIM, N_HEAD_KV, N_LORA_O,
    N_LORA_Q, N_OUT_GROUP, N_ROT, N_VOCAB, RMS_EPS, ROPE_FREQ_BASE, ROPE_YARN_BETA_FAST,
    ROPE_YARN_BETA_SLOW,
};
...
const SCHEMA: &str = "ds4.decode_layer0_attn_output.v1";
const CASE: &str = "token0_layer0_attn_output";
const TOKEN: u32 = 0;
const LAYER: usize = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer0-attn-output: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        read_tensor_output("kv", &kv, kv_dim)?,
        ...
        read_tensor_output("heads", &heads, q_dim)?,
        read_tensor_output("attn_low", &attn_low, low_dim)?,
        read_tensor_output("attn_out", &attn_out, u64::from(N_EMBD))?,
        read_tensor_output("after_attn_hc", &after_attn_hc, hc_dim)?,
    ];
    write_report(
        &gguf,
        &weights,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer0-attn-output --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer0_attn_output_prefix\",");
    println!("    \"method\": \"qkv_rope_prefix+kv_fp8_store_raw+attention_decode_heads+rope_tail_inverse+attention_output_low_q8+matmul_q8_0_hc_expand\",");
