...
use ds4_gpu::graph_plan::{
    ...
    N_HEAD_DIM, N_HEAD_KV, N_LORA_O, N_LORA_Q, N_OUT_GROUP, N_ROT, N_VOCAB, RMS_EPS,
...
const SCHEMA: &str = "ds4.decode_layer0_ffn_output.v1";
const CASE: &str = "token0_layer0_ffn_output";
const TOKEN: u32 = 0;
const LAYER: usize = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer0-ffn-output: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut router_selected = Tensor::allocate(byte_len_i32(u64::from(N_EXPERT_USED))?)
    ...
    let outputs = vec![
        Output::F32(read_tensor_output("after_attn_hc", &after_attn_hc, hc_dim)?),
        Output::F32(read_tensor_output("ffn_cur", &ffn_cur, u64::from(N_EMBD))?),
        ...
        Output::F32(read_tensor_output("shared_mid", &shared_mid, shared_dim)?),
        Output::F32(read_tensor_output(
            "shared_out",
            &shared_out,
            u64::from(N_EMBD),
        )?),
        Output::F32(read_tensor_output("after_ffn_hc", &after_ffn_hc, hc_dim)?),
    ];
    write_report(
        &gguf,
        &weights,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer0-ffn-output --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer0_ffn_output_prefix\",");
    println!("    \"method\": \"layer0_attn_output_prefix+hc_split_weighted_sum_norm+router_select+routed_moe_one+shared_gate_up_swiglu_q8_0+shared_down_hc_expand_q8_0\",");
    ...
    println!("    \"rope_freq_scale\": 1,");
    println!("    \"rope_ext_factor\": 0,");
    println!("    \"rope_attn_factor\": 1,");
    ...
    write_weight("attn_q_a", "base.layer.0.attn_q_a", &layer0.attn_q_a, true);
    write_weight(
        "attn_q_a_norm",
        "base.layer.0.attn_q_a_norm",
        &layer0.attn_q_a_norm,
        true,
    );
    write_weight("attn_q_b", "base.layer.0.attn_q_b", &layer0.attn_q_b, true);
    write_weight("attn_kv", "base.layer.0.attn_kv", &layer0.attn_kv, true);
    write_weight(
        "attn_kv_a_norm",
        "base.layer.0.attn_kv_a_norm",
        &layer0.attn_kv_a_norm,
        true,
    );
    write_weight(
        "attn_sinks",
        "base.layer.0.attn_sinks",
        &layer0.attn_sinks,
        true,
    );
    write_weight(
        "attn_output_a",
        "base.layer.0.attn_output_a",
        &layer0.attn_output_a,
        true,
    );
    write_weight(
        "attn_output_b",
        "base.layer.0.attn_output_b",
        &layer0.attn_output_b,
        true,
    );
    write_weight(
        "hc_ffn_fn",
        "base.layer.0.hc_ffn_fn",
        &layer0.hc_ffn_fn,
        true,
    );
    write_weight(
        "hc_ffn_scale",
        "base.layer.0.hc_ffn_scale",
        &layer0.hc_ffn_scale,
        true,
    );
    write_weight(
        "hc_ffn_base",
        "base.layer.0.hc_ffn_base",
        &layer0.hc_ffn_base,
        true,
    );
    write_weight("ffn_norm", "base.layer.0.ffn_norm", &layer0.ffn_norm, true);
    write_weight(
        "ffn_gate_inp",
        "base.layer.0.ffn_gate_inp",
        &layer0.ffn_gate_inp,
        true,
    );
    if let Some(tensor) = &layer0.ffn_exp_probs_b {
        write_weight(
            "ffn_exp_probs_b",
            "base.layer.0.ffn_exp_probs_b",
            tensor,
            true,
        );
    }
    write_weight(
        ...
        &layer0.ffn_gate_exps,
        true,
    );
    write_weight(
        "ffn_up_exps",
        "base.layer.0.ffn_up_exps",
        &layer0.ffn_up_exps,
        true,
    );
    write_weight(
        "ffn_down_exps",
        "base.layer.0.ffn_down_exps",
        &layer0.ffn_down_exps,
        true,
    );
    write_weight(
        "ffn_gate_shexp",
        "base.layer.0.ffn_gate_shexp",
        &layer0.ffn_gate_shexp,
        true,
    );
    write_weight(
        "ffn_up_shexp",
        "base.layer.0.ffn_up_shexp",
        &layer0.ffn_up_shexp,
        true,
    );
    write_weight(
        "ffn_down_shexp",
        "base.layer.0.ffn_down_shexp",
        &layer0.ffn_down_shexp,
        false,
    );
    println!("  }},");
    println!("  \"outputs\": {{");
