use ds4_gpu::mtp_suffix_plan::{MtpSuffixVerifierCase, MTP_SUFFIX_ORCHESTRATION_CASES};
use std::io::{self, Write};

fn main() {
    let mut out = io::BufWriter::new(io::stdout());
    writeln!(out, "{{").unwrap();
    writeln!(out, "  \"schema\": \"ds4.rust_mtp_suffix_plan.v1\",").unwrap();
    writeln!(
        out,
        "  \"source\": \"rust-model-free-mtp-suffix-orchestration\","
    )
    .unwrap();
    writeln!(out, "  \"oracle\": \"metal_graph_verify_suffix_tops\",").unwrap();
    writeln!(out, "  \"cases\": [").unwrap();
    for (index, case) in MTP_SUFFIX_ORCHESTRATION_CASES.iter().copied().enumerate() {
        if index != 0 {
            writeln!(out, ",").unwrap();
        }
...
fn write_case(out: &mut impl Write, case: MtpSuffixVerifierCase) -> io::Result<()> {
    writeln!(out, "    {{")?;
    write_str_field(out, "id", case.id)?;
    write_str_array_field(out, "source_functions", case.source_functions)?;
    write_str_array_field(out, "command_boundaries", case.command_boundaries)?;
    write_str_field(out, "verifier_input", case.verifier_input)?;
    write_str_field(out, "capture_prefix1", case.capture_prefix1)?;
    write_str_field(out, "snapshot_requirement", case.snapshot_requirement)?;
    write_str_field(out, "commit_rule", case.commit_rule)?;
