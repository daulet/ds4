...
const SCHEMA: &str = "ds4.decode_layer0_output_head.v1";
const CASE: &str = "token0_layer0_output_head";
const TOKEN: u32 = 0;
const LAYER: usize = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer0-output-head: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        read_tensor_output("after_ffn_hc", &after_ffn_hc, hc_dim)?,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer0-output-head --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer0_output_head\",");
    println!("    \"method\": \"layer0_ffn_output_prefix+rms_norm_plain+matmul_f16+output_hc_weights+hc_weighted_sum+rms_norm_weight+matmul_q8_0\",");
    println!("    \"command_batch\": true,");
    println!("    \"synchronized\": true,");
    println!("    \"token\": {TOKEN},");
    println!("    \"layer\": {LAYER},");
    println!("    \"position\": {POSITION},");
    println!("    \"decoded_layers\": 1,");
