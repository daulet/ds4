...
use ds4_gpu::graph_plan::{
    ...
    N_FF_EXP, N_HC, N_HC_SINKHORN_ITER, N_HEAD, N_HEAD_DIM, N_HEAD_KV, N_INDEXER_HEAD_DIM, N_LAYER,
    N_LORA_O, N_LORA_Q, N_OUT_GROUP, N_ROT, N_VOCAB, RMS_EPS, ROPE_FREQ_BASE, ROPE_YARN_BETA_FAST,
    ROPE_YARN_BETA_SLOW,
};
...
const SCHEMA: &str = "ds4.decode_all_layer_final_hc.v1";
const CASE: &str = "token0_all_layer_final_hc";
const TOKEN: u32 = 0;
const POSITION: u32 = 0;
const CTX_SIZE: u32 = 32_768;
const LAYER5: usize = 5;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-all-layer-final-hc: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut after_layer4_hc = Tensor::allocate(byte_len(dims.hc_dim)?)
        .map_err(|err| format!("failed to allocate after_layer4_hc: {err}"))?;
    let mut after_layer5_hc = Tensor::allocate(byte_len(dims.hc_dim)?)
        .map_err(|err| format!("failed to allocate after_layer5_hc: {err}"))?;
    ...
    for layer in 0..N_LAYER {
        ...
        match layer {
            4 => after_layer4_hc
                ...
                .map_err(|err| format!("after_layer4_hc copy failed: {err}"))?,
            LAYER5 => after_layer5_hc
                .copy_from(
                    &state.cur_hc,
                    0,
                    0,
                ...
                .map_err(|err| format!("after_layer5_hc copy failed: {err}"))?,
            LAYER42 => after_layer42_hc
                .copy_from(
                    &state.cur_hc,
                    0,
                    0,
    ...
    let layer5_attn_state_dim = attn_state_dim(LayerCompression::Ratio128);
    let layer42_attn_state_dim = attn_state_dim(LayerCompression::Ratio4);
    let layer42_index_state_dim = index_state_dim(LayerCompression::Ratio4);
    let outputs = vec![
        read_tensor_output("after_layer4_hc", &after_layer4_hc, dims.hc_dim)?,
        read_tensor_output("after_layer5_hc", &after_layer5_hc, dims.hc_dim)?,
        read_tensor_output("after_layer42_hc", &after_layer42_hc, dims.hc_dim)?,
        read_tensor_output_offset(
            "layer5_raw_cache_row",
            &state.raw_cache[LAYER5],
            u64::from(raw_row) * dims.kv_dim * 4,
            dims.kv_dim,
        )?,
        read_tensor_output(
            "layer5_attn_state_kv",
            state.layer_attn_state_kv[LAYER5]
                .as_ref()
                .ok_or("layer5 attn state kv missing")?,
            layer5_attn_state_dim,
        )?,
        read_tensor_output(
            "layer5_attn_state_score",
            state.layer_attn_state_score[LAYER5]
                .as_ref()
        ...
        read_tensor_output_offset(
            ...
            &state.raw_cache[LAYER42],
            u64::from(raw_row) * dims.kv_dim * 4,
            dims.kv_dim,
        )?,
        read_tensor_output(
            "layer42_attn_state_kv",
            state.layer_attn_state_kv[LAYER42]
                .as_ref()
                .ok_or("layer42 attn state kv missing")?,
            layer42_attn_state_dim,
        )?,
        read_tensor_output(
            "layer42_attn_state_score",
            state.layer_attn_state_score[LAYER42]
                .as_ref()
                .ok_or("layer42 attn state score missing")?,
            layer42_attn_state_dim,
        )?,
        read_tensor_output(
            "layer42_index_state_kv",
            state.layer_index_state_kv[LAYER42]
                .as_ref()
                .ok_or("layer42 index state kv missing")?,
            layer42_index_state_dim,
        )?,
        read_tensor_output(
            "layer42_index_state_score",
            state.layer_index_state_score[LAYER42]
                .as_ref()
                .ok_or("layer42 index state score missing")?,
            layer42_index_state_dim,
        )?,
    ];
...
fn execute_layer(
    ...
    if compressed {
        ...
        if (POSITION + 1) % ratio == 0 {
            return Err("all-layer final-HC fixture unexpectedly emitted compressed rows".into());
        }
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-all-layer-final-hc --model FILE".into());
            }
        }
...
fn write_report(
    ...
    let layer5 = &weights.layers[LAYER5];
    let layer42 = &weights.layers[LAYER42];
    println!("{{");
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"rust_gpu_all_layer_final_hc\",");
    println!("    \"method\": \"decode_backend_execute_layer_x43+swap_cur_after_ffn_hc\",");
    ...
    println!("    \"decoded_layers\": {N_LAYER},");
    ...
    write_weight(
        "layer5_attn_compressor_kv",
        "layer5.attn_compressor_kv",
        layer5
            .attn_compressor_kv
            .as_ref()
            .expect("layer5 attn_compressor_kv"),
        true,
    );
    write_weight(
        "layer5_attn_compressor_gate",
        "layer5.attn_compressor_gate",
        layer5
            .attn_compressor_gate
            .as_ref()
            .expect("layer5 attn_compressor_gate"),
        true,
    );
    write_weight(
        "layer5_attn_compressor_ape",
        "layer5.attn_compressor_ape",
        layer5
            .attn_compressor_ape
            .as_ref()
            .expect("layer5 attn_compressor_ape"),
        true,
    );
    write_weight(
        "layer5_attn_compressor_norm",
        "layer5.attn_compressor_norm",
        layer5
            .attn_compressor_norm
            .as_ref()
            .expect("layer5 attn_compressor_norm"),
        true,
    );
    write_weight(
        "layer5_ffn_down_shexp",
        "layer5.ffn_down_shexp",
        &layer5.ffn_down_shexp,
        true,
    );
    write_weight(
    ...
    write_weight(
        "layer42_attn_compressor_kv",
        "layer42.attn_compressor_kv",
        layer42
            .attn_compressor_kv
            .as_ref()
            .expect("layer42 attn_compressor_kv"),
        true,
    );
    write_weight(
        "layer42_attn_compressor_gate",
        "layer42.attn_compressor_gate",
        layer42
            .attn_compressor_gate
            .as_ref()
            .expect("layer42 attn_compressor_gate"),
        true,
    );
    write_weight(
        "layer42_attn_compressor_ape",
        "layer42.attn_compressor_ape",
        layer42
            .attn_compressor_ape
            .as_ref()
            .expect("layer42 attn_compressor_ape"),
        true,
    );
    write_weight(
        "layer42_attn_compressor_norm",
        "layer42.attn_compressor_norm",
        layer42
            .attn_compressor_norm
            .as_ref()
            .expect("layer42 attn_compressor_norm"),
        true,
    );
    write_weight(
        "layer42_indexer_compressor_kv",
        "layer42.indexer_compressor_kv",
        layer42
            .indexer_compressor_kv
            .as_ref()
            .expect("layer42 indexer_compressor_kv"),
        true,
    );
    write_weight(
        "layer42_indexer_compressor_gate",
        "layer42.indexer_compressor_gate",
        layer42
            .indexer_compressor_gate
            .as_ref()
            .expect("layer42 indexer_compressor_gate"),
        true,
    );
    write_weight(
        "layer42_indexer_compressor_ape",
        "layer42.indexer_compressor_ape",
        layer42
            .indexer_compressor_ape
            .as_ref()
            .expect("layer42 indexer_compressor_ape"),
        true,
    );
    write_weight(
        "layer42_indexer_compressor_norm",
        "layer42.indexer_compressor_norm",
        layer42
            .indexer_compressor_norm
            .as_ref()
            .expect("layer42 indexer_compressor_norm"),
        true,
    );
    write_weight(
        "layer42_ffn_down_shexp",
        "layer42.ffn_down_shexp",
        &layer42.ffn_down_shexp,
        false,
    );
    println!("  }},");
