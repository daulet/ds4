...
const SCHEMA: &str = "ds4.decode_two_layer_output_head.v1";
const CASE: &str = "token0_two_layer_output_head";
const TOKEN: u32 = 0;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-two-layer-output-head: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outputs = vec![
        read_tensor_output("after_layer0_hc", &after_ffn_hc, hc_dim)?,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-two-layer-output-head --model FILE".into());
            }
        }
...
fn write_report(
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_two_layer_output_head\",");
    println!("    \"method\": \"dense_layer0+swap+dense_layer1+swap+rms_norm_plain+matmul_f16+output_hc_weights+hc_weighted_sum+rms_norm_weight+matmul_q8_0\",");
