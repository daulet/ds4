...
const SCHEMA: &str = "ds4.decode_layer4_ffn_output.v1";
const CASE: &str = "token0_layer4_ffn_output";
const TOKEN: u32 = 0;
const FIRST_LAYER: usize = 0;
const LAST_DENSE_LAYER: usize = 1;
const LAYER2: usize = 2;
const LAYER3: usize = 3;
const TARGET_LAYER: usize = 4;
const LAYER2_COMPRESSION_RATIO: u32 = 4;
const LAYER2_COMPRESSOR_COEFFICIENT: u32 = 2;
const LAYER3_COMPRESSION_RATIO: u32 = 128;
const LAYER3_COMPRESSOR_COEFFICIENT: u32 = 1;
const TARGET_COMPRESSION_RATIO: u32 = 4;
const TARGET_COMPRESSOR_COEFFICIENT: u32 = 2;
...
fn main() {
    if let Err(err) = run() {
        eprintln!("ds4-decode-layer4-ffn-output: {err}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let layer4 = weights
        .layers
        .get(TARGET_LAYER)
        .ok_or("DS4 weight binding did not include layer 4")?;
    ...
    let layer3_comp_width = u64::from(LAYER3_COMPRESSOR_COEFFICIENT) * u64::from(N_HEAD_DIM);
    let layer3_attn_state_dim =
        layer3_comp_width * u64::from(LAYER3_COMPRESSOR_COEFFICIENT * LAYER3_COMPRESSION_RATIO);
    ...
    let layer4_comp_width = u64::from(TARGET_COMPRESSOR_COEFFICIENT) * u64::from(N_HEAD_DIM);
    let layer4_index_width =
        u64::from(TARGET_COMPRESSOR_COEFFICIENT) * u64::from(N_INDEXER_HEAD_DIM);
    let layer4_attn_state_dim =
        layer4_comp_width * u64::from(TARGET_COMPRESSOR_COEFFICIENT * TARGET_COMPRESSION_RATIO);
    let layer4_index_state_dim =
        layer4_index_width * u64::from(TARGET_COMPRESSOR_COEFFICIENT * TARGET_COMPRESSION_RATIO);
    let layer4_layer_comp_cap = plan.layer_comp_cap(ds4_gpu::graph_plan::LayerCompression::Ratio4);
    let layer4_emit_compressed_row = (POSITION + 1) % TARGET_COMPRESSION_RATIO == 0;
    let layer4_layer_n_comp = 0u32;
    let layer4_layer_n_index_comp = 0u32;
    let layer4_n_comp = layer4_layer_n_comp;
    let layer4_n_selected = 0u32;
    ...
    let layer4_gate_row_bytes = routed_expert_row_bytes(&layer4.ffn_gate_exps)?;
    let layer4_gate_expert_bytes = expert_mid_dim * layer4_gate_row_bytes;
    let layer4_down_row_bytes = routed_expert_row_bytes(&layer4.ffn_down_exps)?;
    let layer4_down_expert_bytes = routed_out_dim * layer4_down_row_bytes;
    ...
    let layer3_router_hash_rows = layer3
        ...
        .and_then(|tensor| tensor.dims.get(1).copied())
        .unwrap_or(0);
    let layer4_router_hash_rows = layer4
        .ffn_gate_tid2eid
        .as_ref()
        .and_then(|tensor| tensor.dims.get(1).copied())
        .unwrap_or(0);
    ...
    let mut raw_cache_layer4 = Tensor::allocate(byte_len(u64::from(raw_cap) * kv_dim)?)
        .map_err(|err| format!("failed to allocate raw_cache_layer4: {err}"))?;
    ...
    let mut layer4_attn_comp_cache = Tensor::allocate(byte_len(
        u64::from(layer4_layer_comp_cap) * u64::from(N_HEAD_DIM),
    )?)
    .map_err(|err| format!("failed to allocate layer4_attn_comp_cache: {err}"))?;
    let mut layer4_attn_state_kv = Tensor::allocate(byte_len(layer4_attn_state_dim)?)
        .map_err(|err| format!("failed to allocate layer4_attn_state_kv: {err}"))?;
    let mut layer4_attn_state_score = Tensor::allocate(byte_len(layer4_attn_state_dim)?)
        .map_err(|err| format!("failed to allocate layer4_attn_state_score: {err}"))?;
    let mut layer4_index_comp_cache = Tensor::allocate(byte_len(
        u64::from(layer4_layer_comp_cap) * u64::from(N_INDEXER_HEAD_DIM),
    )?)
    .map_err(|err| format!("failed to allocate layer4_index_comp_cache: {err}"))?;
    let mut layer4_index_state_kv = Tensor::allocate(byte_len(layer4_index_state_dim)?)
        .map_err(|err| format!("failed to allocate layer4_index_state_kv: {err}"))?;
    let mut layer4_index_state_score = Tensor::allocate(byte_len(layer4_index_state_dim)?)
        .map_err(|err| format!("failed to allocate layer4_index_state_score: {err}"))?;
    ...
    layer4_attn_comp_cache
        .fill_f32(
            0.0,
            usize::try_from(u64::from(layer4_layer_comp_cap) * u64::from(N_HEAD_DIM))?,
        )
        .map_err(|err| format!("failed to zero layer4_attn_comp_cache: {err}"))?;
    layer4_attn_state_kv
        .fill_f32(0.0, usize::try_from(layer4_attn_state_dim)?)
        .map_err(|err| format!("failed to zero layer4_attn_state_kv: {err}"))?;
    layer4_attn_state_score
        .fill_f32(
            COMPRESSOR_SCORE_INIT,
            usize::try_from(layer4_attn_state_dim)?,
        )
        .map_err(|err| format!("failed to fill layer4_attn_state_score: {err}"))?;
    layer4_index_comp_cache
        .fill_f32(
            0.0,
            usize::try_from(u64::from(layer4_layer_comp_cap) * u64::from(N_INDEXER_HEAD_DIM))?,
        )
        .map_err(|err| format!("failed to zero layer4_index_comp_cache: {err}"))?;
    layer4_index_state_kv
        .fill_f32(0.0, usize::try_from(layer4_index_state_dim)?)
        .map_err(|err| format!("failed to zero layer4_index_state_kv: {err}"))?;
    layer4_index_state_score
        .fill_f32(
            COMPRESSOR_SCORE_INIT,
            usize::try_from(layer4_index_state_dim)?,
        )
        .map_err(|err| format!("failed to fill layer4_index_state_score: {err}"))?;
    ...
    backend
        ...
        .map_err(|err| format!("layer4 rms_norm_plain failed: {err}"))?;
    backend
        .matmul_f16(
            hc_mix.as_tensor_mut(),
            layer4.hc_attn_fn.abs_offset,
            hc_dim,
            hc_mix_dim,
            flat_hc.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 hc_attn_fn matmul_f16 failed: {err}"))?;
    backend
        .hc_split_weighted_sum_norm(
            attn_cur.as_tensor_mut(),
            attn_norm.as_tensor_mut(),
            ...
            hc_mix.as_tensor_ref(),
            cur_hc.as_tensor_ref(),
            layer4.hc_attn_scale.abs_offset,
            layer4.hc_attn_base.abs_offset,
            layer4.attn_norm.abs_offset,
            N_EMBD,
            N_HC,
            N_HC_SINKHORN_ITER,
            HC_EPS,
        ...
        .map_err(|err| format!("layer4 hc_split_weighted_sum_norm failed: {err}"))?;
    backend
        .matmul_q8_0(
            qr.as_tensor_mut(),
            layer4.attn_q_a.abs_offset,
            u64::from(N_EMBD),
            q_rank,
            attn_norm.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 attn_q_a matmul_q8_0 failed: {err}"))?;
    backend
        .matmul_q8_0(
            kv_raw.as_tensor_mut(),
            layer4.attn_kv.abs_offset,
            u64::from(N_EMBD),
            kv_dim,
            attn_norm.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 attn_kv matmul_q8_0 failed: {err}"))?;
    backend
        .dsv4_qkv_rms_norm_rows(
            qr_norm.as_tensor_mut(),
            qr.as_tensor_ref(),
            layer4.attn_q_a_norm.abs_offset,
            N_LORA_Q,
            kv.as_tensor_mut(),
            kv_raw.as_tensor_ref(),
            layer4.attn_kv_a_norm.abs_offset,
            N_HEAD_DIM,
            1,
            RMS_EPS,
        )
        .map_err(|err| format!("layer4 dsv4_qkv_rms_norm_rows failed: {err}"))?;
    backend
        .matmul_q8_0(
            q.as_tensor_mut(),
            layer4.attn_q_b.abs_offset,
            q_rank,
            q_dim,
            qr_norm.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 attn_q_b matmul_q8_0 failed: {err}"))?;
    backend
        .head_rms_norm(q.as_tensor_mut(), 1, N_HEAD, N_HEAD_DIM, RMS_EPS)
        .map_err(|err| format!("layer4 head_rms_norm failed: {err}"))?;
    backend
        .rope_tail(
            q.as_tensor_mut(),
            1,
        ...
        .map_err(|err| format!("layer4 q rope_tail failed: {err}"))?;
    backend
        .rope_tail(
            kv.as_tensor_mut(),
            1,
        ...
        .map_err(|err| format!("layer4 kv rope_tail failed: {err}"))?;
    backend
        .kv_fp8_store_raw(
            kv.as_tensor_mut(),
            raw_cache_layer4.as_tensor_mut(),
            raw_cap,
            raw_row,
            N_HEAD_DIM,
            N_ROT,
        )
        .map_err(|err| format!("layer4 kv_fp8_store_raw failed: {err}"))?;
    backend
        .matmul_f16_pair(
            comp_kv_cur.as_tensor_mut(),
            comp_sc_cur.as_tensor_mut(),
            layer4
                .attn_compressor_kv
                .as_ref()
                .ok_or("layer4 attn_compressor_kv missing")?
                .abs_offset,
            layer4
                .attn_compressor_gate
                .as_ref()
                .ok_or("layer4 attn_compressor_gate missing")?
                .abs_offset,
            u64::from(N_EMBD),
            layer4_comp_width,
            attn_norm.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 attention compressor matmul_f16_pair failed: {err}"))?;
    backend
        .compressor_update(
            comp_kv_cur.as_tensor_ref(),
            comp_sc_cur.as_tensor_ref(),
            layer4_attn_state_kv.as_tensor_mut(),
            layer4_attn_state_score.as_tensor_mut(),
            layer4_attn_comp_cache.as_tensor_mut(),
            layer4
                .attn_compressor_ape
                .as_ref()
                .ok_or("layer4 attn_compressor_ape missing")?
                .abs_offset,
            layer4
                .attn_compressor_ape
                .as_ref()
            ...
            layer4
                .attn_compressor_norm
                .as_ref()
                .ok_or("layer4 attn_compressor_norm missing")?
                .abs_offset,
            layer4
                .attn_compressor_norm
                .as_ref()
        ...
        .map_err(|err| format!("layer4 attention compressor_update failed: {err}"))?;
    backend
        .matmul_f16_pair(
            comp_kv_cur.as_tensor_mut(),
            comp_sc_cur.as_tensor_mut(),
            layer4
                .indexer_compressor_kv
                .as_ref()
                .ok_or("layer4 indexer_compressor_kv missing")?
                .abs_offset,
            layer4
                .indexer_compressor_gate
                .as_ref()
                .ok_or("layer4 indexer_compressor_gate missing")?
                .abs_offset,
            u64::from(N_EMBD),
            layer4_index_width,
            attn_norm.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 indexer compressor matmul_f16_pair failed: {err}"))?;
    backend
        .compressor_update(
            comp_kv_cur.as_tensor_ref(),
            comp_sc_cur.as_tensor_ref(),
            layer4_index_state_kv.as_tensor_mut(),
            layer4_index_state_score.as_tensor_mut(),
            layer4_index_comp_cache.as_tensor_mut(),
            layer4
                .indexer_compressor_ape
                .as_ref()
                .ok_or("layer4 indexer_compressor_ape missing")?
                .abs_offset,
            layer4
                .indexer_compressor_ape
                .as_ref()
            ...
            layer4
                .indexer_compressor_norm
                .as_ref()
                .ok_or("layer4 indexer_compressor_norm missing")?
                .abs_offset,
            layer4
                .indexer_compressor_norm
                .as_ref()
        ...
        .map_err(|err| format!("layer4 indexer compressor_update failed: {err}"))?;
    backend
        .attention_decode_heads(
            heads.as_tensor_mut(),
            layer4.attn_sinks.abs_offset,
            q.as_tensor_ref(),
            raw_cache_layer4.as_tensor_ref(),
            n_raw,
            raw_cap,
            raw_start,
            None,
        ...
        .map_err(|err| format!("layer4 attention_decode_heads failed: {err}"))?;
    backend
        .rope_tail(
            heads.as_tensor_mut(),
            1,
        ...
        .map_err(|err| format!("layer4 heads inverse rope_tail failed: {err}"))?;
    backend
        .attention_output_low_q8(
            attn_low.as_tensor_mut(),
            layer4.attn_output_a.abs_offset,
            group_dim,
            rank,
            N_OUT_GROUP,
            heads.as_tensor_ref(),
        )
        .map_err(|err| format!("layer4 attention_output_low_q8 failed: {err}"))?;
    backend
        .matmul_q8_0_hc_expand(
            after_attn_hc.as_tensor_mut(),
            attn_out.as_tensor_mut(),
            layer4.attn_output_b.abs_offset,
            low_dim,
            u64::from(N_EMBD),
            attn_low.as_tensor_ref(),
            cur_hc.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 matmul_q8_0_hc_expand failed: {err}"))?;
    backend
        .rms_norm_plain(
            flat_hc.as_tensor_mut(),
            after_attn_hc.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 ffn rms_norm_plain failed: {err}"))?;
    backend
        .matmul_f16(
            hc_mix.as_tensor_mut(),
            layer4.hc_ffn_fn.abs_offset,
            hc_dim,
            hc_mix_dim,
            flat_hc.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 hc_ffn_fn matmul_f16 failed: {err}"))?;
    backend
        .hc_split_weighted_sum_norm(
            ffn_cur.as_tensor_mut(),
            ffn_norm.as_tensor_mut(),
            ...
            hc_mix.as_tensor_ref(),
            after_attn_hc.as_tensor_ref(),
            layer4.hc_ffn_scale.abs_offset,
            layer4.hc_ffn_base.abs_offset,
            layer4.ffn_norm.abs_offset,
            N_EMBD,
            N_HC,
            N_HC_SINKHORN_ITER,
            HC_EPS,
        ...
        .map_err(|err| format!("layer4 ffn hc_split_weighted_sum_norm failed: {err}"))?;
    backend
        .matmul_f16(
            router_logits.as_tensor_mut(),
            layer4.ffn_gate_inp.abs_offset,
            u64::from(N_EMBD),
            u64::from(N_EXPERT),
            ffn_norm.as_tensor_ref(),
            1,
        )
        .map_err(|err| format!("layer4 ffn_gate_inp matmul_f16 failed: {err}"))?;
    backend
        .router_select(
            router_selected.as_tensor_mut(),
            router_weights.as_tensor_mut(),
            ...
            layer4
                ...
                .map(|tensor| tensor.abs_offset)
                .unwrap_or(0),
            u32::try_from(layer4_router_hash_rows)?,
            TOKEN,
            0,
            0,
            layer4.ffn_exp_probs_b.is_some(),
            layer4.ffn_gate_tid2eid.is_some(),
            router_logits.as_tensor_ref(),
        )
        .map_err(|err| format!("layer4 router_select failed: {err}"))?;
    backend
        .routed_moe_one(
            routed_out.as_tensor_mut(),
            routed_gate.as_tensor_mut(),
            ...
            routed_mid.as_tensor_mut(),
            routed_down.as_tensor_mut(),
            layer4.ffn_gate_exps.abs_offset,
            layer4.ffn_up_exps.abs_offset,
            layer4.ffn_down_exps.abs_offset,
            layer4.ffn_gate_exps.type_id,
            layer4.ffn_down_exps.type_id,
            layer4_gate_expert_bytes,
            layer4_gate_row_bytes,
            layer4_down_expert_bytes,
            layer4_down_row_bytes,
            u32::try_from(expert_in_dim)?,
            u32::try_from(down_in_dim)?,
            u32::try_from(routed_out_dim)?,
            router_selected.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 routed_moe_one failed: {err}"))?;
    backend
        .shared_gate_up_swiglu_q8_0(
            shared_gate.as_tensor_mut(),
            shared_up.as_tensor_mut(),
            shared_mid.as_tensor_mut(),
            layer4.ffn_gate_shexp.abs_offset,
            layer4.ffn_up_shexp.abs_offset,
            u64::from(N_EMBD),
            shared_dim,
            ffn_norm.as_tensor_ref(),
            SWIGLU_CLAMP_EXP,
        )
        .map_err(|err| format!("layer4 shared_gate_up_swiglu_q8_0 failed: {err}"))?;
    backend
        .shared_down_hc_expand_q8_0(
            after_ffn_hc.as_tensor_mut(),
            shared_out.as_tensor_mut(),
            layer4.ffn_down_shexp.abs_offset,
            shared_dim,
            u64::from(N_EMBD),
            shared_mid.as_tensor_ref(),
            routed_out.as_tensor_ref(),
        ...
        .map_err(|err| format!("layer4 shared_down_hc_expand_q8_0 failed: {err}"))?;
    command_batch
        .finish()
    ...
    let outputs = vec![
        Output::F32(read_tensor_output("after_layer3_hc", &cur_hc, hc_dim)?),
        Output::F32(read_tensor_output_offset(
            "layer4_raw_cache_row",
            &raw_cache_layer4,
        ...
        Output::F32(read_tensor_output("layer4_heads", &heads, q_dim)?),
        Output::F32(read_tensor_output("layer4_attn_low", &attn_low, low_dim)?),
        Output::F32(read_tensor_output(
            "layer4_attn_out",
            &attn_out,
            u64::from(N_EMBD),
        ...
        Output::I32(read_tensor_i32_output(
            "layer4_router_selected",
            &router_selected,
            u64::from(N_EXPERT_USED),
        )?),
        Output::F32(read_tensor_output(
            "layer4_router_weights",
            &router_weights,
            u64::from(N_EXPERT_USED),
        )?),
        Output::F32(read_tensor_output(
    ...
    write_report(
        ...
        layer4_down_row_bytes,
        layer4_down_expert_bytes,
        layer4_router_hash_rows,
        layer4_layer_comp_cap,
        layer4_emit_compressed_row,
        layer4_layer_n_comp,
        layer4_layer_n_index_comp,
        layer4_n_comp,
        layer4_n_selected,
    ...
    fn parse() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        while let Some(arg) = args.next() {
            if arg == "--model" {
                ...
                return Err("usage: ds4-decode-layer4-ffn-output --model FILE".into());
            }
        }
...
fn write_report(
    ...
    let layer4 = &weights.layers[TARGET_LAYER];
    println!("{{");
    ...
    println!("  }},");
    println!("  \"operation\": {{");
    println!("    \"name\": \"ds4_gpu_layer4_ffn_output\",");
    println!("    \"method\": \"dense_layer0+swap+dense_layer1+swap+layer2_full_decode+swap+layer3_full_decode+swap+layer4_qkv_rope+kv_fp8_store_raw+attn_compressor_update+indexer_compressor_update+attention_decode_heads+inverse_rope+attention_output_low_q8+matmul_q8_0_hc_expand+ffn_hc_pre+router_select+routed_moe_one+shared_gate_up_swiglu_q8_0+shared_down_hc_expand_q8_0\",");
    ...
    write_weight(
        "layer4_hc_attn_scale",
        "layer4.hc_attn_scale",
        &layer4.hc_attn_scale,
        true,
    );
    write_weight(
    ...
    write_weight("layer4_attn_q_a", "layer4.attn_q_a", &layer4.attn_q_a, true);
    write_weight("layer4_attn_kv", "layer4.attn_kv", &layer4.attn_kv, true);
    write_weight(
        "layer4_attn_q_a_norm",
        "layer4.attn_q_a_norm",
        &layer4.attn_q_a_norm,
        true,
    );
    write_weight(
        "layer4_attn_kv_a_norm",
        "layer4.attn_kv_a_norm",
        &layer4.attn_kv_a_norm,
        true,
    );
    write_weight("layer4_attn_q_b", "layer4.attn_q_b", &layer4.attn_q_b, true);
    write_weight(
        "layer4_attn_compressor_kv",
        "layer4.attn_compressor_kv",
        layer4
            .attn_compressor_kv
            .as_ref()
            .expect("layer4 attn_compressor_kv"),
        true,
    );
    write_weight(
        "layer4_attn_compressor_gate",
        "layer4.attn_compressor_gate",
        layer4
            .attn_compressor_gate
            .as_ref()
            .expect("layer4 attn_compressor_gate"),
        true,
    );
    write_weight(
        "layer4_attn_compressor_ape",
        "layer4.attn_compressor_ape",
        layer4
            .attn_compressor_ape
            .as_ref()
            .expect("layer4 attn_compressor_ape"),
        true,
    );
    write_weight(
        "layer4_attn_compressor_norm",
        "layer4.attn_compressor_norm",
        layer4
            .attn_compressor_norm
            .as_ref()
            .expect("layer4 attn_compressor_norm"),
        true,
    );
    write_weight(
        "layer4_indexer_compressor_kv",
        "layer4.indexer_compressor_kv",
        layer4
            .indexer_compressor_kv
            .as_ref()
            .expect("layer4 indexer_compressor_kv"),
        true,
    );
    write_weight(
        "layer4_indexer_compressor_gate",
        "layer4.indexer_compressor_gate",
        layer4
            .indexer_compressor_gate
            .as_ref()
            .expect("layer4 indexer_compressor_gate"),
        true,
    );
    write_weight(
        "layer4_indexer_compressor_ape",
        "layer4.indexer_compressor_ape",
        layer4
            .indexer_compressor_ape
            .as_ref()
            .expect("layer4 indexer_compressor_ape"),
        true,
    );
    write_weight(
        "layer4_indexer_compressor_norm",
        "layer4.indexer_compressor_norm",
        layer4
            .indexer_compressor_norm
            .as_ref()
            .expect("layer4 indexer_compressor_norm"),
        true,
    );
    write_weight(
        "layer4_attn_sinks",
    ...
    write_weight(
        "layer4_attn_output_a",
        "layer4.attn_output_a",
        &layer4.attn_output_a,
        true,
    );
    write_weight(
        "layer4_attn_output_b",
        "layer4.attn_output_b",
        &layer4.attn_output_b,
        true,
    );
    write_weight(
    ...
    write_weight("layer4_ffn_norm", "layer4.ffn_norm", &layer4.ffn_norm, true);
    write_weight(
        "layer4_ffn_gate_inp",
        "layer4.ffn_gate_inp",
        &layer4.ffn_gate_inp,
    ...
    if let Some(tensor) = &layer4.ffn_exp_probs_b {
        write_weight(
            "layer4_ffn_exp_probs_b",
            "layer4.ffn_exp_probs_b",
            tensor,
            true,
        );
    }
    if let Some(tensor) = &layer4.ffn_gate_tid2eid {
        write_weight(
            "layer4_ffn_gate_tid2eid",
            "layer4.ffn_gate_tid2eid",
            tensor,
            true,
        );
    }
    write_weight(
        "layer4_ffn_gate_exps",
        "layer4.ffn_gate_exps",
        &layer4.ffn_gate_exps,
        true,
    );
    write_weight(
    ...
    write_weight(
        "layer4_ffn_down_exps",
        "layer4.ffn_down_exps",
        &layer4.ffn_down_exps,
        true,
    );
    write_weight(
        "layer4_ffn_gate_shexp",
        "layer4.ffn_gate_shexp",
        &layer4.ffn_gate_shexp,
        true,
    );
    write_weight(
    ...
    write_weight(
        "layer4_ffn_down_shexp",
        "layer4.ffn_down_shexp",
        &layer4.ffn_down_shexp,
        false,
    );
    println!("  }},");
