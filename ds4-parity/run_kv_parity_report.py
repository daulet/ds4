#!/usr/bin/env python3
"""Run the local DS4 Milestone 7 KV/snapshot parity report."""
...
M78_MANIFEST = ROOT / "ds4-parity" / "baselines" / "kv" / "m7.8" / "manifest.json"
...
class KvParityReport:
    ...
    def run(self) -> None:
        for name, command in build_commands():
            item = ReportItem(name=name, kind="local-build", command=command)
            self.items.append(item)
            self.run_command(item)
        for name, command in comparator_commands():
            item = ReportItem(name=name, kind="kv-comparator", command=command)
            self.items.append(item)
            self.run_command(item)
    ...
    def report_text(self) -> str:
        lines = [
            "DS4 Milestone 7 KV/snapshot parity report",
            f"root: {self.root}",
...
def build_commands() -> list[tuple[str, list[str]]]:
    ...
    return [
        (
            "M7 current-C session payload helper build",
            prefix + ["make", "ds4-session-payload-dump"],
        ),
    ]
...
def comparator_commands() -> list[tuple[str, list[str]]]:
    return [
        (
            "M7.2 current-C KV header/policy oracle schema",
            [sys.executable, "ds4-parity/check_kv_policy_dump.py", "--negative-test"],
        ),
        (
            "M7.3 Rust KV header/policy parity",
            [sys.executable, "ds4-parity/compare_kv_policy.py", "--negative-test"],
        ),
        (
            "M7.4a Rust generic KVC file parity",
            [sys.executable, "ds4-parity/compare_kvc_file.py", "--negative-test"],
        ),
        (
            "M7.4b Rust KVC trailer parity",
            [sys.executable, "ds4-parity/compare_kv_trailer.py", "--negative-test"],
        ),
        (
            "M7.5 current-C DSV4 session payload shape oracle",
            [sys.executable, "ds4-parity/check_session_payload_shape.py", "--negative-test"],
        ),
        (
            "M7.6 Rust DSV4 session payload reader parity",
            [sys.executable, "ds4-parity/compare_session_payload.py", "--negative-test"],
        ),
        (
            "M7.7 Rust KV replay parity",
            [sys.executable, "ds4-parity/compare_kv_replay.py", "--negative-test"],
        ),
        (
            "M7.8 current-C B300 restore oracle schema",
            [
                sys.executable,
...
def refresh_skip_items() -> list[ReportItem]:
    return [
        ReportItem(
            name="B300 M7.8 disk payload and memory snapshot restore refresh",
            kind="b300-refresh",
            status="SKIP",
            reason=(
                "model-backed restore recapture requires the B300 q2-imatrix "
                "model and exact temp-kubeconfig/context workflow"
            ),
            rerun_command=manifest_refresh_command(M78_MANIFEST),
        ),
    ]
...
def main(argv: Iterable[str]) -> int:
    ...
    if args.timeout_seconds <= 0:
        ...
        return 2
    report = KvParityReport(root=args.root, timeout=args.timeout_seconds)
    report.run()
