...
int main(void) {
    ...
    memcpy(model_map + offset, original, sizeof(original));
    reference_weight(want, x_in, original, 7, 1.0e-5f);

    if (setenv("DS4_CUDA_COPY_MODEL", "", 1) != 0 ||
        setenv("DS4_CUDA_COPY_MODEL_CHUNKED", "1", 1) != 0 ||
        unsetenv("DS4_CUDA_NO_MODEL_COPY") != 0 ||
        unsetenv("DS4_CUDA_DIRECT_MODEL") != 0 ||
        unsetenv("DS4_CUDA_WEIGHT_CACHE") != 0 ||
        unsetenv("DS4_CUDA_WEIGHT_PRELOAD") != 0) return 2;
    ...
    if (!ds4_gpu_set_model_fd(-1) ||
        !ds4_gpu_set_model_map_range(model_map, model_size, offset, bytes)) return 5;

    memcpy(model_map + offset, changed, sizeof(changed));
    if (!ds4_gpu_set_model_map_range(model_map, model_size, offset, bytes) ||
        !ds4_gpu_cache_model_range(model_map, model_size, offset, bytes, "whole-registration") ||
    ...
    ds4_gpu_cleanup();
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"page_aligned_model_map\":true,\"empty_copy_model_env_preserves_registration_attempt\":true,\"rejected_whole_map_registration_continues_to_chunk_copy\":true,\"repeated_map_range_reuses_copied_image\":true,\"weighted_output_matches\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
