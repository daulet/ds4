...
typedef CUresult (*cu_mem_alloc_host_fn)(void **, size_t);
...
static uint32_t pinned_alloc_calls = 0;
static uint32_t injected_second_range_stage_alloc_failures = 0;
static uint32_t host_register_calls = 0;
static int fail_future_stage_alloc = 0;

CUresult cuMemAllocHost_v2(void **ptr, size_t bytes) {
    static cu_mem_alloc_host_fn real_alloc = NULL;
    pinned_alloc_calls += 1;
    if (fail_future_stage_alloc) {
        injected_second_range_stage_alloc_failures += 1;
        return CU_ERROR_OUT_OF_MEMORY;
    }
    if (!real_alloc) {
        real_alloc = (cu_mem_alloc_host_fn)dlsym(RTLD_NEXT, "cuMemAllocHost_v2");
    }
    return real_alloc ? real_alloc(ptr, bytes) : CU_ERROR_NOT_SUPPORTED;
}
...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-fd-stage-pool-reuse-XXXXXX";
    ...
    reference_weight(want_first, x_in, file_first, 7, 1.0e-5f);
    reference_weight(want_second, x_in, file_second, 7, 1.0e-5f);
    ...
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, first_offset, weight_bytes, "fd-stage-pool-first") ||
        pinned_alloc_calls != 4 ||
        host_register_calls != register_calls_after_map ||
        !ds4_gpu_rms_norm_weight_tensor(
            out, x, model_map, model_size, first_offset, 7, 1.0e-5f) ||
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
        !close_array(got, want_first, 7)) return 6;

    fail_future_stage_alloc = 1;
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, second_offset, weight_bytes, "fd-stage-pool-second") ||
        pinned_alloc_calls != 4 ||
        ...
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
        !close_array(got, want_second, 7)) return 7;
    ...
    if (!ds4_gpu_rms_norm_weight_tensor(
            out, x, model_map, model_size, first_offset, 7, 1.0e-5f) ||
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
        !close_array(got, want_first, 7) ||
        !ds4_gpu_rms_norm_weight_tensor(
            out, x, model_map, model_size, second_offset, 7, 1.0e-5f) ||
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
        !close_array(got, want_second, 7) ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"buffered_fd_selection_active\":true,\"four_slot_stage_pool_created_once\":true,\"second_range_reuses_stage_pool\":true,\"armed_second_allocation_failure_not_triggered\":true,\"fd_bytes_win_after_pool_reuse\":true,\"cached_ranges_retain_original_fd_bytes\":true,\"registration_fallback_not_entered_for_second_range\":true,\"weighted_outputs_match\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
