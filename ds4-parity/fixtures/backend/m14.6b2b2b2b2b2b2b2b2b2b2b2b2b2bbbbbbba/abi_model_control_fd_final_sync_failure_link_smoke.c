...
typedef CUresult (*cu_stream_synchronize_fn)(CUstream);
...
static uint32_t stream_sync_failures = 0;
static uint32_t host_register_calls = 0;
static int fail_next_stream_sync = 0;

CUresult cuStreamSynchronize(CUstream stream) {
    static cu_stream_synchronize_fn real_synchronize = NULL;
    if (fail_next_stream_sync) {
        fail_next_stream_sync = 0;
        stream_sync_failures += 1;
        return CU_ERROR_INVALID_VALUE;
    }
    if (!real_synchronize) {
        real_synchronize = (cu_stream_synchronize_fn)dlsym(RTLD_NEXT, "cuStreamSynchronize");
    }
    return real_synchronize ? real_synchronize(stream) : CU_ERROR_NOT_SUPPORTED;
}
...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-fd-final-sync-failure-XXXXXX";
    ...
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, first_offset, weight_bytes, "fd-final-sync-first") ||
        stream_sync_failures != 1 ||
    ...
    if (setenv("DS4_CUDA_STRICT_WEIGHT_CACHE", "1", 1) != 0 ||
        !ds4_gpu_cache_model_range(
            model_map, model_size, second_offset, weight_bytes, "fd-final-sync-second") ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"buffered_fd_selection_active\":true,\"interposed_fd_final_sync_failure\":true,\"one_shot_failure_allows_fallback_sync\":true,\"final_sync_failure_retries_without_cache_full_latch\":true,\"non_strict_final_sync_failure_continues_to_cached_device_copy\":true,\"strict_final_sync_failure_continues_to_cached_device_copy\":true,\"first_final_sync_failure_enters_registration_fallback\":true,\"subsequent_final_sync_failure_respects_registration_disable\":true,\"cached_fallback_retains_original_host_bytes\":true,\"host_bytes_precede_file_bytes_after_final_sync_failure\":true,\"weighted_outputs_match\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
