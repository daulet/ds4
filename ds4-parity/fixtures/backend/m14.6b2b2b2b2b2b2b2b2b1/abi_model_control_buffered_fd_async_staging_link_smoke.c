...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-buffered-fd-async-XXXXXX";
    int fd = -1;
    ...
    if (!ds4_gpu_set_model_fd(fd) ||
        ...
        !ds4_gpu_cache_model_range(model_map, model_size, offset, cache_bytes, "buffered-async") ||
    ...
    if (!write_at(fd, host_weights, sizeof(host_weights), (off_t)offset) ||
        !ds4_gpu_cache_model_range(model_map, model_size, offset, cache_bytes, "buffered-async-repeat") ||
        !ds4_gpu_rms_norm_weight_tensor(
            out, x, model_map, model_size, offset, 7, 1.0e-5f) ||
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
