...
static uint32_t injected_read_failures = 0;
static uint32_t host_register_calls = 0;
static int fail_model_reads = 0;
static int selected_model_fd = -1;

ssize_t pread(int fd, void *buf, size_t bytes, off_t offset) {
    static pread_fn real_pread = NULL;
    if (fail_model_reads && fd == selected_model_fd) {
        (void)buf;
        (void)bytes;
        (void)offset;
...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-fd-read-failure-XXXXXX";
    ...
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, first_offset, weight_bytes, "fd-read-failure-first") ||
        injected_read_failures != 1 ||
    ...
    if (setenv("DS4_CUDA_STRICT_WEIGHT_CACHE", "1", 1) != 0 ||
        !ds4_gpu_cache_model_range(
            model_map, model_size, second_offset, weight_bytes, "fd-read-failure-second") ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"buffered_fd_selection_active\":true,\"interposed_fd_read_failure\":true,\"read_failure_retries_without_cache_full_latch\":true,\"non_strict_read_failure_continues_to_cached_device_copy\":true,\"strict_read_failure_continues_to_cached_device_copy\":true,\"first_read_failure_enters_registration_fallback\":true,\"subsequent_read_failure_respects_registration_disable\":true,\"cached_fallback_retains_original_host_bytes\":true,\"host_bytes_precede_file_bytes_after_read_failure\":true,\"weighted_outputs_match\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
