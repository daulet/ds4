...
int main(void) {
    ...
    if (!ds4_gpu_set_model_fd(-1) ||
        ...
        !ds4_gpu_cache_model_range(model_map, model_size, offset, bytes, "registered-fallback") ||
    ...
    ds4_gpu_cleanup();
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"page_aligned_model_map\":true,\"page_bounded_public_range_consumed_by_weighted_rms\":true,\"device_copy_fallback_output_matches\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
