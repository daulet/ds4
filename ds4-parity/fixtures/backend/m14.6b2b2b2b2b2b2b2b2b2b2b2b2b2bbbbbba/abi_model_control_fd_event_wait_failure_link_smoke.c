...
typedef CUresult (*cu_event_synchronize_fn)(CUevent);
...
static uint32_t event_wait_failures = 0;
static uint32_t host_register_calls = 0;
static int fail_event_waits = 0;

CUresult cuEventSynchronize(CUevent event) {
    static cu_event_synchronize_fn real_synchronize = NULL;
    if (fail_event_waits) {
        event_wait_failures += 1;
        return CU_ERROR_INVALID_VALUE;
    }
    if (!real_synchronize) {
        real_synchronize = (cu_event_synchronize_fn)dlsym(RTLD_NEXT, "cuEventSynchronize");
    }
    return real_synchronize ? real_synchronize(event) : CU_ERROR_NOT_SUPPORTED;
}
...
int main(void) {
    ...
    const uint64_t chunk_bytes = 16 * 1024 * 1024;
    const uint64_t weight_bytes = 7 * sizeof(float);
    const uint64_t range_bytes = chunk_bytes * 4 + weight_bytes;
    const uint64_t first_offset = page_size;
    const uint64_t second_offset = chunk_bytes * 6;
    const uint64_t model_size = second_offset + range_bytes + page_size;
    ...
    char file_path[] = "/tmp/ds4-fd-event-wait-failure-XXXXXX";
    ...
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, first_offset, range_bytes, "fd-event-wait-first") ||
        event_wait_failures != 1 ||
    ...
    if (setenv("DS4_CUDA_STRICT_WEIGHT_CACHE", "1", 1) != 0 ||
        !ds4_gpu_cache_model_range(
            model_map, model_size, second_offset, range_bytes, "fd-event-wait-second") ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"buffered_fd_selection_active\":true,\"multi_chunk_event_wait_selected\":true,\"interposed_fd_event_wait_failure\":true,\"event_wait_failure_retries_without_cache_full_latch\":true,\"non_strict_event_wait_failure_continues_to_cached_device_copy\":true,\"strict_event_wait_failure_continues_to_cached_device_copy\":true,\"first_event_wait_failure_enters_registration_fallback\":true,\"subsequent_event_wait_failure_respects_registration_disable\":true,\"cached_fallback_retains_original_host_bytes\":true,\"host_bytes_precede_file_bytes_after_event_wait_failure\":true,\"weighted_outputs_match\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
