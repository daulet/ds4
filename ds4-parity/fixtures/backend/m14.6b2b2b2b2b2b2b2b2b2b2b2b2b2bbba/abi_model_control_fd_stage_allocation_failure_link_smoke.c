...
static uint32_t stage_alloc_failures = 0;
static uint32_t host_register_calls = 0;
static int fail_stage_alloc = 0;
...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-fd-stage-allocation-failure-XXXXXX";
    ...
    if (!ds4_gpu_cache_model_range(
            model_map, model_size, first_offset, weight_bytes, "fd-stage-allocation-first") ||
        stage_alloc_failures != 1 ||
    ...
    if (setenv("DS4_CUDA_STRICT_WEIGHT_CACHE", "1", 1) != 0 ||
        !ds4_gpu_cache_model_range(
            model_map, model_size, second_offset, weight_bytes, "fd-stage-allocation-second") ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"buffered_fd_selection_active\":true,\"interposed_initial_stage_allocation_failure\":true,\"stage_failure_retries_without_cache_full_latch\":true,\"non_strict_stage_failure_continues_to_cached_device_copy\":true,\"strict_stage_failure_continues_to_cached_device_copy\":true,\"first_stage_failure_enters_registration_fallback\":true,\"subsequent_stage_failure_respects_registration_disable\":true,\"cached_fallback_retains_original_host_bytes\":true,\"host_bytes_precede_file_bytes_after_stage_failure\":true,\"weighted_outputs_match\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
