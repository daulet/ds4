...
int main(void) {
    ...
    char file_path[] = "/tmp/ds4-buffered-fd-cache-XXXXXX";
    int fd = -1;
    ...
    if (!ds4_gpu_set_model_fd(fd) ||
        ...
        !ds4_gpu_cache_model_range(model_map, model_size, offset, bytes, "buffered-fd") ||
    ...
    if (!write_full(fd, file_image, sizeof(file_image)) ||
        !ds4_gpu_cache_model_range(model_map, model_size, offset, bytes, "buffered-fd-repeat") ||
        !ds4_gpu_rms_norm_weight_tensor(
            out, x, model_map, model_size, offset, 7, 1.0e-5f) ||
        !ds4_gpu_tensor_read(out, 0, got, sizeof(got)) ||
    ...
    close(fd);
    free(allocation);
    puts("{\"c_linked_rust_staticlib\":true,\"page_aligned_host_map\":true,\"fd_before_map_binds_host_base\":true,\"buffered_fd_weight_cache_selected\":true,\"fd_bytes_precede_mutated_host_map\":true,\"repeated_cache_reuses_fd_device_copy\":true,\"weighted_output_matches\":true,\"embedded_libdevice_module_loaded\":true}");
    return 0;
}
