#!/usr/bin/env python3
"""Compare Rust full output-head/logits readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_full_output_head_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
...
ORACLE_SCHEMA = "ds4.full_output_head_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_full_output_head.v1"
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust full output-head current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-full-output-head-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-full-output-head")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_full_output_head_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_full_output_head_oracle_json", "C oracle dump implementation missing"),
        ...
        ("metal_graph_encode_output_head(&g", "C oracle does not use production output-head encoder"),
    ...
    report.check(
        "int ds4_dump_full_output_head_oracle_json" in texts["ds4_h"],
        "header declaration for full output-head oracle missing",
    )
    makefile = texts["makefile"]
    report.check("ds4-full-output-head-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_full_output_head_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2b Rust full output-head comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2b B300 full output-head oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-full-output-head-oracle-dump" in report_text
        and "compare_decode_full_output_head.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    readme = texts["readme"]
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2b" in readme
        and "compare_decode_full_output_head.py" in readme
        and "ds4-full-output-head-oracle-dump" in readme,
        "README entry missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a: Rust All-Layer Final HC B300 Execution" in texts["roadmap"],
        "roadmap all-layer final-HC predecessor split missing",
    )
    report.check(
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2b: Rust Full One-Token Output Head And Logits B300 Execution" in texts["todo"],
        "TODO output-head/logits item missing",
    )
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_full_output_head.removed"),
        ...
        ("remove report comparator", "report", "compare_decode_full_output_head.py", "compare_decode_full_output_head_removed.py"),
        (
            "remove roadmap item",
            "roadmap",
            "M10.5c4c2b2b2b2b2b2b2b2b2b: Rust Full One-Token Output Head And Logits B300 Execution",
            "M10.5c4c2b2b2b2b2b2b2b2b2b removed",
        ),
    ]
    failures: list[str] = []
...
def print_errors(errors: list[str]) -> None:
    print("Rust full output-head comparator failures:")
    for error in errors:
        print(f"  - {error}")
