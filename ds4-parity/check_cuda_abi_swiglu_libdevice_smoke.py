#!/usr/bin/env python3
"""Validate the M14.6b2b2b1 Rust CUDA SwiGLU libdevice ABI smoke."""
...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.6b2b2b1/abi-swiglu-libdevice-smoke.json"
...
HARNESS = ROOT / "ds4-parity/fixtures/backend/m14.6b2b2b1/abi_swiglu_link_smoke.c"
...
CURRENT_SUCCESSOR_SYMBOLS = [
    ...
    "ds4_gpu_hc_split_sinkhorn_tensor", "ds4_gpu_hc_split_weighted_sum_tensor", "ds4_gpu_hc_split_weighted_sum_norm_tensor", "ds4_gpu_output_hc_weights_tensor", "ds4_gpu_embed_token_hc_tensor", "ds4_gpu_embed_tokens_hc_tensor", "ds4_gpu_head_rms_norm_tensor", "ds4_gpu_dsv4_fp8_kv_quantize_tensor",
]
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"M14.6b2b2b1 Rust CUDA SwiGLU libdevice ABI smoke: {status} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_swiglu_libdevice_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.6b2b2b1", "milestone drift")
    report.check(
        fixture.get("status") == "b300-pass-staticlib-swiglu-libdevice-abi",
        "status drift",
    )
...
def validate_oracle(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(oracle.get("symbol") == "ds4_gpu_swiglu_tensor", "oracle symbol drift")
    for marker in [
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        ...
        report.check(marker in texts["kernels"], f"embedded libdevice marker missing: {marker}")
    for marker in [
        "pub const M14_6B2B2B1_SCOPE",
        "exported_abi_symbol_count: 21",
        "exported_compute_symbol_count: 5",
        "owns_swiglu_tensor: true",
        "uses_embedded_libdevice_link_path: true",
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(implementation.get("kernel_entry") == "abi_swiglu_kernel", "kernel entry drift")
    report.check(
        implementation.get("libdevice_path") == "cuda_host::ltoir::build_cubin_from_ptx_with_libdevice",
        "libdevice path drift",
    )
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 91),
        ("feature_release_test_count", 93),
        ("generated_artifact_target", "sm_80"),
        ("linked_cubin_target", "sm_103"),
    ]:
        report.check(execution.get(key) == expected, f"execution drift: {key}")
    report.check(
        "std::f32::<impl f32>::exp" in execution.get("non_release_feature_test_blocker", ""),
        "non-release feature blocker missing",
    )
    ...
    report.check("find /tmp" in execution.get("cleanup_check_command", ""), "temporary cleanup proof missing")
    observed = require_dict(report, execution.get("observed"), "observed")
    for key in [
        "c_linked_rust_staticlib",
        "embedded_libdevice_module_loaded",
        "swiglu_clamped_output_matches",
        "swiglu_unclamped_output_matches",
        "swiglu_alias_output_matches",
    ...
    for marker in [
        "ds4_gpu_swiglu_tensor(out, gate, up, 6, 1.5f, 0.75f)",
        "ds4_gpu_swiglu_tensor(out, gate, up, 4, 0.0f, 0.75f)",
        "ds4_gpu_swiglu_tensor(gate, gate, up, 6, 1.5f, 0.75f)",
        "ds4_gpu_swiglu_tensor(NULL, gate, up, 6, 1.5f, 0.75f)",
        "embedded_libdevice_module_loaded",
    ]:
    ...
    report.check(any("non-release" in value for value in risks), "debug codegen risk missing")


def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = "ds4-parity/baselines/backend/m14.6b2b2b1/abi-swiglu-libdevice-smoke.json"
    checker = "check_cuda_abi_swiglu_libdevice_smoke.py"
    item = "M14.6b2b2b1: SwiGLU Libdevice ABI Export"
    ...
    report.check("M14.6b2b2b1 SwiGLU Libdevice ABI Export" in texts["status"], "status evidence missing")
    report.check(checker in texts["readme"], "README checker wiring missing")
    report.check(checker in texts["report"], "unified report checker wiring missing")
    report.check(
        fixture.get("next_required_stage") == "M14.6b2b2b2 Remaining Rust CUDA Kernel ABI Assembly",
        "next stage drift",
    )
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("missing swiglu export", lambda value: value["exported_symbols"].remove("ds4_gpu_swiglu_tensor")),
        ...
        ("libdevice module failure", lambda value: value["b300_execution"]["observed"].update({"embedded_libdevice_module_loaded": False})),
        ("alias failure", lambda value: value["b300_execution"]["observed"].update({"swiglu_alias_output_matches": False})),
        ("cleanup failure", lambda value: value["b300_execution"]["observed"].update({"temporary_link_artifacts_cleaned": False})),
    ]:
