#!/usr/bin/env python3
"""Validate the Rust CUDA public fused hyperconnection split weighted-sum ABI smoke."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-hc-split-weighted-sum-smoke.json"
...
HARNESS = ROOT / f"ds4-parity/fixtures/backend/{MILESTONE_DIR}/abi_hc_split_weighted_sum_link_smoke.c"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA public HC split weighted-sum ABI smoke: {state} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_hc_split_weighted_sum_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(
        fixture.get("status") == "b300-pass-staticlib-public-hc-split-weighted-sum-abi",
        "status drift",
    )
...
def validate_oracle(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(oracle.get("symbol") == "ds4_gpu_hc_split_weighted_sum_tensor", "oracle symbol drift")
    for marker in [
        ...
        "cuda_model_range_ptr(model_map, scale_offset",
        "cuda_model_range_ptr(model_map, base_offset",
    ]:
        report.check(marker in texts["cuda_c"], f"current-C fused weighted-sum marker missing: {marker}")
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("ds4_gpu_hc_split_weighted_sum_tensor" in symbols, "fused weighted-sum export missing")
    ...
    for marker in [
        ...
        report.check(marker in texts["abi"], f"Rust fused weighted-sum ABI marker missing: {marker}")
    for marker in [
        ...
        "fn hc_split_weighted_sum_tensor(",
    ]:
        report.check(marker in texts["kernels"], f"embedded fused weighted-sum marker missing: {marker}")
    for marker in [
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(
        implementation.get("kernel_entry") == "abi_hc_split_weighted_sum_fused_kernel",
        "kernel entry drift",
    )
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 132),
        ("feature_release_test_count", 139),
        ("staticlib_export_count", 46),
    ]:
        report.check(execution.get(key) == expected, f"execution drift: {key}")
    observed = require_dict(report, execution.get("observed"), "observed")
    for key in [
        "c_linked_rust_staticlib",
        "fused_split_weighted_output_matches",
        "emitted_split_matches",
        "output_row_count_drives_launch",
        ...
        "invalid_hc_count_rejected",
        "embedded_hc_split_weighted_sum_fused_kernel_loaded",
    ]:
        report.check(observed.get(key) is True, f"observed smoke drift: {key}")
    report.check(observed.get("predecessor_c_linked_regression_consumers_passed") == 42, "predecessor count drift")
    report.check(observed.get("predecessor_relink_executable_stack_warning_count") == 42, "warning count drift")
    for marker in [
    ...
    report.check(any("normalized fused reduction" in value for value in risks), "norm boundary missing")
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-hc-split-weighted-sum-smoke.json"
    checker = "check_cuda_abi_hc_split_weighted_sum_smoke.py"
    item = f"{MILESTONE}: Public Hyperconnection Split Weighted Sum ABI"
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("fused result removed", lambda value: value["b300_execution"]["observed"].update({"fused_split_weighted_output_matches": False})),
        ("split result removed", lambda value: value["b300_execution"]["observed"].update({"emitted_split_matches": False})),
        ("buffer rejection removed", lambda value: value["b300_execution"]["observed"].update({"short_residual_rejected": False})),
        ("norm overclaim", lambda value: value["ownership"].update({"owns_hc_split_weighted_sum_norm_tensor": True})),
