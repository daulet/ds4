...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("pub unsafe extern \"C\" fn ds4_gpu_set_model_map" not in texts["abi"], "public model-map control overclaim")
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(
        "Active item: M14.6b2b2b2b2 Public Model-Map Control ABI Assembly" in texts["status"],
        "active item missing",
    )
