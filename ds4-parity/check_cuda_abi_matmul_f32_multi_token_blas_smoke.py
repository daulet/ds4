#!/usr/bin/env python3
"""Validate the Rust CUDA public multi-token F32 BLAS projection ABI smoke."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-matmul-f32-multi-token-blas-smoke.json"
...
HARNESS = ROOT / f"ds4-parity/fixtures/backend/{MILESTONE_DIR}/abi_matmul_f32_multi_token_blas_link_smoke.c"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA public multi-token F32 BLAS projection ABI smoke: {state} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_matmul_f32_multi_token_blas_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(fixture.get("status") == "b300-pass-staticlib-multi-token-f32-blas-abi", "status drift")
...
def validate_oracle(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        ...
        report.check(marker in texts["cuda_c"], f"current-C F32 BLAS oracle marker missing: {marker}")
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        ...
        report.check(marker in texts["abi"], f"Rust F32 BLAS ABI marker missing: {marker}")
    for marker in [
        "pub fn abi_matmul_f32_kernel",
        "matmul_f32_kernel: CudaFunction",
        'load_function("abi_matmul_f32_kernel")',
    ]:
        report.check(marker in texts["kernels"], f"embedded single-token F32 marker missing: {marker}")
    for marker in [
        "pub struct CudaAbiF32BlasProjectionScope",
        "pub const M14_6B2B2B2B2B2B2B2B2B2B2B2B2B2BBBBBBBBBBBA_SCOPE",
        "consumes_single_token_f32_projection: true",
        "owns_matmul_f32_multi_token_blas_tensor: true",
        "owns_default_f32_blas_math_selection: true",
        "owns_quality_mode_mutation: false",
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("project_f32" in implementation.get("owned_dispatch_boundary", ""), "owned dispatch boundary missing")
    report.check("F16" in implementation.get("remaining_compute_boundary", ""), "remaining F16 boundary missing")
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 123),
        ("feature_release_test_count", 130),
    ...
    for key in [
        ...
        "single_token_base_predecessor_matches",
        "multi_token_cublas_output_matches",
        "cached_f32_weights_survive_blas_after_host_mutation",
        "no_tf32_init_selection_survives_env_unset",
        "zero_token_rejected",
        "invalid_model_range_rejected",
        "null_model_rejected",
        "cuda_oxide_blas_adapter_loaded",
        "historical_single_token_negative_witness_superseded",
    ]:
    ...
    report.check(any("F16" in value for value in risks), "F16 boundary missing")
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-matmul-f32-multi-token-blas-smoke.json"
    checker = "check_cuda_abi_matmul_f32_multi_token_blas_smoke.py"
    item = f"{MILESTONE}: Public Multi-Token F32 BLAS Projection ABI"
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("BLAS output removed", lambda value: value["b300_execution"]["observed"].update({"multi_token_cublas_output_matches": False})),
        ("cached range observation removed", lambda value: value["b300_execution"]["observed"].update({"cached_f32_weights_survive_blas_after_host_mutation": False})),
        ("quality overclaim", lambda value: value["ownership"].update({"owns_quality_mode_mutation": True})),
        ("F16 overclaim", lambda value: value["ownership"].update({"owns_multi_token_f16_blas_projection": True})),
