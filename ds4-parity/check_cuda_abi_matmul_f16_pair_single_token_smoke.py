#!/usr/bin/env python3
"""Validate the Rust CUDA public single-token paired F16 projection ABI smoke."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-matmul-f16-pair-single-token-smoke.json"
...
HARNESS = ROOT / f"ds4-parity/fixtures/backend/{MILESTONE_DIR}/abi_matmul_f16_pair_single_token_link_smoke.c"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA public single-token paired F16 projection ABI smoke: {state} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(
        fixture.get("schema") == "ds4.cuda_abi_matmul_f16_pair_single_token_smoke.v1",
        "schema drift",
    )
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(
        fixture.get("status") == "b300-pass-staticlib-single-token-f16-pair-projection-abi",
        "status drift",
    )
...
def validate_oracle(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(oracle.get("symbols") == ["ds4_gpu_matmul_f16_pair_tensor"], "oracle symbol drift")
    for marker in [
        ...
        report.check(marker in texts["cuda_c"], f"current-C F16-pair oracle marker missing: {marker}")
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(len(symbols) == 31, "Rust ABI export implementation count drift")
    ...
    for marker in [
        ...
        report.check(marker in texts["abi"], f"Rust F16-pair ABI marker missing: {marker}")
    for marker in [
        ...
        report.check(marker in texts["kernels"], f"embedded F16-pair kernel marker missing: {marker}")
    for marker in [
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 121),
        ("feature_release_test_count", 128),
        ("staticlib_export_count", 31),
    ]:
        report.check(execution.get(key) == expected, f"execution drift: {key}")
    observed = require_dict(report, execution.get("observed"), "observed")
    for key in [
        "c_linked_rust_staticlib",
        "single_token_default_paired_output_matches",
        "single_token_no_pair_independent_output_matches",
        "single_token_no_ordered_independent_output_matches",
        "single_token_serial_independent_output_matches",
        "cached_pair_weights_survive_host_mutation",
        "multi_token_pair_blas_rejected_until_owned",
        "invalid_second_model_range_rejected",
        "null_model_rejected",
        "embedded_rust_kernel_module_loaded",
        "single_token_projection_predecessor_regression_passed",
    ]:
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-matmul-f16-pair-single-token-smoke.json"
    checker = "check_cuda_abi_matmul_f16_pair_single_token_smoke.py"
    item = f"{MILESTONE}: Public Single-Token Paired F16 Projection ABI"
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("paired observation removed", lambda value: value["b300_execution"]["observed"].update({"single_token_default_paired_output_matches": False})),
        ("fallback observation removed", lambda value: value["b300_execution"]["observed"].update({"single_token_no_pair_independent_output_matches": False})),
