#!/usr/bin/env python3
"""Compare Rust short decode-continuation output-head readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_short_continuation_output_head_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-short-continuation-output-head.rs",
...
ORACLE_SCHEMA = "ds4.short_continuation_output_head_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_short_continuation_output_head.v1"
CASE = "tokens0_21_short_continuation_output_head"
...
EXPECTED_FNV1A64 = {
    ...
    "layer42_attn_state_kv": "cf3576176ae9d092",
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust short continuation output-head current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-short-continuation-output-head-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-short-continuation-output-head")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        (
            "ds4_dump_short_continuation_output_head_oracle_json",
            "C helper does not call exported short-continuation oracle dump",
        ),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_short_continuation_output_head_oracle_json", "C oracle dump implementation missing"),
        ("metal_graph_alloc_raw_cap(&g", "C oracle does not allocate the raw-cap graph"),
        ("for (uint32_t pos = 0; ok && pos < sequence_len; pos++)", "C oracle does not loop the short sequence"),
        ("metal_graph_eval_token_raw_swa(&g", "C oracle does not use production decode continuation"),
        ("const uint32_t sequence_len = 22", "C oracle sequence length drift"),
    ...
    report.check(
        "int ds4_dump_short_continuation_output_head_oracle_json" in texts["ds4_h"],
        "header declaration for short-continuation output-head oracle missing",
    )
    makefile = texts["makefile"]
    report.check("ds4-short-continuation-output-head-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_short_continuation_output_head_oracle_dump_cpu.o" in makefile, "CPU helper object missing")

    rust_bin = texts["rust_bin"]
    for needle, message in (
        (CANDIDATE_SCHEMA, "Rust candidate schema missing"),
        (CASE, "Rust candidate case missing"),
        ("const SEQUENCE_LEN: u32 = 22", "Rust sequence length drift"),
        ("GraphPlan::for_context(CTX_SIZE, CTX_SIZE, false)", "graph plan raw-cap setup missing"),
        ("for position in 0..SEQUENCE_LEN", "Rust candidate does not loop the short sequence"),
    ...
    report.check(
        "M10.5c4d1 Rust short continuation output-head comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4d1 B300 short continuation output-head oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-short-continuation-output-head-oracle-dump" in report_text
        and "compare_decode_short_continuation_output_head.py" in report_text,
    ...
    report.check(
        "M10.5c4d1" in readme
        and "compare_decode_short_continuation_output_head.py" in readme
        and "ds4-short-continuation-output-head-oracle-dump" in readme,
        "README entry missing",
    )
    report.check(
        "M10.5c4d1: Rust Short Decode-Continuation Output-Head B300 Execution" in texts["roadmap"],
        "roadmap short-continuation item missing",
    )
    report.check(
        "M10.5c4d2: Rust Ratio-Boundary Continuation Coverage" in texts["roadmap"],
        "roadmap ratio-boundary follow-up split missing",
    )
    report.check(
        "M10.5c4d1: Rust Short Decode-Continuation Output-Head B300 Execution" in texts["todo"],
        "TODO short-continuation item missing",
    )
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_short_continuation_output_head.removed"),
        ...
        (
            ...
            "report",
            "compare_decode_short_continuation_output_head.py",
            "compare_decode_short_continuation_output_head_removed.py",
        ),
        (
            "remove roadmap item",
            "roadmap",
...
def print_errors(errors: list[str]) -> None:
    print("Rust short continuation output-head comparator failures:")
    for error in errors:
        print(f"  - {error}")
