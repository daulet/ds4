...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.4d2/attention-decode-batch-mixed-smoke.json"
CARGO = ROOT / "rust/ds4-cuda/Cargo.toml"
LOCK = ROOT / "Cargo.lock"
CRATE_LIB = ROOT / "rust/ds4-cuda/src/lib.rs"
SMOKE = ROOT / "rust/ds4-cuda/src/bin/attention_decode_batch_mixed_smoke.rs"
...
EXPECTED_OWNED = [
    "executable-local cuda-oxide generic batched mixed attention decode launch proof",
    "raw-batch and mixed-batch causal window, ring-wrap, visible-compressed, mask, and sink numeric validation",
]
EXPECTED_NOT_CLAIMED = [
    "heads8 online decode kernel or dispatch selection",
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"M14.4d2 attention batched mixed smoke: {status} ({report.checks} checks)")
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(
        fixture.get("schema") == "ds4.attention_decode_batch_mixed_smoke.v1",
        "schema drift",
    )
    report.check(fixture.get("milestone") == "M14.4d2", "milestone drift")
    ...
    report.check(
        'name = "ds4-cuda-attention-decode-batch-mixed-smoke"' in texts["cargo"],
        "binary missing",
    )
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 58, "feature test count drift")
    ...
    report.check(
        "--bin ds4-cuda-attention-decode-batch-mixed-smoke" in command,
...
def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.4d2/attention-decode-batch-mixed-smoke.json"
    checker = "check_attention_decode_batch_mixed_smoke.py"
    item = "M14.4d2: Generic Batched Mixed Attention Decode Surfaces"
    report.check(item in texts["roadmap"], "roadmap item missing")
    report.check(fixture in texts["roadmap"], "roadmap fixture missing")
    report.check(item in texts["todo"], "TODO item missing")
    report.check(fixture in texts["todo"], "TODO fixture missing")
    report.check(
        "Active item: M14.4d3 Heads8 Online Attention Decode Kernels" in texts["status"],
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        (
            "batched mask output absent",
            lambda value: value["b300_execution"]["stdout"].update({"batched_mask_matches": False}),
        ),
        (
            "online overclaim",
            lambda value: value["ownership"].update({"owns_heads8_online_decode": True}),
        ),
        (
            "prefill overclaim",
