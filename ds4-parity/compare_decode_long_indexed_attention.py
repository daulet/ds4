#!/usr/bin/env python3
"""Compare Rust long indexed-attention decode readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_long_indexed_attention_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-long-indexed-attention.rs",
...
ORACLE_SCHEMA = "ds4.long_indexed_attention_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_long_indexed_attention.v1"
CASE = "tokens0_2051_long_indexed_attention"
MODEL_SIZE = 86720111488
TENSOR_DATA_OFFSET = 5333824
OUTPUTS = {
    "after_layer2_hc": 16384,
    "layer2_heads": 32768,
    "layer2_attn_out": 4096,
    "layer2_after_attn_hc": 16384,
    "layer2_indexer_q": 8192,
    "layer2_indexer_weights": 64,
    "layer2_indexer_scores": 513,
    "layer2_comp_selected": 512,
    "layer2_raw_cache_row": 512,
    "layer2_attn_comp_row512": 512,
    "layer2_index_comp_row512": 128,
}
OUTPUT_DTYPES = {
    "layer2_comp_selected": "i32",
}
WEIGHT_ROLES = {
    "token_embd": "base.token_embd",
    "layer2_attn_sinks": "layer2.attn_sinks",
    "layer2_indexer_attn_q_b": "layer2.indexer_attn_q_b",
    "layer2_indexer_proj": "layer2.indexer_proj",
...
EXPECTED_FNV1A64 = {
    "after_layer2_hc": "8e3d1c2ef4ac4e1f",
    ...
    "layer2_attn_comp_row512": "25b13ef81b3cc643",
    "layer2_index_comp_row512": "8bf040cdf84597fb",
}
EXPECTED_WEIGHTS = {
    "token_embd": (77928033088, 1059061760, 1, "f16"),
    "layer2_attn_sinks": (79275269952, 256, 0, "f32"),
    "layer2_indexer_attn_q_b": (79296778816, 16777216, 1, "f16"),
    "layer2_indexer_proj": (79296254528, 524288, 1, "f16"),
...
EXPECTED_OPERATION = {
    ...
    "last_layer": 2,
    "indexed_layer": 2,
    "full_decode_tokens": 2051,
    "decoded_layers_per_full_token": 43,
    "final_decoded_layers": 3,
    "total_decode_layer_calls": 88196,
    "dense_layers": 2,
    "ratio4_layers": 21,
    "ratio128_layers": 20,
    "allow_split_flush": 1,
    ...
    "indexer_q_dim": 8192,
    "indexer_scores_dim": 513,
    "layer2_comp_cap": 8194,
    "layer2_n_comp": 513,
    "layer2_n_index_comp": 513,
    "layer2_final_comp_row": 512,
}
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust long indexed attention current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-long-indexed-attention-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-long-indexed-attention")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        (
            "ds4_dump_long_indexed_attention_oracle_json",
            "C helper does not call exported long indexed-attention oracle dump",
        ),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_long_indexed_attention_oracle_json", "C oracle dump implementation missing"),
        ("metal_graph_alloc_raw_cap(&g", "C oracle does not allocate the raw-cap graph"),
        ("for (uint32_t pos = 0; ok && pos < final_pos; pos++)", "C oracle does not warm up the long indexed sequence"),
        ("metal_graph_eval_token_raw_swa(&g", "C oracle does not use production decode continuation"),
        ("metal_graph_encode_decode_layer(&g", "C oracle does not manually execute the final indexed layer"),
        ("il <= indexed_layer", "C oracle does not stop the final token at the indexed layer"),
        ("DS4_N_INDEXER_TOP_K * 4u + 4u", "C oracle sequence length does not cross the indexer threshold"),
        ("g.layer_n_comp[indexed_layer] <= DS4_N_INDEXER_TOP_K", "C oracle does not fail closed before indexed threshold"),
        ("ds4_gpu_tensor_read(g.comp_selected", "C oracle does not read selected top-k rows"),
        ("layer0_attn_oracle_write_i32_output(fp, \"layer2_comp_selected\"", "C oracle does not emit selected rows as i32"),
        ("\"layer2_heads\"", "C oracle does not emit layer2 indexed-attention heads"),
        ("\"layer2_attn_out\"", "C oracle does not emit layer2 attention output"),
        ("\"layer2_after_attn_hc\"", "C oracle does not emit layer2 attention HC"),
        ("\"layer2_indexer_q\"", "C oracle does not emit indexer q"),
        ("\"layer2_indexer_weights\"", "C oracle does not emit indexer weights"),
        ("\"layer2_indexer_scores\"", "C oracle does not emit indexer scores"),
        ("\"layer2_raw_cache_row\"", "C oracle does not emit layer2 raw cache row"),
        ("\"layer2_attn_comp_row512\"", "C oracle does not emit final compressed attention row"),
        ("\"layer2_index_comp_row512\"", "C oracle does not emit final compressed index row"),
        ("\"after_layer2_hc\"", "C oracle does not emit final HC output"),
        (ORACLE_SCHEMA, "C oracle schema missing"),
    ):
        report.check(needle in ds4_c, message)

    report.check(
        "int ds4_dump_long_indexed_attention_oracle_json" in texts["ds4_h"],
        "header declaration for long indexed attention oracle missing",
    )
    makefile = texts["makefile"]
    report.check("ds4-long-indexed-attention-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_long_indexed_attention_oracle_dump_cpu.o" in makefile, "CPU helper object missing")

    rust_bin = texts["rust_bin"]
    for needle, message in (
        (CANDIDATE_SCHEMA, "Rust candidate schema missing"),
        (CASE, "Rust candidate case missing"),
        ("const SEQUENCE_LEN: u32 = N_INDEXER_TOP_K * 4 + 4", "Rust sequence length does not cross the indexer threshold"),
        ("GraphPlan::for_context(CTX_SIZE, CTX_SIZE, false)", "graph plan raw-cap setup missing"),
        ("for position in 0..SEQUENCE_LEN", "Rust candidate does not loop the long indexed sequence"),
        ("let layer_limit = if position == FINAL_POSITION", "Rust candidate does not use a partial final-token layer limit"),
        ("for layer in 0..layer_limit", "Rust candidate does not loop through the selected layer limit"),
        ...
        (".indexer_score_one(", "Rust indexer score facade call missing"),
        (".indexer_topk(", "Rust top-k selected-row facade call missing"),
        (".attention_indexed_mixed_batch_heads(", "Rust indexed attention facade call missing"),
        ...
        ("\"after_layer2_hc\"", "final HC readback missing"),
        ("\"layer2_heads\"", "indexed heads readback missing"),
        ("read_tensor_i32_output(\n            \"layer2_comp_selected\"", "selected top-k row readback missing"),
        ("\"layer2_index_comp_row512\"", "final index comp row readback missing"),
    ...
    report.check(
        "M10.5c4d3 Rust long indexed attention comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4d3 B300 long indexed attention oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-long-indexed-attention-oracle-dump" in report_text
        and "compare_decode_long_indexed_attention.py" in report_text,
    ...
    report.check(
        "M10.5c4d3" in readme
        and "compare_decode_long_indexed_attention.py" in readme
        and "ds4-long-indexed-attention-oracle-dump" in readme,
        "README entry missing",
    )
    report.check(
        "M10.5c4d3: Rust Long Indexed-Continuation Attention Coverage" in texts["roadmap"],
        "roadmap indexed-continuation item missing",
    )
    report.check(
        "M10.5c4d3: Rust Long Indexed-Continuation Attention Coverage" in texts["todo"],
        "TODO indexed-continuation item missing",
    )
...
def validate_output(report: Report, output: Any, label: str, field: str, elements: int) -> None:
    ...
    expected_dtype = OUTPUT_DTYPES.get(field)
    if expected_dtype is not None:
        report.check(output.get("dtype") == expected_dtype, f"{label} {field} dtype drift")
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_long_indexed_attention.removed"),
        ("remove C sequence loop", "ds4_c", "for (uint32_t pos = 0; ok && pos < final_pos; pos++)", "for_removed"),
        ("remove C selected rows", "ds4_c", "\"layer2_comp_selected\"", "\"layer2_comp_selected_removed\""),
        ("remove Rust sequence loop", "rust_bin", "for position in 0..SEQUENCE_LEN", "for position in 0..1"),
        ("remove Rust final layer limit", "rust_bin", "let layer_limit = if position == FINAL_POSITION", "let layer_limit = if false"),
        ...
        ("remove Rust indexed attention", "rust_bin", ".attention_indexed_mixed_batch_heads(", ".attention_indexed_mixed_batch_heads_removed("),
        ("remove Rust selected row read", "rust_bin", "read_tensor_i32_output(\n            \"layer2_comp_selected\"", "read_tensor_i32_output(\n            \"removed\""),
        (
            "remove report comparator",
            "report",
            "compare_decode_long_indexed_attention.py",
            "compare_decode_long_indexed_attention_removed.py",
        ),
        (
            "remove roadmap item",
            "roadmap",
...
def valid_outputs(include_sha: bool) -> dict[str, Any]:
    ...
    for idx, (field, elements) in enumerate(OUTPUTS.items()):
        ...
        if field in OUTPUT_DTYPES:
            output["dtype"] = OUTPUT_DTYPES[field]
            output["samples"] = [{"index": sample, "value": idx + sample} for sample in sample_indices(elements)]
        if include_sha:
...
def mutate_candidate_digest(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["after_layer2_hc"]["fnv1a64"] = "0000000000000000"
...
def mutate_candidate_tensor_size(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer2_indexer_scores"]["elements"] = 1


def mutate_candidate_weight_offset(candidate: dict[str, Any]) -> None:
    candidate["weights"]["layer2_indexer_proj"]["abs_offset"] += 4


def mutate_candidate_layer_count(candidate: dict[str, Any]) -> None:
    candidate["operation"]["final_decoded_layers"] = 42


def print_errors(errors: list[str]) -> None:
    print("Rust long indexed attention comparator failures:")
    for error in errors:
        print(f"  - {error}")
