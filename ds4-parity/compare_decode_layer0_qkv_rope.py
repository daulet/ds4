#!/usr/bin/env python3
"""Compare Rust layer-0 QKV/RoPE readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_layer0_qkv_rope_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-layer0-qkv-rope.rs",
...
ORACLE_SCHEMA = "ds4.layer0_qkv_rope_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_layer0_qkv_rope.v1"
...
EXPECTED_FNV1A64: dict[str, str] = {
    "attn_norm": "24e0d5fc736b2ace",
    "qr": "a04804371dd4a6ae",
    "kv_raw": "a1c0a2aae4fedc0e",
    "qr_norm": "059a86592fc239a9",
    "q": "a4af0bca4d611025",
    "kv": "c18013c1fe8e0391",
}
EXPECTED_WEIGHTS: dict[str, tuple[int, int, int, str]] = {
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust layer-0 QKV/RoPE current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-layer0-qkv-rope-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-layer0-qkv-rope")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_layer0_qkv_rope_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_layer0_qkv_rope_oracle_json", "C oracle dump implementation missing"),
        ...
        ("ds4_gpu_matmul_q8_0_tensor(kv_raw_tensor", "C oracle does not compute raw KV projection"),
        ("ds4_gpu_dsv4_qkv_rms_norm_rows_tensor(qr_norm_tensor", "C oracle does not use fused QKV RMS norm"),
        ("ds4_gpu_matmul_q8_0_tensor(q_tensor", "C oracle does not compute Q projection"),
        ("ds4_gpu_head_rms_norm_tensor(q_tensor", "C oracle does not apply head RMS norm"),
        ("ds4_gpu_rope_tail_tensor(q_tensor", "C oracle does not apply Q RoPE"),
        ("ds4_gpu_rope_tail_tensor(kv_tensor", "C oracle does not apply KV RoPE"),
    ...
    report.check(
        "int ds4_dump_layer0_qkv_rope_oracle_json" in texts["ds4_h"],
        "header declaration for layer-0 QKV/RoPE oracle missing",
    )

    makefile = texts["makefile"]
    report.check("ds4-layer0-qkv-rope-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_layer0_qkv_rope_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    for needle, message in (
        ...
        (".matmul_q8_0(", "matmul_q8_0 facade call missing"),
        (".dsv4_qkv_rms_norm_rows(", "fused QKV RMS facade call missing"),
        (".head_rms_norm(", "head RMS facade call missing"),
        (".rope_tail(", "RoPE facade call missing"),
        ("read_tensor_output(\"kv\"", "KV readback missing"),
    ...
    for needle, message in (
        ("pub const ROPE_FREQ_BASE", "ROPE_FREQ_BASE graph constant missing"),
        ("pub const ROPE_YARN_BETA_FAST", "ROPE_YARN_BETA_FAST graph constant missing"),
        ("pub const ROPE_YARN_BETA_SLOW", "ROPE_YARN_BETA_SLOW graph constant missing"),
    ):
        report.check(needle in graph_plan, message)

    report_text = texts["report"]
    report.check(
        "M10.5c4c2b2b2b2a Rust layer-0 QKV/RoPE comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2a B300 layer-0 QKV/RoPE oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-layer0-qkv-rope-oracle-dump" in report_text
        and "compare_decode_layer0_qkv_rope.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    report.check("M10.5c4c2b2b2b2a Rust layer-0 QKV/RoPE" in texts["readme"], "README entry missing")
    report.check(
        "M10.5c4c2b2b2b2a: Rust Layer-0 QKV RoPE B300 Execution" in texts["roadmap"],
        "roadmap split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b: Rust One-Token Decode B300 Execution" in texts["roadmap"],
        "roadmap remainder split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2a: Rust Layer-0 QKV RoPE B300 Execution" in texts["todo"],
        "TODO split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b: Rust One-Token Decode B300 Execution" in texts["todo"],
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_layer0_qkv_rope.removed"),
        ("remove C Q projection", "ds4_c", "ds4_gpu_matmul_q8_0_tensor(qr_tensor", "ds4_gpu_matmul_q8_0_removed(qr_tensor"),
        ("remove C fused norm", "ds4_c", "ds4_gpu_dsv4_qkv_rms_norm_rows_tensor(qr_norm_tensor", "ds4_gpu_dsv4_qkv_rms_norm_removed(qr_norm_tensor"),
        ("remove C RoPE", "ds4_c", "ds4_gpu_rope_tail_tensor(q_tensor", "ds4_gpu_rope_tail_removed(q_tensor"),
        ("remove Rust matmul", "rust_bin", ".matmul_q8_0(", ".matmul_q8_0_removed("),
        ("remove Rust fused norm", "rust_bin", ".dsv4_qkv_rms_norm_rows(", ".dsv4_qkv_rms_norm_rows_removed("),
        ("remove Rust RoPE", "rust_bin", ".rope_tail(", ".rope_tail_removed("),
        ("remove B300 candidate check", "report", "compare_decode_layer0_qkv_rope.py", "compare_decode_layer0_qkv_rope_removed.py"),
        (
            "remove roadmap split",
            "roadmap",
            "M10.5c4c2b2b2b2a: Rust Layer-0 QKV RoPE B300 Execution",
            "M10.5c4c2b2b2b2a removed",
        ),
    ]
    failures: list[str] = []
...
def mutate_candidate_digest(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["q"]["fnv1a64"] = "0000000000000000"


def mutate_candidate_sample(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["kv"]["samples"][0]["value"] += 0.01


def mutate_candidate_tensor_size(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["qr_norm"]["elements"] = 1


def mutate_candidate_weight_offset(candidate: dict[str, Any]) -> None:
    candidate["weights"]["attn_q_b"]["abs_offset"] += 4


def print_errors(errors: list[str]) -> None:
    print("Rust layer-0 QKV/RoPE comparator failures:")
    for error in errors:
        print(f"  - {error}")
