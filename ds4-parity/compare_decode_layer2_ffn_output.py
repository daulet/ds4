#!/usr/bin/env python3
"""Compare Rust layer-2 FFN-output readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_layer2_ffn_output_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-layer2-ffn-output.rs",
...
ORACLE_SCHEMA = "ds4.layer2_ffn_output_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_layer2_ffn_output.v1"
CASE = "token0_layer2_ffn_output"
MODEL_SIZE = 86720111488
TENSOR_DATA_OFFSET = 5333824
OUTPUTS = {
    "after_layer1_hc": (16384, "f32"),
    "layer2_raw_cache_row": (512, "f32"),
    "layer2_attn_state_kv": (8192, "f32"),
    "layer2_attn_state_score": (8192, "f32"),
    "layer2_index_state_kv": (2048, "f32"),
    "layer2_index_state_score": (2048, "f32"),
    "layer2_heads": (32768, "f32"),
    "layer2_attn_low": (8192, "f32"),
    "layer2_attn_out": (4096, "f32"),
    "layer2_after_attn_hc": (16384, "f32"),
    "layer2_ffn_cur": (4096, "f32"),
    "layer2_ffn_norm": (4096, "f32"),
    "layer2_router_logits": (256, "f32"),
    "layer2_router_probs": (256, "f32"),
    "layer2_router_selected": (6, "i32"),
    "layer2_router_weights": (6, "f32"),
    "layer2_routed_mid": (12288, "f32"),
    "layer2_routed_out": (4096, "f32"),
    "layer2_shared_mid": (2048, "f32"),
    "layer2_shared_out": (4096, "f32"),
    "layer2_after_ffn_hc": (16384, "f32"),
}
WEIGHT_ROLES = {
    ...
    "layer2_hc_ffn_fn": "layer2.hc_ffn_fn",
    "layer2_hc_ffn_scale": "layer2.hc_ffn_scale",
    "layer2_hc_ffn_base": "layer2.hc_ffn_base",
    "layer2_ffn_norm": "layer2.ffn_norm",
    "layer2_ffn_gate_inp": "layer2.ffn_gate_inp",
    "layer2_ffn_gate_tid2eid": "layer2.ffn_gate_tid2eid",
    "layer2_ffn_gate_exps": "layer2.ffn_gate_exps",
    "layer2_ffn_up_exps": "layer2.ffn_up_exps",
    "layer2_ffn_down_exps": "layer2.ffn_down_exps",
    "layer2_ffn_gate_shexp": "layer2.ffn_gate_shexp",
    "layer2_ffn_up_shexp": "layer2.ffn_up_shexp",
    "layer2_ffn_down_shexp": "layer2.ffn_down_shexp",
}
OPTIONAL_WEIGHT_ROLES = {
    "layer2_ffn_exp_probs_b": "layer2.ffn_exp_probs_b",
}
EXPECTED_FNV1A64 = {
    ...
    "layer2_ffn_cur": "d0becc7729c8b33d",
    ...
    "layer2_shared_out": "3e7b4f4e70d9b893",
    "layer2_after_ffn_hc": "26babcdeac41b377",
}
EXPECTED_WEIGHTS = {
    ...
    "layer2_hc_ffn_fn": (79456856864, 786432, 1, "f16"),
    "layer2_hc_ffn_scale": (79457643296, 12, 0, "f32"),
    "layer2_hc_ffn_base": (79456856768, 96, 0, "f32"),
    "layer2_ffn_norm": (79456053824, 16384, 0, "f32"),
    "layer2_ffn_gate_inp": (79427217984, 2097152, 1, "f16"),
    ...
    "layer2_ffn_down_exps": (4192168768, 704643072, 10, "q2_k"),
    "layer2_ffn_gate_shexp": (79429315136, 8912896, 8, "q8_0"),
    "layer2_ffn_up_shexp": (79447140928, 8912896, 8, "q8_0"),
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust layer-2 FFN-output current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-layer2-ffn-output-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-layer2-ffn-output")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_layer2_ffn_output_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_layer2_ffn_output_oracle_json", "C oracle dump implementation missing"),
        ...
        ("ds4_gpu_tensor_read(g.ffn_cur", "C oracle does not read layer-2 FFN HC-pre current tensor"),
        ("ds4_gpu_tensor_read(g.ffn_norm", "C oracle does not read layer-2 FFN norm tensor"),
        ("ds4_gpu_tensor_read(g.router_logits", "C oracle does not read layer-2 router logits"),
        ("ds4_gpu_tensor_read(g.router_selected", "C oracle does not read layer-2 router selected tensor"),
        ("ds4_gpu_tensor_read(g.routed_mid", "C oracle does not read layer-2 routed intermediate tensor"),
        ("ds4_gpu_tensor_read(g.shared_mid", "C oracle does not read layer-2 shared expert intermediate tensor"),
        ("ds4_gpu_tensor_read(g.after_ffn_hc", "C oracle does not read final layer-2 after-FFN HC"),
        ("layer0_attn_oracle_write_i32_output(fp, \"layer2_router_selected\"", "C oracle does not emit router_selected as i32"),
        ("layer2_hc_ffn_fn", "C oracle does not report layer-2 HC FFN weights"),
        ("layer2_ffn_gate_tid2eid", "C oracle does not report layer-2 router hash table"),
        ("layer2_ffn_down_shexp", "C oracle does not report layer-2 shared down weight"),
        ("\"layer2_after_ffn_hc\"", "C oracle does not emit final layer-2 after-FFN HC"),
        (ORACLE_SCHEMA, "C oracle schema missing"),
    ):
        report.check(needle in ds4_c, message)

    report.check(
        "int ds4_dump_layer2_ffn_output_oracle_json" in texts["ds4_h"],
        "header declaration for layer-2 FFN-output oracle missing",
    )

    makefile = texts["makefile"]
    report.check("ds4-layer2-ffn-output-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_layer2_ffn_output_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    for needle, message in (
        ...
        ("read_tensor_i32_output(\n            \"layer2_router_selected\"", "router_selected i32 readback missing"),
        ("read_tensor_output(\n            \"layer2_after_ffn_hc\"", "layer-2 after-FFN HC readback missing"),
        ("layer2_ffn_gate_tid2eid", "Rust report does not include layer-2 router hash table"),
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b1 Rust layer-2 FFN-output comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b1 B300 layer-2 FFN-output oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-layer2-ffn-output-oracle-dump" in report_text
        and "compare_decode_layer2_ffn_output.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    report.check("M10.5c4c2b2b2b2b2b2b2b1 Rust layer-2 FFN-output" in texts["readme"], "README entry missing")
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b1: Rust Layer-2 FFN Output B300 Execution" in texts["roadmap"],
        "roadmap FFN-output split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2: Rust One-Token Decode B300 Execution" in texts["roadmap"],
        "roadmap one-token remainder split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b1: Rust Layer-2 FFN Output B300 Execution" in texts["todo"],
        "TODO FFN-output split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2: Rust One-Token Decode B300 Execution" in texts["todo"],
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_layer2_ffn_output.removed"),
        ...
        ("remove C after FFN", "ds4_c", "\"layer2_after_ffn_hc\"", "\"layer2_after_ffn_hc_removed\""),
        ...
        ("remove Rust i32 read", "rust_bin", "read_tensor_i32_output(\n            \"layer2_router_selected\"", "read_tensor_i32_output_removed(\n            \"layer2_router_selected\""),
        ("remove B300 candidate check", "report", "compare_decode_layer2_ffn_output.py", "compare_decode_layer2_ffn_output_removed.py"),
        (
            "remove roadmap split",
            "roadmap",
            "M10.5c4c2b2b2b2b2b2b2b1: Rust Layer-2 FFN Output B300 Execution",
            "M10.5c4c2b2b2b2b2b2b2b1 removed",
        ),
    ]
    failures: list[str] = []
...
def mutate_candidate_digest(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer2_after_ffn_hc"]["fnv1a64"] = "0000000000000000"


def mutate_candidate_sample(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer2_routed_out"]["samples"][0]["value"] += 0.01


def mutate_candidate_router_selected(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer2_router_selected"]["samples"][0]["value"] += 1


def mutate_candidate_tensor_size(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer2_shared_mid"]["elements"] = 1


def mutate_candidate_weight_offset(candidate: dict[str, Any]) -> None:
    candidate["weights"]["layer2_ffn_down_shexp"]["abs_offset"] += 4
...
def print_errors(errors: list[str]) -> None:
    print("Rust layer-2 FFN-output comparator failures:")
    for error in errors:
        print(f"  - {error}")
