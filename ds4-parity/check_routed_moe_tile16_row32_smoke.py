...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.5c2c5/routed-moe-tile16-row32-smoke.json"
...
EXPECTED_OWNED = [
    "executable-local functional tile16 row32 Q2_K/Q8_K atomic-down proof",
    "tile16 down metadata selection paired with retained tile8 gate metadata",
    "high-token tile16 row32 atomic-down scheduling surface",
]
EXPECTED_NOT_CLAIMED = [
    "gate/down row2048 or templated rowspan scheduling and shared-cache specialization",
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"M14.5c2c5 routed MoE tile16 row32 smoke: {status} ({report.checks} checks)")
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.routed_moe_tile16_row32_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.5c2c5", "milestone drift")
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 81, "feature test count drift")
    ...
    expected = {
        ...
        "tile16_atomic_down_matches": True,
        "tile16_partial_tile_matches": True,
        "gate_tile8_metadata_retained": True,
    ...
    for marker in [
        ...
        'std::env::var_os("DS4_CUDA_MOE_DOWN_TILE16")',
...
def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.5c2c5/routed-moe-tile16-row32-smoke.json"
    checker = "check_routed_moe_tile16_row32_smoke.py"
    item = "M14.5c2c5: Tile16 Row32 Atomic Down"
    report.check(item in texts["roadmap"], "roadmap item missing")
    report.check(fixture in texts["roadmap"], "roadmap fixture missing")
    report.check(item in texts["todo"], "TODO item missing")
    report.check(fixture in texts["todo"], "TODO fixture missing")
    report.check("Active item: M14.6 CUDA Route Promotion And C CUDA Removal Gate" in texts["status"], "next active missing")
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("tile16 output absent", lambda value: value["b300_execution"]["stdout"].update({"tile16_atomic_down_matches": False})),
        ("gate schedule absent", lambda value: value["b300_execution"]["stdout"].update({"gate_tile8_metadata_retained": False})),
        ("rowspan overclaim", lambda value: value["ownership"].update({"owns_rowspan_dispatch": True})),
    ]:
