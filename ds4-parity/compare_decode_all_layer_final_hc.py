#!/usr/bin/env python3
"""Compare Rust all-layer final-HC readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_all_layer_final_hc_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-all-layer-final-hc.rs",
...
ORACLE_SCHEMA = "ds4.all_layer_final_hc_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_all_layer_final_hc.v1"
CASE = "token0_all_layer_final_hc"
MODEL_SIZE = 86720111488
TENSOR_DATA_OFFSET = 5333824
OUTPUTS = {
    "after_layer4_hc": 16384,
    "after_layer5_hc": 16384,
    ...
    "layer42_attn_state_score": 8192,
    "layer42_index_state_kv": 2048,
    "layer42_index_state_score": 2048,
}
WEIGHT_ROLES = {
    "token_embd": "base.token_embd",
    "layer5_hc_attn_fn": "layer5.hc_attn_fn",
    "layer5_attn_compressor_kv": "layer5.attn_compressor_kv",
    "layer5_attn_compressor_gate": "layer5.attn_compressor_gate",
    "layer5_attn_compressor_ape": "layer5.attn_compressor_ape",
    "layer5_attn_compressor_norm": "layer5.attn_compressor_norm",
    "layer5_ffn_down_shexp": "layer5.ffn_down_shexp",
    "layer42_hc_attn_fn": "layer42.hc_attn_fn",
    "layer42_attn_compressor_kv": "layer42.attn_compressor_kv",
    "layer42_attn_compressor_gate": "layer42.attn_compressor_gate",
    "layer42_attn_compressor_ape": "layer42.attn_compressor_ape",
    "layer42_attn_compressor_norm": "layer42.attn_compressor_norm",
    "layer42_indexer_compressor_kv": "layer42.indexer_compressor_kv",
    "layer42_indexer_compressor_gate": "layer42.indexer_compressor_gate",
    "layer42_indexer_compressor_ape": "layer42.indexer_compressor_ape",
    "layer42_indexer_compressor_norm": "layer42.indexer_compressor_norm",
    "layer42_ffn_down_shexp": "layer42.ffn_down_shexp",
}
EXPECTED_FNV1A64 = {
    ...
    "layer42_index_state_kv": "2f5aefc0f5ed2728",
    "layer42_index_state_score": "6a5003b30aad1406",
}
EXPECTED_WEIGHTS = {
    "token_embd": (77928033088, 1059061760, 1, "f16"),
    "layer5_hc_attn_fn": (79943665312, 786432, 1, "f16"),
    "layer5_attn_compressor_kv": (79796955712, 4194304, 1, "f16"),
    "layer5_attn_compressor_gate": (79792761408, 4194304, 1, "f16"),
    "layer5_attn_compressor_ape": (79792628288, 131072, 1, "f16"),
    "layer5_attn_compressor_norm": (79792759360, 2048, 0, "f32"),
    "layer5_ffn_down_shexp": (79925823040, 8912896, 8, "q8_0"),
    "layer42_hc_attn_fn": (86155764384, 786432, 1, "f16"),
    "layer42_attn_compressor_kv": (85983362112, 8388608, 1, "f16"),
    "layer42_attn_compressor_gate": (85974973504, 8388608, 1, "f16"),
    "layer42_attn_compressor_ape": (85974963264, 8192, 1, "f16"),
    "layer42_attn_compressor_norm": (85974971456, 2048, 0, "f32"),
    "layer42_indexer_compressor_kv": (85993850432, 2097152, 1, "f16"),
    "layer42_indexer_compressor_gate": (85991753280, 2097152, 1, "f16"),
    "layer42_indexer_compressor_ape": (85991750720, 2048, 1, "f16"),
    "layer42_indexer_compressor_norm": (85991752768, 512, 0, "f32"),
    "layer42_ffn_down_shexp": (86137922112, 8912896, 8, "q8_0"),
}
EXPECTED_OPERATION = {
    "token": 0,
    "first_layer": 0,
    ...
    "layer5_comp_cap": 258,
    "layer5_n_comp": 0,
    "layer5_attn_state_dim": 65536,
    "layer42_comp_cap": 8194,
    "layer42_n_comp": 0,
    "layer42_n_index_comp": 0,
    "layer42_attn_state_dim": 8192,
    "layer42_index_state_dim": 2048,
}
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust all-layer final-HC current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-all-layer-final-hc-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-all-layer-final-hc")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_all_layer_final_hc_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_all_layer_final_hc_oracle_json", "C oracle dump implementation missing"),
        ("metal_graph_alloc_raw_cap(&g", "C oracle does not allocate the raw-cap graph"),
        ("for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++)", "C oracle does not loop all layers"),
        ("metal_graph_encode_decode_layer(&g", "C oracle does not use production decode-layer encoder"),
        ("g.cur_hc = g.after_ffn_hc", "C oracle does not swap cur/after HC buffers"),
        ("ds4_gpu_tensor_copy(after_layer42_tensor", "C oracle does not checkpoint final layer HC"),
        ("g.layer_attn_state_kv[layer5]", "C oracle does not read layer-5 compressor state"),
        ("g.layer_index_state_kv[layer42]", "C oracle does not read layer-42 index state"),
        ("layer42_indexer_compressor_norm", "C oracle does not report layer-42 indexer weights"),
    ...
    report.check(
        "int ds4_dump_all_layer_final_hc_oracle_json" in texts["ds4_h"],
        "header declaration for all-layer final-HC oracle missing",
    )
    makefile = texts["makefile"]
    report.check("ds4-all-layer-final-hc-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_all_layer_final_hc_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    for needle, message in (
        ...
        ("layer42_indexer_compressor_norm", "Rust report does not include layer-42 indexer weights"),
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a Rust all-layer final-HC comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a B300 all-layer final-HC oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-all-layer-final-hc-oracle-dump" in report_text
        and "compare_decode_all_layer_final_hc.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    readme = texts["readme"]
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a" in readme
        and "compare_decode_all_layer_final_hc.py" in readme
        and "ds4-all-layer-final-hc-oracle-dump" in readme,
        "README entry missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a: Rust All-Layer Final HC B300 Execution" in texts["roadmap"],
        "roadmap all-layer final-HC split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2b: Rust Full One-Token Output Head And Logits B300 Execution" in texts["roadmap"],
        "roadmap output-head/logits remainder split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b2b2b2a: Rust All-Layer Final HC B300 Execution" in texts["todo"],
        "TODO all-layer final-HC split missing",
    )
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_all_layer_final_hc.removed"),
        ("remove C loop", "ds4_c", "for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++)", "for_removed"),
        ("remove C layer42 output", "ds4_c", "\"after_layer42_hc\"", "\"after_layer42_hc_removed\""),
        ("remove Rust loop", "rust_bin", "for layer in 0..N_LAYER", "for layer in 0..1"),
        ("remove Rust compression", "rust_bin", "layer_compression(layer)", "layer_compression_removed(layer)"),
        ("remove Rust final read", "rust_bin", "read_tensor_output(\"after_layer42_hc\"", "read_tensor_output(\"removed\""),
        ("remove report comparator", "report", "compare_decode_all_layer_final_hc.py", "compare_decode_all_layer_final_hc_removed.py"),
        (
            "remove roadmap split",
            "roadmap",
            "M10.5c4c2b2b2b2b2b2b2b2b2a: Rust All-Layer Final HC B300 Execution",
            "M10.5c4c2b2b2b2b2b2b2b2b2a removed",
        ),
    ]
    failures: list[str] = []
...
def mutate_candidate_sample(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer42_attn_state_kv"]["samples"][0]["value"] += 0.01


def mutate_candidate_tensor_size(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["layer5_attn_state_kv"]["elements"] = 1


def mutate_candidate_weight_offset(candidate: dict[str, Any]) -> None:
    candidate["weights"]["layer42_ffn_down_shexp"]["abs_offset"] += 4
...
def print_errors(errors: list[str]) -> None:
    print("Rust all-layer final-HC comparator failures:")
    for error in errors:
        print(f"  - {error}")
