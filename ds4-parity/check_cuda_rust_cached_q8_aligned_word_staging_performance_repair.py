...
def validate_implementation(report: Report, fixture: dict[str, Any], kernels: str, report_text: str) -> None:
    ...
    report.check(
        ...
        "aligned Q8 load usage drift outside registered paired-load/staging successors",
    )
    for block, label, shared in [(gate, "gate", "SXQ"), (down, "down", "SMIDQ")]:
        if pair_staging_successor:
            report.check("let mut staged_pair = thread_index;" in block, f"{label} pair staging missing")
            report.check("ABI_MOE_CACHED_Q8_TAIL_PAIRS" in block, f"{label} pair staging bound missing")
            report.check("abi_moe_cached_store_aligned_u64(" in block, f"{label} paired store missing")
        else:
