#!/usr/bin/env python3
"""Validate retained gate row-span kernel in the Rust CUDA ABI module."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba"
...
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-routed-moe-gate-rowspan-module-smoke.json"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA embedded gate row-span ABI module: {status} ({report.checks} checks)")
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_routed_moe_gate_rowspan_module_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(fixture.get("status") == "b300-pass-staticlib-embedded-gate-rowspan-module", "status drift")
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    entry = "abi_moe_gate_up_mid_expert_tile8_rowspan_kernel"
    ...
    report.check("if write_aux != 0" in texts["kernels"], "auxiliary write control missing")
    report.check("abi_moe_apply_clamp(&mut gate, &mut up, clamp)" in texts["kernels"], "clamp behavior missing")
    report.check("down row-span" in implementation.get("remaining_compute_boundary", ""), "down boundary missing")
    ...
    for key, expected in [
        ...
        ("target", "sm_80"),
        ("local_library_test_count", 159),
        ("feature_release_test_count", 166),
    ...
    for key in [
        ...
        "gate_rowspan_entry_generated",
        "write_aux_and_clamp_semantics_retained",
    ...
    for key, expected in [
        ("local_ds4_cuda_library_test_count", 159),
        ("b300_feature_release_test_count", 166),
        ("cuda_abi_comparators_passed", 73),
        ("unified_report_passed", 246),
...
def validate_wiring(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-routed-moe-gate-rowspan-module-smoke.json"
    checker = "check_cuda_abi_routed_moe_gate_rowspan_module_smoke.py"
    item = f"{MILESTONE}: Embedded Gate Rowspan Routed MoE ABI Module"
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("gate entry missing", lambda value: value["implementation"]["embedded_kernel_entries"].clear()),
        ("gate ownership missing", lambda value: value["ownership"].update({"owns_gate_rowspan_functional_surface": False})),
        ("down overclaim", lambda value: value["ownership"].update({"owns_down_rowspan_or_shared_cache_dispatch": True})),
        ("kernel count mismatch", lambda value: value["ownership"].update({"embedded_kernel_count": 87})),
