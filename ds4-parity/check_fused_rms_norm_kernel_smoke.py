#!/usr/bin/env python3
"""Validate the M14.3b1 Rust CUDA fused QKV and head RMS normalization smoke."""
...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.3b1/fused-rms-norm-kernel-smoke.json"
CARGO = ROOT / "rust/ds4-cuda/Cargo.toml"
LOCK = ROOT / "Cargo.lock"
CRATE_LIB = ROOT / "rust/ds4-cuda/src/lib.rs"
SMOKE = ROOT / "rust/ds4-cuda/src/bin/fused_rms_norm_smoke.rs"
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.fused_rms_norm_kernel_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.3b1", "milestone drift")
    ...
    report.check('name = "ds4-cuda-fused-rms-norm-smoke"' in texts["cargo"], "smoke binary wiring missing")
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 40, "feature test count drift")
    ...
    report.check("--bin ds4-cuda-fused-rms-norm-smoke" in command, "smoke command missing")
    ...
    report.check(stdout == expected, "B300 fused RMS normalization result drift")
    for marker in [
...
def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.3b1/fused-rms-norm-kernel-smoke.json"
    checker = "check_fused_rms_norm_kernel_smoke.py"
    item = "M14.3b1: Fused QKV And Basic Head RMS Norm Kernels"
    report.check(item in texts["roadmap"], "roadmap item missing")
    report.check(fixture in texts["roadmap"], "roadmap fixture missing")
    report.check(item in texts["todo"], "TODO item missing")
    report.check(fixture in texts["todo"], "TODO fixture missing")
    report.check("Active item: M14." in texts["status"], "next active M14 stage missing")
    report.check("M14.3b1 Fused QKV And Basic Head RMS Norm Kernels" in texts["status"], "status evidence missing")
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("fused QKV result absent", lambda value: value["b300_execution"]["stdout"].update({"qkv_fused_output_matches": False})),
        ("asymmetric-width result absent", lambda value: value["b300_execution"]["stdout"].update({"asymmetric_q_kv_widths_match": False})),
        ("head result absent", lambda value: value["b300_execution"]["stdout"].update({"head_in_place_output_matches": False})),
        ("rope-tail overclaim", lambda value: value["ownership"].update({"owns_head_rms_norm_rope_tail_kernel": True})),
...
def print_report(report: Report) -> None:
    ...
    print(f"M14.3b1 fused RMS normalization kernel smoke: {status} ({report.checks} checks)")
