#!/usr/bin/env python3
"""Compare Rust two-layer output-head readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_two_layer_output_head_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-two-layer-output-head.rs",
...
ORACLE_SCHEMA = "ds4.two_layer_output_head_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_two_layer_output_head.v1"
CASE = "token0_two_layer_output_head"
MODEL_SIZE = 86720111488
TENSOR_DATA_OFFSET = 5333824
OUTPUTS = {
    "after_layer0_hc": 16384,
...
EXPECTED_FNV1A64 = {
    "after_layer0_hc": "3d49316c93ce351f",
    "after_layer1_hc": "f764d7067de5c945",
    "output_pre": "ebc1b8ccc088d27a",
    "output_weights": "e20bda6aca5453b2",
    "output_embd": "b5d1377b7c179886",
    "output_norm": "2ce848a4cc2363db",
    "logits": "14dbbac3cd6ed7a8",
}
EXPECTED_WEIGHTS = {
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust two-layer output-head current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-two-layer-output-head-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-two-layer-output-head")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_two_layer_output_head_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_two_layer_output_head_oracle_json", "C oracle dump implementation missing"),
        ...
        ("for (uint32_t il = 0; ok && il < 2; il++)", "C oracle does not encode exactly two layers"),
        ...
        ("metal_graph_encode_output_head(&g", "C oracle does not use the production output-head encoder"),
        ("ds4_gpu_tensor_read(g.after_ffn_hc", "C oracle does not read layer-0 HC after two swaps"),
        ("ds4_gpu_tensor_read(g.cur_hc", "C oracle does not read layer-1 HC after two swaps"),
        ("ds4_gpu_tensor_read(g.output_pre", "C oracle does not read output_pre"),
        ("ds4_gpu_tensor_read(g.output_weights", "C oracle does not read output_weights"),
        ("ds4_gpu_tensor_read(g.output_embd", "C oracle does not read output_embd"),
        ("ds4_gpu_tensor_read(g.output_norm", "C oracle does not read output_norm"),
        ("ds4_gpu_tensor_read(g.logits", "C oracle does not read logits"),
        (ORACLE_SCHEMA, "C oracle schema missing"),
    ):
        report.check(needle in ds4_c, message)

    report.check(
        "int ds4_dump_two_layer_output_head_oracle_json" in texts["ds4_h"],
        "header declaration for two-layer output-head oracle missing",
    )

    makefile = texts["makefile"]
    report.check("ds4-two-layer-output-head-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_two_layer_output_head_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    for needle, message in (
        ...
        (".rms_norm_plain(", "output-head HC normalization missing"),
        ("weights.output_hc_fn.abs_offset", "output HC function weight missing"),
        ...
        (".matmul_q8_0(", "output vocab projection facade call missing"),
        ("read_tensor_output(\"logits\"", "logits readback missing"),
        ("read_tensor_output(\"after_layer0_hc\"", "layer-0 HC readback missing"),
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b2a Rust two-layer output-head comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2a B300 two-layer output-head oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-two-layer-output-head-oracle-dump" in report_text
        and "compare_decode_two_layer_output_head.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    report.check("M10.5c4c2b2b2b2b2b2a Rust two-layer output-head" in texts["readme"], "README entry missing")
    report.check(
        "M10.5c4c2b2b2b2b2b2a: Rust Two Dense-Layer Output Head B300 Execution" in texts["roadmap"],
        "roadmap output-head split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b: Rust One-Token Decode B300 Execution" in texts["roadmap"],
        "roadmap remainder split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2a: Rust Two Dense-Layer Output Head B300 Execution" in texts["todo"],
        "TODO output-head split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2b: Rust One-Token Decode B300 Execution" in texts["todo"],
...
def samples_match(left: Any, right: Any) -> bool:
    ...
    for index, left_value in left_map.items():
        if abs(float(left_value) - float(right_map[index])) > 1e-6:
            return False
    return True
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_two_layer_output_head.removed"),
        ("remove C decode-layer", "ds4_c", "metal_graph_encode_decode_layer(&g", "metal_graph_encode_decode_layer_removed(&g"),
        ("remove C output-head", "ds4_c", "metal_graph_encode_output_head(&g", "metal_graph_encode_output_head_removed(&g"),
        ("remove C logits read", "ds4_c", "ds4_gpu_tensor_read(g.logits", "ds4_gpu_tensor_read_removed(g.logits"),
        ("remove Rust HC swap", "rust_bin", "std::mem::swap(&mut cur_hc, &mut after_ffn_hc)", "std::mem::swap_removed(&mut cur_hc, &mut after_ffn_hc)"),
        ("remove Rust layer-1 cache", "rust_bin", "raw_cache_layer1", "raw_cache_l1_removed"),
        ("remove Rust output weights", "rust_bin", ".output_hc_weights(", ".output_hc_weights_removed("),
        ("remove Rust HC weighted sum", "rust_bin", ".hc_weighted_sum(", ".hc_weighted_sum_removed("),
        ("remove Rust logits read", "rust_bin", "read_tensor_output(\"logits\"", "read_tensor_output_removed(\"logits\""),
        ("remove B300 candidate check", "report", "compare_decode_two_layer_output_head.py", "compare_decode_two_layer_output_head_removed.py"),
        (
            "remove roadmap split",
            "roadmap",
            "M10.5c4c2b2b2b2b2b2a: Rust Two Dense-Layer Output Head B300 Execution",
            "M10.5c4c2b2b2b2b2b2a removed",
        ),
    ]
    failures: list[str] = []
    ...
    pair_mutations = [
        ...
        ("candidate operation vocab_dim", mutate_candidate_vocab_dim),
    ]
...
def mutate_candidate_digest(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["logits"]["fnv1a64"] = "0000000000000000"


def mutate_candidate_sample(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["output_norm"]["samples"][0]["value"] += 0.01


def mutate_candidate_tensor_size(candidate: dict[str, Any]) -> None:
    candidate["outputs"]["output_embd"]["elements"] = 1
...
def mutate_candidate_vocab_dim(candidate: dict[str, Any]) -> None:
    candidate["operation"]["vocab_dim"] = 42


def print_errors(errors: list[str]) -> None:
    print("Rust two-layer output-head comparator failures:")
    for error in errors:
        print(f"  - {error}")
