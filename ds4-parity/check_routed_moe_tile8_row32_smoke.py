...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.5c2c2/routed-moe-tile8-row32-smoke.json"
...
EXPECTED_OWNED = [
    "executable-local functional tile8 row32 IQ2-XXS/Q8_K gate/up projection proof",
    "executable-local functional tile8 row32 Q2_K/Q8_K non-atomic down projection proof",
    "default tile8 row32 projection dispatch over expert-tile metadata",
]
EXPECTED_NOT_CLAIMED = [
    "shared-cache optimization, tile4, atomic-down, tile16, or rowspan dispatch",
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"M14.5c2c2 routed MoE tile8 row32 smoke: {status} ({report.checks} checks)")
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.routed_moe_tile8_row32_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.5c2c2", "milestone drift")
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 78, "feature test count drift")
    ...
    expected = {
        ...
        "tile8_gate_up_matches": True,
        "tile8_down_matches": True,
        "multi_tile_expert_matches": True,
...
def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.5c2c2/routed-moe-tile8-row32-smoke.json"
    checker = "check_routed_moe_tile8_row32_smoke.py"
    item = "M14.5c2c2: Default Tile8 Row32 Projection"
    report.check(item in texts["roadmap"], "roadmap item missing")
    report.check(fixture in texts["roadmap"], "roadmap fixture missing")
    report.check(item in texts["todo"], "TODO item missing")
    report.check(fixture in texts["todo"], "TODO fixture missing")
    report.check(
        "Active item: M14.5c2d Single-Token Q4_K Routed MoE" in texts["status"],
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        (
            "gate output absent",
            lambda value: value["b300_execution"]["stdout"].update({"tile8_gate_up_matches": False}),
        ),
        (
            "down output absent",
            lambda value: value["b300_execution"]["stdout"].update({"tile8_down_matches": False}),
        ),
        (
            "shared cache overclaim",
            lambda value: value["ownership"].update({"owns_shared_cache_specialization": True}),
        ),
    ]:
