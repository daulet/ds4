...
def validate_implementation(report: Report, fixture: dict[str, Any], kernels: str, report_text: str) -> None:
    ...
    shared_alignment = 8 if paired_successor else 4
    report.check(
        "static mut SXQ: SharedArray<" in gate and f"\n            {shared_alignment},\n" in gate,
        "gate staging alignment missing",
    )
    report.check(
        "static mut SMIDQ: SharedArray<" in down and f"\n            {shared_alignment},\n" in down,
        "down staging alignment missing",
    )
    ...
    expected_aligned_load_uses = (
        5
        if gate_global_load_successor
        else (8 if pair_staging_successor else (4 if paired_successor else (5 if staging_successor else 3)))
    )
    report.check(
