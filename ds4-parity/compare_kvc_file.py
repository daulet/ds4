#!/usr/bin/env python3
"""Compare the Rust generic KVC file port against the M7.4a C oracle."""
...
BASELINE = ROOT / "ds4-parity" / "baselines" / "kv" / "m7.4a" / "current-c.json"
...
def run_rust_dump() -> tuple[int, str, str]:
    proc = subprocess.run(
        ["cargo", "run", "--quiet", "-p", "ds4-gguf", "--bin", "ds4-kvc-file-dump-rs"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def compare_dumps(expected_raw: Any, got_raw: Any) -> Report:
    ...
    report.check(expected.get("schema") == "ds4.kvc_file_oracle.v1", "C schema mismatch")
    report.check(got.get("schema") == "ds4.rust_kvc_file_oracle.v1", "Rust schema mismatch")
    report.check(expected.get("source") == "current-c-kvstore-file-no-model", "C source mismatch")
    report.check(got.get("source") == "rust-kvstore-file-no-model", "Rust source mismatch")
    for key, expected_value in expected.items():
...
def run_negative_tests(expected: Any, got: Any) -> Report:
    ...
    mutations: list[tuple[str, list[str | int], Any]] = [
        ("schema drift", ["schema"], "ds4.kvc_file_oracle.v1"),
        ("full-file byte drift", ["cases", 0, "file_hex"], "00"),
        ("reader trailer drift", ["cases", 1, "read_entry", "trailer_bytes"], 0),
        ("budget drift", ["cases", 2, "budget", "fits"], True),
        ("reader acceptance drift", ["malformed_cases", 0, "read_entry", "ok"], True),
        ("malformed category drift", ["malformed_cases", 6, "expected_error"], "invalid-header"),
        ("truncated trailer drift", ["malformed_cases", 7, "read_entry", "expected_total_ok"], True),
...
def main() -> int:
    ...
    print_report("KVC file C/Rust comparator", compare)

    negative = Report()
    if args.negative_test:
        negative = run_negative_tests(expected, got)
        print_report("KVC file C/Rust negative tests", negative)
