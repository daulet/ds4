#!/usr/bin/env python3
"""Compare Rust CLI runtime-control output against the M8.12b current-C oracle."""
...
import check_cli_runtime_controls_dump as oracle


ROOT = Path(__file__).resolve().parents[1]
BASELINE = ROOT / "ds4-parity" / "baselines" / "cli" / "m8.12b" / "current-c.json"
...
FORBIDDEN_STDERR = (
    ...
    b"M8.13c one-shot implementation",
...
def check_candidate_case(
    ...
    report.check(case.get("availability") in {"executed", "blocked", "blocked_missing_mtp_model"}, f"{case_id}.availability invalid")
    report.check(isinstance(case.get("support_artifact"), (str, type(None))), f"{case_id}.support_artifact invalid")
...
def compare_dump(obj: Any, outputs: dict[str, CandidateOutput]) -> Report:
    ...
    report.check(preconditions.ok, "current-C runtime-control oracle preconditions failed")
    for error in preconditions.errors:
        report.check(False, f"oracle precondition: {error}")

    raw_cases = cases(obj)
    report.check(len(raw_cases) == 5, "M8.13d runtime-control case count drift")
    report.check(set(outputs) == {str(case.get("id")) for case in raw_cases}, "candidate case id drift")
...
def run_negative_tests(obj: Any, outputs: dict[str, CandidateOutput]) -> Report:
    ...
    expect_failure(
        "steering support-artifact hash drift",
        lambda root: root["support_artifacts"]["directional_steering"].__setitem__("sha256", "0" * 64),
    )
    expect_failure(
        "warm stderr anchor drift",
        lambda root: case_by_id(root, "warm_weights").__setitem__("stderr_anchors", ["missing stderr anchor"]),
    )
    expect_failure(
        "directional steering stderr anchor drift",
        lambda root: case_by_id(root, "directional_steering").__setitem__("stderr_anchors", ["missing stderr anchor"]),
    )
    expect_failure(
        "metal backend exit drift",
        lambda root: case_by_id(root, "backend_metal_error").__setitem__("exit_code", 0),
    )
    expect_failure(
        "mtp availability drift",
        lambda root: root["support_artifacts"]["mtp"].__setitem__("available", True),
    )
    expect_failure(
        "mtp support-artifact drift",
        lambda root: case_by_id(root, "mtp_missing_model").__setitem__("support_artifact", "/tmp/other.gguf"),
    )
    return report
...
def main() -> int:
    ...
    print_report("CLI runtime-controls runtime comparator", report)
    ok = report.ok
    if args.negative_test:
        negative = run_negative_tests(obj, outputs)
        print_report("CLI runtime-controls runtime negative tests", negative)
