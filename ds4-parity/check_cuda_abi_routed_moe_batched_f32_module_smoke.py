#!/usr/bin/env python3
"""Validate the retained batched-F32 routed-MoE ABI prerequisite module."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
NEXT_STAGE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb Remaining Runtime Correctness Performance And C CUDA Removal Policy"
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-routed-moe-batched-f32-module-smoke.json"
...
def main(argv: Iterable[str]) -> int:
    ...
    texts = {
        ...
        "prior_smoke": (ROOT / "rust/ds4-cuda/src/bin/routed_moe_f32_smoke.rs").read_text(encoding="utf-8"),
    ...
    print(f"{MILESTONE} Rust CUDA batched F32 routed-MoE ABI prerequisite: {status} ({report.checks} checks)")
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_routed_moe_batched_f32_module_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(fixture.get("status") == "b300-pass-staticlib-batched-f32-prerequisite-module", "status drift")
    ...
    for marker in [
        ...
        report.check(marker in texts["cuda"], f"current-C F32 marker missing: {marker}")
    for marker in [
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for entry in [
        ...
        report.check(entry in implementation.get("widened_kernel_entries", []), f"evidence entry missing: {entry}")
        report.check(f"pub fn {entry}" in texts["kernels"], f"embedded kernel missing: {entry}")
    for marker in [
        "pair >= n_tokens * n_expert",
        "let token = pair / n_expert;",
        "x_base + block as usize * ABI_MOE_QK_K",
        "index >= n_tokens * out_dim",
        "moe_gate_up_mid_f32_tensor(",
        "moe_down_f32_tensor(",
        "moe_sum_tensor(",
    ]:
        report.check(marker in texts["kernels"], f"batched F32 marker missing: {marker}")
    report.check("now accept n_tokens" in implementation.get("batch_boundary", ""), "batch boundary missing")
    ...
    for call in ["moe_gate_up_mid_f32_tensor", "moe_down_f32_tensor"]:
        report.check(
            re.search(rf"kernels\.{call}\(\s*backend\.stream\(\),\s*1,", texts["abi"], re.S) is not None,
            f"single-token caller does not pass one token to {call}",
        )
    report.check(
        len(re.findall(r"kernels\.moe_sum_tensor\(\s*backend\.stream\(\),\s*1,", texts["abi"], re.S)) >= 2,
        "single-token caller does not pass one token to both sum paths",
    )
    ...
    for key, expected in [
        ...
        ("target", "sm_80"),
        ("local_library_test_count", 162),
        ("feature_release_test_count", 169),
    ...
    for key in [
        ...
        "widened_f32_gate_entry_generated",
        "widened_f32_down_entry_generated",
        "widened_sum_entry_generated",
        "single_token_callsite_passes_one_token",
    ]:
    ...
    for key, expected in [
        ("local_ds4_cuda_library_test_count", 162),
        ("b300_feature_release_test_count", 169),
        ("cuda_abi_comparators_passed", 76),
        ("unified_report_passed", 249),
...
def validate_wiring(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-routed-moe-batched-f32-module-smoke.json"
    checker = "check_cuda_abi_routed_moe_batched_f32_module_smoke.py"
    item = f"{MILESTONE}: Batched F32 Routed MoE ABI Prerequisite"
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("widened entries missing", lambda value: value["implementation"]["widened_kernel_entries"].clear()),
        ("F32 ownership missing", lambda value: value["ownership"].update({"owns_batched_f32_launch_prerequisite": False})),
        ("batch overclaim", lambda value: value["ownership"].update({"owns_routed_moe_batch_tensor": True})),
        ("kernel count mismatch", lambda value: value["ownership"].update({"embedded_kernel_count": 90})),
