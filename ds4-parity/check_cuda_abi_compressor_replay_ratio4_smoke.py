#!/usr/bin/env python3
"""Validate the Rust CUDA public compressor ratio-4 replay ABI smoke."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-compressor-replay-ratio4-smoke.json"
...
HARNESS = ROOT / f"ds4-parity/fixtures/backend/{MILESTONE_DIR}/abi_compressor_replay_ratio4_link_smoke.c"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA public compressor ratio-4 replay ABI smoke: {state} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_compressor_replay_ratio4_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(
        fixture.get("status") == "b300-pass-staticlib-public-compressor-replay-ratio4-abi",
        "status drift",
    )
    oracle = require_dict(report, fixture.get("oracle"), "oracle")
    report.check(oracle.get("source") == "ds4_cuda.cu", "oracle source drift")
    report.check(
        oracle.get("symbols") == ["ds4_gpu_compressor_prefill_ratio4_replay_tensor"],
        "oracle symbols drift",
    )
    for marker in [
        'extern "C" int ds4_gpu_compressor_prefill_ratio4_replay_tensor(',
        "n_tokens == 0 || (n_tokens & 3u) != 0 || (pos0 & 3u) != 0",
        ...
        report.check(marker in texts["cuda_c"], f"current-C replay marker missing: {marker}")
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("ds4_gpu_compressor_prefill_ratio4_replay_tensor" in symbols, "replay export missing")
    ...
    for marker in [
        ...
        target = texts["kernels"] if marker.startswith("pub fn abi_") or "wrapping_add" in marker or "pos_stride =" in marker else texts["abi"]
        report.check(marker in target, f"Rust replay marker missing: {marker}")
    for marker in [
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(
        implementation.get("embedded_kernel_entries") == ["abi_compressor_prefill_ratio4_replay_pool_kernel"],
        "kernel ownership drift",
    )
    report.check("abi_compressor_set_rows_kernel" in implementation.get("reused_embedded_kernels", []), "set-rows reuse missing")
    report.check("abi_rope_tail_kernel" in implementation.get("reused_embedded_kernels", []), "RoPE reuse missing")
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 144),
        ("feature_release_test_count", 151),
        ("staticlib_export_count", 60),
        ("embedded_kernel_count", 36),
    ]:
        report.check(execution.get(key) == expected, f"execution drift: {key}")
    observed = require_dict(report, execution.get("observed"), "observed")
    for key in [
        "c_linked_rust_staticlib",
        "ratio4_replay_f32_output_matches",
        "ratio4_replay_f16_output_matches",
        "stride4_rope_composition_matches",
        "optional_fp8_composition_matches",
        "state_rebuild_after_output_matches",
        "n_rot_zero_branch_matches",
        "invalid_model_range_preserves_output_and_state",
        "invalid_shape_rejected",
        "checked_overflow_rejected",
        "null_rejected",
        "embedded_compressor_prefill_ratio4_replay_pool_kernel_loaded",
    ]:
        report.check(observed.get(key) is True, f"observed smoke drift: {key}")
    report.check(observed.get("predecessor_c_linked_regression_consumers_passed") == 54, "predecessor count drift")
    report.check(observed.get("predecessor_relink_executable_stack_warning_count") == 54, "warning count drift")
    for marker in [
        "#define RATIO4 4u",
        "#define N_TOKENS 8u",
        "reference_replay(",
    ...
    report.check(any("rejects width and launch overflow" in value for value in risks), "overflow boundary risk missing")
    report.check(any("observationally indistinguishable" in value for value in risks), "ratio-4 wrap caveat missing")
    report.check(any("prior state before the final state rebuild" in value for value in risks), "ordering risk missing")
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-compressor-replay-ratio4-smoke.json"
    checker = "check_cuda_abi_compressor_replay_ratio4_smoke.py"
    item = f"{MILESTONE}: Public Compressor Ratio-4 Replay ABI"
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("stride-4 RoPE output failure", lambda value: value["b300_execution"]["observed"].update({"stride4_rope_composition_matches": False})),
        ("state rebuild ordering removed", lambda value: value["b300_execution"]["observed"].update({"state_rebuild_after_output_matches": False})),
        ("general prefill overclaim", lambda value: value["ownership"].update({"owns_compressor_update_or_general_prefill": True})),
