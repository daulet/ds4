#!/usr/bin/env python3
"""Validate the M14.2d2b2a Rust CUDA WMMA32 indexer score kernel smoke."""
...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.2d2b2a/indexer-wmma32-kernel-smoke.json"
CARGO = ROOT / "rust/ds4-cuda/Cargo.toml"
LOCK = ROOT / "Cargo.lock"
CRATE_LIB = ROOT / "rust/ds4-cuda/src/lib.rs"
SMOKE = ROOT / "rust/ds4-cuda/src/bin/indexer_wmma32_smoke.rs"
...
EXPECTED_RUST_OWNED = [
    "executable-local cuda-oxide indexer_scores_wmma32_kernel launch proof",
    "current-C-shaped 32-component two-warp tensor-core tile and causal masking semantics",
]
EXPECTED_NOT_CLAIMED = [
    "WMMA64 and WMMA128 score dispatch",
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.indexer_wmma32_kernel_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.2d2b2a", "milestone drift")
    ...
    report.check('name = "ds4-cuda-indexer-wmma32-smoke"' in texts["cargo"], "smoke binary wiring missing")
...
def validate_oracle(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        ...
        "fmaxf(c_sh[i], 0.0f) * w",
        "indexer_scores_wmma32_kernel<<<grid, 64>>>",
    ]:
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 29, "feature test count drift")
    ...
    report.check("--bin ds4-cuda-indexer-wmma32-smoke" in command, "smoke command missing")
    ...
    expected = {
        "milestone": "M14.2d2b2a",
        "device_name": "NVIDIA B300 SXM6 AC",
        "rust_kernel_toolchain": True,
        "wmma32_output_matches": True,
        "causal_mask_output_matches": True,
        "two_warp_tile_mapping_matches": True,
    ...
    report.check(stdout == expected, "B300 WMMA32 result drift")
    for marker in [
        "#![feature(f16)]",
        "pub fn indexer_scores_wmma32_kernel",
        "const WMMA_WARPS: usize = 2;",
        "let warp_id = tid >> 5;",
        "load_a_m16n8k16",
        "load_b_m16n8k16",
        ...
        "two_warp_tile_mapping_matches",
        "Err(IndexerWmma32Error::InvalidShape)",
    ]:
        report.check(marker in texts["smoke"], f"smoke marker missing: {marker}")


def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.2d2b2a/indexer-wmma32-kernel-smoke.json"
    checker = "check_indexer_wmma32_kernel_smoke.py"
    report.check("M14.2d2b2a: WMMA32 Tensor-Core Indexer Score Kernel" in texts["roadmap"], "roadmap item missing")
    report.check(fixture in texts["roadmap"], "roadmap fixture missing")
    report.check("M14.2d2b2a: WMMA32 Tensor-Core Indexer Score Kernel" in texts["todo"], "TODO item missing")
    report.check(fixture in texts["todo"], "TODO fixture missing")
    report.check("Active item: M14.2d2b2b WMMA64 Tensor-Core Indexer Score Kernel" in texts["status"], "next active stage missing")
    report.check("M14.2d2b2a WMMA32 Tensor-Core Indexer Score Kernel" in texts["status"], "status evidence missing")
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("WMMA32 result absent", lambda value: value["b300_execution"]["stdout"].update({"wmma32_output_matches": False})),
        ("causal result absent", lambda value: value["b300_execution"]["stdout"].update({"causal_mask_output_matches": False})),
        ("two-warp result absent", lambda value: value["b300_execution"]["stdout"].update({"two_warp_tile_mapping_matches": False})),
        ...
        ("larger WMMA overclaim", lambda value: value["ownership"].update({"owns_wmma64_and_wmma128_dispatch": True})),
...
def print_report(report: Report) -> None:
    ...
    print(f"M14.2d2b2a WMMA32 indexer kernel smoke: {status} ({report.checks} checks)")
