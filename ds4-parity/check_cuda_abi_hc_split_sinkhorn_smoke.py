#!/usr/bin/env python3
"""Validate the Rust CUDA public hyperconnection split-Sinkhorn ABI smoke."""
...
MILESTONE = "M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbba"
MILESTONE_DIR = MILESTONE.lower()
FIXTURE = ROOT / f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-hc-split-sinkhorn-smoke.json"
...
HARNESS = ROOT / f"ds4-parity/fixtures/backend/{MILESTONE_DIR}/abi_hc_split_sinkhorn_link_smoke.c"
...
def main(argv: Iterable[str]) -> int:
    ...
    print(f"{MILESTONE} Rust CUDA public HC split-Sinkhorn ABI smoke: {state} ({report.checks} checks)")
...
def validate(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.cuda_abi_hc_split_sinkhorn_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == MILESTONE, "milestone drift")
    report.check(
        fixture.get("status") == "b300-pass-staticlib-public-hc-split-sinkhorn-abi",
        "status drift",
    )
...
def validate_oracle(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(oracle.get("symbol") == "ds4_gpu_hc_split_sinkhorn_tensor", "oracle symbol drift")
    for marker in [
        ...
        report.check(marker in texts["cuda_c"], f"current-C split-Sinkhorn marker missing: {marker}")
...
def validate_ownership(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("ds4_gpu_hc_split_sinkhorn_tensor" in symbols, "split-Sinkhorn public export missing")
    ...
    for marker in [
        ...
        report.check(marker in texts["abi"], f"Rust split-Sinkhorn ABI marker missing: {marker}")
    for marker in [
        ...
        report.check(marker in texts["kernels"], f"embedded split-Sinkhorn marker missing: {marker}")
    for marker in [
        "pub struct CudaAbiHcSplitSinkhornScope",
        "pub const M14_6B2B2B2B2B2B2B2B2B2B2B2B2B2BBBBBBBBBBBBBBBBBBBA_SCOPE",
        "exported_abi_symbol_count: 45",
        "exported_compute_symbol_count: 22",
        "owns_hc_split_sinkhorn_tensor: true",
        "owns_fused_hc_reductions: false",
...
def validate_execution(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(implementation.get("kernel_entry") == "abi_hc_split_sinkhorn_kernel", "kernel entry drift")
    ...
    for key, expected in [
        ...
        ("local_library_test_count", 131),
        ("feature_release_test_count", 138),
        ("staticlib_export_count", 45),
    ]:
        report.check(execution.get(key) == expected, f"execution drift: {key}")
    observed = require_dict(report, execution.get("observed"), "observed")
    for key in [
        "c_linked_rust_staticlib",
        "sinkhorn_split_output_matches",
        "shorter_output_row_floor_matches",
        "alternate_parameter_range_matches",
        "short_mix_rejected",
        "invalid_model_range_rejected",
        "invalid_hc_count_rejected",
        "embedded_hc_split_sinkhorn_kernel_loaded",
    ]:
        report.check(observed.get(key) is True, f"observed smoke drift: {key}")
    report.check(observed.get("predecessor_c_linked_regression_consumers_passed") == 41, "predecessor count drift")
    report.check(observed.get("predecessor_relink_executable_stack_warning_count") == 41, "warning count drift")
    for marker in [
    ...
    report.check(any("fused split weighted" in value for value in risks), "fused reduction boundary missing")
...
def validate_wiring(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    fixture_path = f"ds4-parity/baselines/backend/{MILESTONE_DIR}/abi-hc-split-sinkhorn-smoke.json"
    checker = "check_cuda_abi_hc_split_sinkhorn_smoke.py"
    item = f"{MILESTONE}: Public Hyperconnection Split Sinkhorn ABI"
...
def run_negative_tests(report: ReportState, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("numeric result removed", lambda value: value["b300_execution"]["observed"].update({"sinkhorn_split_output_matches": False})),
        ("range result removed", lambda value: value["b300_execution"]["observed"].update({"alternate_parameter_range_matches": False})),
        ("range rejection removed", lambda value: value["b300_execution"]["observed"].update({"invalid_model_range_rejected": False})),
        ("fused reduction overclaim", lambda value: value["ownership"].update({"owns_fused_hc_reductions": True})),
