...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.3a/rms-norm-kernel-smoke.json"
CARGO = ROOT / "rust/ds4-cuda/Cargo.toml"
LOCK = ROOT / "Cargo.lock"
CRATE_LIB = ROOT / "rust/ds4-cuda/src/lib.rs"
SMOKE = ROOT / "rust/ds4-cuda/src/bin/rms_norm_smoke.rs"
...
EXPECTED_RUST_OWNED = [
    ...
    "plain and weighted row-wise RMS normalization tensor validation",
]
EXPECTED_NOT_CLAIMED = [
    "fused QKV or head RMS normalization kernels",
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.rms_norm_kernel_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.3a", "milestone drift")
    ...
    report.check('name = "ds4-cuda-rms-norm-smoke"' in texts["cargo"], "smoke binary wiring missing")
...
def validate_oracle(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        "__global__ static void rms_norm_plain_kernel",
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check(execution.get("test_count") == 39, "feature test count drift")
    ...
    report.check("--bin ds4-cuda-rms-norm-smoke" in command, "smoke command missing")
    ...
    expected = {
        ...
        "plain_rows_output_matches": True,
        "weighted_rows_output_matches": True,
        "single_row_output_matches": True,
        "shape_rejected": True,
    ...
    report.check(stdout == expected, "B300 RMS normalization result drift")
    for marker in [
        "pub fn rms_norm_plain_kernel",
        "pub fn rms_norm_weight_kernel",
        "static mut PARTIAL: SharedArray<f32, 256>",
        "uses_libdevice_link_path",
        "rms_norm_plain_rows_tensor(&module",
...
def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.3a/rms-norm-kernel-smoke.json"
    checker = "check_rms_norm_kernel_smoke.py"
    item = "M14.3a: Plain And Weighted RMS Norm Kernels"
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("plain result absent", lambda value: value["b300_execution"]["stdout"].update({"plain_rows_output_matches": False})),
        ("weighted result absent", lambda value: value["b300_execution"]["stdout"].update({"weighted_rows_output_matches": False})),
        ("single-row result absent", lambda value: value["b300_execution"]["stdout"].update({"single_row_output_matches": False})),
        ("fused norm overclaim", lambda value: value["ownership"].update({"owns_fused_qkv_and_head_norm_kernels": True})),
...
def print_report(report: Report) -> None:
    ...
    print(f"M14.3a RMS normalization kernel smoke: {status} ({report.checks} checks)")
