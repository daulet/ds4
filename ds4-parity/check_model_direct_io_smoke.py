#!/usr/bin/env python3
"""Validate the M14.1b2b3b1 Rust-owned direct-I/O pinned range smoke."""
...
FIXTURE = ROOT / "ds4-parity/baselines/backend/m14.1b2b3b1/model-direct-io-smoke.json"
CRATE_LIB = ROOT / "rust/ds4-cuda/src/lib.rs"
MODEL_MAP = ROOT / "rust/ds4-cuda/src/model_map.rs"
SUBSTRATE = ROOT / "rust/ds4-cuda/src/substrate.rs"
SMOKE = ROOT / "rust/ds4-cuda/src/bin/model_direct_io_smoke.rs"
...
EXPECTED_RUST_OWNED = [
    "Linux O_DIRECT model-file open and aligned pinned read selection",
    "buffered pinned read fallback when an aligned direct read cannot cover the requested bytes",
    "synchronized selected-range pinned host-to-device upload",
    "exact CUDA readback for both direct and buffered-fallback selections",
]
EXPECTED_NOT_CLAIMED = [
    "four-buffer asynchronous staging ring or event overlap",
    "range-cache arena allocation or budget policy",
    "persistent O_DIRECT disable-after-error state",
...
def validate(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    report.check(fixture.get("schema") == "ds4.model_direct_io_smoke.v1", "schema drift")
    report.check(fixture.get("milestone") == "M14.1b2b3b1", "milestone drift")
...
def validate_model_range(report: Report, fixture: dict[str, Any]) -> None:
    ...
    report.check(model.get("tail_range_offset") == MODEL_SIZE - 13, "tail offset drift")
    report.check(model.get("tail_range_bytes") == 13, "tail bytes drift")
...
def validate_ownership(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    for marker in [
        ...
        "stage_pinned_model_range",
        "try_direct_pinned_source",
        "libc::O_DIRECT",
        "BufferedFallback",
    ]:
        report.check(marker in texts["model_map"], f"model-map marker missing: {marker}")
    for marker in ["PinnedHostBuffer", "pinned_zeroed", "upload_pinned_u8_range", "self.synchronize()"]:
...
def validate_execution(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    report.check("--bin ds4-cuda-model-direct-io-smoke" in execution.get("command", ""), "smoke command missing")
    stdout = require_dict(report, execution.get("stdout"), "b300_execution.stdout")
    expected = {
        "milestone": "M14.1b2b3b1",
        ...
        "tail_range_offset": MODEL_SIZE - 13,
        "tail_range_bytes": 13,
        "tail_buffered_fallback": True,
        "tail_fallback_readback_matches": True,
        "pinned_upload_synchronized_for_smoke": True,
        "owns_pinned_file_staging": True,
        "owns_o_direct_open_and_aligned_read": True,
        "owns_buffered_read_fallback": True,
        "owns_asynchronous_staging_ring": False,
    ...
    report.check(stdout == expected, "B300 direct-I/O result drift")
    for marker in [
        "stage_pinned_model_range",
        "PinnedStageResolution::DirectIo",
        "PinnedStageResolution::BufferedFallback",
        "tail_fallback_readback_matches",
    ]:
        report.check(marker in texts["smoke"], f"smoke marker missing: {marker}")


def validate_wiring(report: Report, texts: dict[str, str]) -> None:
    fixture = "ds4-parity/baselines/backend/m14.1b2b3b1/model-direct-io-smoke.json"
    checker = "check_model_direct_io_smoke.py"
    report.check("M14.1b2b3b1: Direct-I/O Pinned Read Selection" in texts["roadmap"], "roadmap item missing")
...
def run_negative_tests(report: Report, fixture: dict[str, Any], texts: dict[str, str]) -> None:
    for label, mutate in [
        ("direct selection failure", lambda value: value["b300_execution"]["stdout"].update({"direct_io_selected": False})),
        ("direct readback mismatch", lambda value: value["b300_execution"]["stdout"].update({"direct_io_readback_matches": False})),
        ("tail fallback missing", lambda value: value["b300_execution"]["stdout"].update({"tail_buffered_fallback": False})),
        ("tail readback mismatch", lambda value: value["b300_execution"]["stdout"].update({"tail_fallback_readback_matches": False})),
        ("ring overclaim", lambda value: value["ownership"].update({"owns_asynchronous_staging_ring": True})),
    ]:
...
def print_report(report: Report) -> None:
    ...
    print(f"M14.1b2b3b1 model direct-I/O smoke: {status} ({report.checks} checks)")
