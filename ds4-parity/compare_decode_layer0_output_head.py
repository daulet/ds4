#!/usr/bin/env python3
"""Compare Rust layer-0 output-head readback against the current-C GPU oracle."""
...
FILES = {
    "c_helper": ROOT / "ds4_layer0_output_head_oracle_dump.c",
    "ds4_c": ROOT / "ds4.c",
    "ds4_h": ROOT / "ds4.h",
    "makefile": ROOT / "Makefile",
    "rust_bin": ROOT / "rust/ds4-gpu/src/bin/ds4-decode-layer0-output-head.rs",
...
ORACLE_SCHEMA = "ds4.layer0_output_head_oracle.v1"
CANDIDATE_SCHEMA = "ds4.decode_layer0_output_head.v1"
CASE = "token0_layer0_output_head"
...
EXPECTED_FNV1A64 = {
    ...
    "output_pre": "67cd67e9413ba488",
    "output_weights": "b7b3f62be8581476",
    "output_embd": "0b0d4f86243397e3",
    "output_norm": "24029c3b5c92306e",
    "logits": "27d2e668424d8d9f",
}
EXPECTED_WEIGHTS = {
...
def main(argv: Iterable[str]) -> int:
    ...
    if report.ok:
        print(f"Rust layer-0 output-head current-C oracle comparator: {report.checks} checks")
    else:
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--oracle", type=Path, help="JSON from ds4-layer0-output-head-oracle-dump")
    parser.add_argument("--candidate", type=Path, help="JSON from ds4-decode-layer0-output-head")
...
def validate_static(report: Report, texts: dict[str, str]) -> None:
    ...
    for needle, message in (
        ("ds4_dump_layer0_output_head_oracle_json", "C helper does not call exported oracle dump"),
    ...
    ds4_c = texts["ds4_c"]
    for needle, message in (
        ("ds4_dump_layer0_output_head_oracle_json", "C oracle dump implementation missing"),
    ...
    report.check(
        "int ds4_dump_layer0_output_head_oracle_json" in texts["ds4_h"],
        "header declaration for layer-0 output-head oracle missing",
    )

    makefile = texts["makefile"]
    report.check("ds4-layer0-output-head-oracle-dump:" in makefile, "Makefile helper target missing")
    report.check("ds4_layer0_output_head_oracle_dump_cpu.o" in makefile, "CPU helper object missing")
    ...
    report.check(
        "M10.5c4c2b2b2b2b2b1 Rust layer-0 output-head comparator" in report_text,
        "unified report comparator missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b1 B300 layer-0 output-head oracle rerun" in report_text,
        "unified report B300 skip missing",
    )
    report.check(
        "ds4-layer0-output-head-oracle-dump" in report_text
        and "compare_decode_layer0_output_head.py" in report_text,
        "B300 oracle rerun command missing helper/comparator",
    )

    report.check("M10.5c4c2b2b2b2b2b1 Rust layer-0 output-head" in texts["readme"], "README entry missing")
    report.check(
        "M10.5c4c2b2b2b2b2b1: Rust Layer-0 Output Head B300 Execution" in texts["roadmap"],
        "roadmap output-head split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2: Rust One-Token Decode B300 Execution" in texts["roadmap"],
        "roadmap remainder split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b1: Rust Layer-0 Output Head B300 Execution" in texts["todo"],
        "TODO output-head split missing",
    )
    report.check(
        "M10.5c4c2b2b2b2b2b2: Rust One-Token Decode B300 Execution" in texts["todo"],
...
def run_negative_tests(texts: dict[str, str]) -> int:
    static_mutations = [
        ("remove schema", "rust_bin", CANDIDATE_SCHEMA, "ds4.decode_layer0_output_head.removed"),
        ...
        ("remove B300 candidate check", "report", "compare_decode_layer0_output_head.py", "compare_decode_layer0_output_head_removed.py"),
        (
            "remove roadmap split",
            "roadmap",
            "M10.5c4c2b2b2b2b2b1: Rust Layer-0 Output Head B300 Execution",
            "M10.5c4c2b2b2b2b2b1 removed",
        ),
    ]
    failures: list[str] = []
...
def print_errors(errors: list[str]) -> None:
    print("Rust layer-0 output-head comparator failures:")
    for error in errors:
        print(f"  - {error}")
