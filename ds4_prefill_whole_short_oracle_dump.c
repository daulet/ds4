...
static void usage(const char *prog) {
    fprintf(stderr, "usage: %s --model FILE --prompt FILE [--limit-tokens N] [--resume-prefix-tokens N] [--backend cuda|metal] [--output FILE]\n", prog);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "-m") == 0 || strcmp(argv[i], "--model") == 0) && i + 1 < argc) {
            ...
            limit_tokens = (int)v;
        } else if (strcmp(argv[i], "--resume-prefix-tokens") == 0 && i + 1 < argc) {
            char *end = NULL;
            long v = strtol(argv[++i], &end, 10);
            if (end == argv[i] || *end || v <= 0 || v > 1000000L) {
                usage(argv[0]);
    ...
    int rc = ds4_dump_prefill_whole_short_oracle_json(model, prompt, limit_tokens, resume_prefix_tokens, backend, fp);
