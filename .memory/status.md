...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Rust CUDA Down Metadata Global-Load Address-Space Performance Repair
  ...
  Q2 metadata fields `weight_scale` and `weight_min` at offsets `80` and
  `82`; packed Q2 words, Q8 staging, reductions, and `256` DP4A sites remain
  unchanged. Down PTX gains exactly `2` `ld.global.u16` sites and keeps
  shared-load/store counts at `152`/`2`. The repaired DSO SHA-256 is
  ...
  Two order-reversed B300 comparisons lower down from `423.669 ms` to
  `420.358 ms` and from `423.519 ms` to `420.123 ms`, lowering total
  routed-MoE from `961.924 ms` to `958.728 ms` and from `961.746 ms` to
  `958.190 ms`. The packed-Q2 `ld.global.u32` probe was rejected after down
  regressed to `574.252 ms`. Official vectors pass `1958` checks plus `8`
  negative checks and B300 feature tests pass `176` tests. The unified report
  passes `285` comparators with `50` skipped and `0` failed; both requested
  Claude review attempts timed out after `60` seconds. Rust remains `1.037x`
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Rust CUDA Cached Q8 Aligned-Pair Staging Performance Repair
  ...
  remaining `72` words as `36` assembled `u64` shared stores, preserving the
  external `292`-byte Q8_K layout, existing paired consumers, DP4A topology,
  fixed-order reductions, row-span policy, and default current-C routing.
  Gate/up PTX adds one `st.shared.u64` site (`1` to `2`) and down adds one
  (`0` to `1`), while paired shared loads remain `72`/`128`, DP4A counts
  remain `128`/`256`, and neither kernel gains local loads or stores. The
  repaired DSO SHA-256 is
  `5c06a124db2c5a0854ae8aebca63a870ebd07032ea9bb81f7c16e0379b497de6`
  and PTX SHA-256 is
  `92e32bdacd74a7aac3cbf443bef79d9c54cbb193c68f063b5ef6be0a430572e9`.
  An adjacent B300 comparison lowers gate/up from `583.532 ms` to
  `570.157 ms`, down from `430.267 ms` to `423.871 ms`, and total
  routed-MoE from `1030.111 ms` to `1010.385 ms`, a `1.91%` total phase
  reduction. Official vectors pass `1958` checks plus `8` negative checks
  and B300 feature tests pass `176` tests. The unified report passes `283`
  comparators with `50` skipped and `0` failed. Both requested Claude review
  attempts timed out after `60` seconds. Total routed-MoE remains `1.09x`
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Rust CUDA Fixed-Order Quarter-Warp Reduction Performance Repair
  ...
  `4, 2, 1` stride loop to the same three explicit shuffle additions,
  preserving arithmetic order, the PRMT transform, DP4A topology, row-span
  policy, and default current-C routing. Cuda-oxide emits the new helper as
  a branch-free out-of-line device function with `3` `shfl.sync` sites and
  `0` branch sites; gate/up consequently carries `16` reduction calls again
  and down carries `8`, with no local loads or stores. The repaired DSO
  SHA-256 is
  ...
  `709.869 ms`, down from `512.130 ms` to `506.772 ms`, and total
  routed-MoE from `1243.192 ms` to `1232.947 ms`; the one-shot prefill
  reading moves from `248.23` to `244.71 tok/s`, so retention is based on
  the instrumented MoE phase totals. Official vectors pass `1958` checks
  plus `8` negative checks and B300 feature tests pass `176` tests. The
  unified report passes with `279` passed, `50` skipped, and `0` failed.
  Both Claude review attempts returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Total routed-MoE remains `1.33x`
  current-C, so default routing and promotion blockers remain in force.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Indexer Score Dispatch ABI
  ...
  `ds4_gpu_indexer_scores_decode_batch_tensor` through shared current-C
  dispatch selection, and embeds the scalar, direct-one, and
  WMMA128/64/32/base score kernels. The rebuilt static library exposes 80
  Rust ABI symbols and embeds 99 kernels. A C-linked consumer passes all six
  score-route checks, quality-mode scalar fallback, decode-batch output, and
  rejection controls; the public top-k mask consumer remains passing. Local
  tests pass with 168 tests; B300 feature tests pass with 175 tests; all 82
  CUDA ABI comparators pass; and the unified report passes with 255 passed,
  45 skipped, and 0 failed. The pre-implementation and final pass-end
  non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Indexer top-k selection, graph-wide
  route promotion, whole-archive policy, and C CUDA removal remain deferred.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Batched Routed MoE ABI Dispatch
  ...
  schedules batched F32 fallback plus quantized sorted, tiled, atomic, and
  row-span routes over persistent device scratch with active-stream metadata
  clearing. The rebuilt static library exposes 75 Rust ABI symbols and
  embeds 91 kernels. A C-linked public batch consumer passes F32 fallback,
  sorted P2/no-P2, tiled row32, tile4 atomic, and default 128-token tile16
  row-span atomic invocations; the existing single-token consumer also
  remains passing. Local tests pass with 165 tests; B300 feature tests pass
  with 172 tests; all 79 CUDA ABI comparators pass; and the unified report
  passes with 252 passed, 45 skipped, and 0 failed. The pre-implementation
  and final pass-end non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Graph-wide route promotion, remaining
  compute ownership, whole-archive policy, and C CUDA removal remain
  deferred.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Batched F32 Routed MoE ABI Prerequisite
  ...
  proved flattened batched layout. The existing public single-token
  routed-MoE wrapper passes `n_tokens = 1`; it still does not export
  `ds4_gpu_routed_moe_batch_tensor`. The rebuilt static library still
  exposes 74 Rust ABI symbols and embeds 91 kernels, and links/runs the
  existing C-linked public single-token consumer. Local tests pass with 162
  tests; B300 feature tests pass with 169 tests; all 76 CUDA ABI comparators
  pass; and the unified report passes with 249 passed, 45 skipped, and 0
  failed. The pre-implementation and final pass-end non-interactive Claude
  review attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  Host-side batched orchestration, public batch ownership, route promotion,
  and C CUDA removal remain deferred.
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Embedded Shared Cache Routed MoE ABI Module
  is validated on B300. The retained Rust CUDA ABI module now embeds cached
  tile8 gate and tile16 atomic down row-span entries with synchronized packed
  Q8 staging and synchronized IQ2 lookup/sign staging for gate, bounded to
  `xq_blocks <= 16` and `midq_blocks <= 8`. The rebuilt static library still
  exposes 74 Rust ABI symbols, now embeds 91 kernels, and links/runs the
  existing C-linked public single-token consumer while loading the enlarged
  module. Local tests pass with 161 tests; B300 feature tests pass with 168
  tests; all 75 CUDA ABI comparators pass; and the unified report passes with
  248 passed, 45 skipped, and 0 failed. The pre-implementation and final
  pass-end non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Public batched routed-MoE ownership,
  route promotion, and C CUDA removal remain deferred.
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Embedded Down Rowspan Routed MoE ABI Module
  is validated on B300. The retained Rust CUDA ABI module now embeds one
  tile16 atomic down row-span functional projection entry for the current
  row512, row1024, and row2048 route surface while preserving the tile-start
  alignment guard and token-indexed atomic accumulation branch. The rebuilt
  static library still exposes 74 Rust ABI symbols, now embeds 89 kernels,
  and links/runs the existing C-linked public single-token consumer while
  loading the enlarged module. Local tests pass with 160 tests; B300 feature
  tests pass with 167 tests; all 74 CUDA ABI comparators pass; and the
  unified report passes with 247 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Shared-cache
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Embedded Tile16 Row32 Atomic Routed MoE ABI Module
  ...
  guard, reusing retained block-16 descriptor construction and output
  zeroing. The rebuilt static library still exposes 74 Rust ABI symbols,
  now embeds 87 kernels, and links/runs the existing C-linked public
  single-token consumer while loading the enlarged module. Local tests pass
  with 158 tests; B300 feature tests pass with 165 tests; all 72 CUDA ABI
  comparators pass; and the unified report passes with 245 passed, 45
  skipped, and 0 failed. The pre-implementation and final pass-end
  non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Row-span, shared-cache specialization,
  and public batched routed-MoE ownership remain deferred.
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Embedded Row32 Tiled And Atomic Routed MoE ABI Module
  is validated on B300. The retained Rust CUDA ABI module now embeds tile4
  and tile8 row32 gate/down kernels plus their atomic-output zeroing
  prerequisite. The rebuilt static library still exposes 74 Rust ABI
  symbols, now embeds 86 kernels, and links/runs the existing C-linked
  public single-token consumer while loading the enlarged module. B300
  `sm_80` code generation lowers the retained float atomic path. Local tests
  pass with 157 tests; B300 feature tests pass with 164 tests; all 71 CUDA
  ABI comparators pass; and the unified report passes with 244 passed, 45
  skipped, and 0 failed. The pre-implementation and final pass-end
  non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Tile16, row-span, shared-cache
  specialization, and public batched routed-MoE ownership remain deferred.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Router Selection ABI
  ...
  parallel, and warp dispatch priority. A C-linked B300 witness proves
  biased selection, hash fallback, forced parallel/scalar dispatch, partial
  warp batches, stable ties, model-range and output-span rejection, invalid
  group/hash-row rejection, and null rejection. Local tests pass with 153
  tests; B300 feature tests pass with 160 tests; the static library exposes
  73 symbols and embeds 62 kernels. All 63 preceding linked ABI consumers
  pass against the rebuilt archive with the known executable-stack warning.
  All 67 CUDA ABI comparators pass, and the unified report passes with 239
  passed, 45 skipped, and 0 failed.
  The pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Router selection ownership is complete;
  routed MoE, remaining graph compute,
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Batched Q8 Attention Output ABI
  ...
  output-A projection, optional F16-rounded safe-SGEMM output-A dispatch, and
  an attention-labeled output-B Q8 projection. A C-linked B300 witness proves
  native two-stage output, partial Q8 block handling, cuBLAS output-A
  selection, environment and minimum-token fallback gates, output-B attention
  cache-label behavior, invalid model-range output preservation,
  invalid-shape rejection, and null rejection. Local tests pass with 151
  tests; B300 feature tests pass with 158 tests; the static library exposes
  68 symbols and embeds 50 kernels. All 61 preceding linked ABI consumers
  pass against the rebuilt archive with the known executable-stack warning.
  All 65 CUDA ABI comparators pass, and the unified report passes with 237
  passed, 45 skipped, and 0 failed. The pre-implementation and final pass-end
  non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Prefill attention, routed MoE, remaining
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public General Compressor Prefill ABI
  ...
  state-row placement, weighted RMS, stride-by-ratio RoPE, and optional FP8
  paths. A C-linked B300 witness proves ratio-4 FP16 output with previous and
  remainder state banks, optional FP8 composition, general-ratio FP32 output
  with remainder state, `n_comp == 0` state-only success, successful
  zero-RoPE compressed output, wrapped positions, and invalid-range,
  invalid-shape, checked-overflow, and null rejection. Local tests pass with
  146 tests; B300 feature tests pass with 153 tests; the static library
  exposes 62 symbols and embeds 39 kernels. All 56 preceding linked ABI
  consumers pass against the rebuilt archive with the known executable-stack
  warning. All 60 CUDA ABI comparators pass, and the unified report passes
  with 232 passed, 45 skipped, and 0 failed. The pre-implementation and final
  pass-end non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Attention, routed MoE,
  remaining graph compute, whole-archive/route promotion, C CUDA removal, and
  the executable-stack warning remain open.
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Compressor Update ABI
  is validated on B300. Rust exports `ds4_gpu_compressor_update_tensor`
  through embedded update-pool and ratio-4 shift kernels composed with the
  already-owned public store, weighted RMS, and stride-1 RoPE paths. A
  C-linked B300 witness proves non-emitting state-only mutation, ratio-4 FP16
  emission and post-output duplication, general-ratio FP32 emission without
  shifting, wrapped-position emission, the emitted zero-RoPE partial-failure
  boundary, and invalid-range, invalid-shape, checked-overflow, and null
  rejection. Local tests pass with 145 tests; B300 feature tests pass with
  152 tests; the static library exposes 61 symbols and embeds 38 kernels.
  All 55 preceding linked ABI consumers pass against the rebuilt archive with
  the known executable-stack warning. All 59 CUDA ABI comparators pass, and
  the unified report passes with 231 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. General prefill,
  attention, routed MoE, remaining graph compute, whole-archive/route
  promotion, C CUDA removal, and the executable-stack warning remain open.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Compressor Ratio-4 State ABI
  ...
  compressor set-rows kernel after driver-backed zero and negative-infinity
  state initialization on the same stream. A C-linked B300 witness proves
  ratio-4 lower-bank state placement, both APE formats, initialization of
  untouched rows, invalid model-range no-mutation ordering, invalid-shape,
  checked-overflow, and null rejection. Local tests pass with 143 tests;
  B300 feature tests pass with 150 tests; the static library exposes 59
  symbols and embeds 35 kernels; all 53 preceding linked ABI consumers pass
  against the rebuilt archive with the known executable-stack warning. All
  57 CUDA ABI comparators pass, and the unified report passes with 229
  passed, 45 skipped, and 0 failed. The pre-implementation and final pass-end
  non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Compressor update,
  full prefill, attention, routed MoE, remaining graph compute,
  whole-archive/route promotion, C CUDA removal, and the executable-stack
  warning remain open.
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbba: Public Compressor Store Batch ABI
  is validated on B300. Rust exports `ds4_gpu_compressor_store_batch_tensor`
  through one embedded compressor-store kernel consuming cached FP32 or FP16
  APE model ranges. A C-linked B300 witness proves ratio-4 state-row
  placement, both APE formats, `uint32_t` wraparound using a non-power-of-two
  ratio, untouched rows, and invalid-range, invalid-shape, checked-overflow,
  and null rejection. Local tests pass with 142 tests; B300 feature tests pass
  with 149 tests; the static library exposes 58 symbols and embeds 34 kernels;
  all 52 preceding linked ABI consumers pass against the rebuilt archive with
  the known executable-stack warning. All 56 CUDA ABI comparators pass, and
  the unified report passes with 228 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review attempts
  each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Compressor
  update/prefill, attention, routed MoE, remaining graph compute,
  whole-archive/route promotion, C CUDA removal, and the executable-stack
  warning remain open.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbba: Public Embedding Hyperconnection ABI
  ...
  while resolving the model embedding range through the cached ABI path. A
  C-linked B300 witness proves single-token hyperconnection replication,
  batched invalid-token row-zero fallback, alternate model ranges, and
  invalid-input rejection. Local tests pass with 135 tests; B300 feature
  tests pass with 142 tests; the static library exposes 50 symbols; all 45
  preceding linked ABI consumers pass against the rebuilt archive with the
  known executable-stack warning. All 49 CUDA ABI comparators pass, and the
  unified report passes with 221 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Rust rejects unsafe
  single-token bounds and zero-dimensional/overflowing launches that current
  C does not safely define. Remaining graph compute, whole-archive/route
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbba: Public Hyperconnection Split Weighted Sum ABI
  ...
  fused split-and-reduction kernel, resolving scale/base inputs through
  cached model ranges while retaining output-derived row semantics. A
  C-linked B300 witness proves fused output, emitted split values,
  output-defined row count, alternate parameter ranges, and invalid-input
  rejection. Local tests pass with 132 tests; B300 feature tests pass with
  139 tests; the static library exposes 46 symbols; all 42 preceding linked
  ABI consumers pass against the rebuilt archive with the known
  executable-stack warning. All 46 CUDA ABI comparators pass, and the unified
  report passes with 218 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. The normalized
  fused consumer, output HC weights, remaining graph compute,
  whole-archive/route promotion, C CUDA removal, and the warning remain open.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbba: Public Single-Token F32 Projection ABI
  ...
  122 tests, B300 feature tests pass with 129 tests, the static library
  exposes 32 symbols, and sixteen predecessor C-linked consumers pass against
...
- Last validated source before the active item: M14.6b2b2b2b2a Basic Model-Control Device-Copy ABI Export.
...
- Earlier M14.4d5 Static Heads8 Online And CUBLAS Attention Prefill Dispatch.
- Earlier M14.4d4 Generic Raw And Mixed Attention Prefill Kernels.
...
- Earlier M14.4b Raw KV Storage And Indexer QAT Kernels.
...
- Earlier M14.3c2 Ordered Paired And Serial F16 Projection Kernels.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbba Public Fd Final Sync Failure
  ...
  fd caching, injects one `cuStreamSynchronize` failure per staged fd attempt
  across a strict-mode transition, forwards subsequent synchronization so
  fallback can complete, rejects the first range-registration attempt, and
  proves both ranges retain host-backed cached output rather than divergent fd
  bytes. Local library tests pass with 119 tests; B300 release-feature tests
  pass with 126 tests, the static library retains 29 exports, and event-wait
  failure, event-record failure, fd-read failure, stage-allocation failure,
  ...
  comparator and default unified parity report pass with 205 passed, 45
  skipped, and 0 failed. The required non-interactive Claude review returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S` without completed findings. Q8/f16 hooks,
  remaining graph compute, route promotion, and C CUDA removal remain active.
...
- M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbba Public Fd Read Failure Continuation
  ...
  mode. A C-linked B300 consumer selects buffered fd caching, injects `EIO`
  from `pread` only for the configured model fd before two disjoint ranges
  across a strict-mode transition, rejects the first range-registration
  attempt, and proves both ranges retain host-backed cached output rather than
  divergent fd bytes. Two read failures prove retry without arena cache-full
  latching; one registration attempt preserves the existing
  registration-disable boundary. Local library tests pass with 116 tests;
  B300 release-feature tests pass with 123 tests, the static library retains
  29 exports, and stage-allocation failure, stage-pool reuse, fd-upload
  ...
  pass against it. The focused comparator and default unified parity report
  pass with 202 passed, 45 skipped, and 0 failed. The required non-interactive
  Claude review returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S` without completed
  findings. Event and final synchronization observations, route promotion,
...
- M14.6b2b2b2b2b2b2b2b2b2b2a Public Fd Source-Page And Progress ABI applies
  ...
  progress state with synchronized model replacement or cleanup. A C-linked
  B300 consumer interposes both POSIX advice calls and captures stderr across
  two ordinary multi-chunk uploads plus one suppressed upload; it observes
  advice invocation, progress reset, and keep-pages/verbose suppression
  without claiming physical eviction or TTY refresh rendering. Local library
  tests pass with 106 tests; B300 release-feature tests pass with 112 tests,
  the static library retains 29 exports, and the preceding public budget,
  fd-arena, buffered asynchronous staging, and direct-I/O asynchronous staging
  linked consumers pass against it. The public page/progress checker passes
  122 checks, and the default unified report passes with 192 passed, 45
  skipped, and 0 failed. The required non-interactive Claude review returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S` without completed findings. Residual
  model-control selection, whole-archive retention, route promotion, and
  remaining graph compute remain active.
- M14.6b2b2b2b2b2b2b2b2b2b1 Public Fd Cache Budget Fallback ABI retains
  admitted fd range byte accounting with the arena owner, parses the
  current-C `DS4_CUDA_WEIGHT_CACHE_LIMIT_GB` GiB/unlimited policy, rejects
  over-budget requests before source construction or staging, and returns an
  uncached direct model pointer for that operation. A C-linked B300 consumer
  admits and computes a small fd-backed range, then successfully returns from
  repeated rejected 1 GiB cache requests whose source pages are inaccessible
  and whose file bytes are absent. It intentionally does not execute a
  kernel through the returned host fallback pointer. Local library tests pass
  with 105 tests; B300 release-feature tests pass with 111 tests, and the
  static library retains 29 exports. The preceding public fd-arena, buffered
  asynchronous staging, and direct-I/O asynchronous staging C-linked
  consumers pass against the budget-aware static library. The public budget
  checker passes 110 checks, and the default unified report passes with 191
  ...
  findings. Source-page/progress policy, residual selection, whole-archive
  retention, route promotion, and remaining graph compute remain active.
...
- M14.6b2b2b2b2a Basic Model-Control Device-Copy ABI Export adds Rust-owned
  ...
  A changed model mapping synchronizes submitted work and clears retained
  device-copy ranges before replacement; a B300 C-linked static-library
  consumer pre-caches a weighted RMS range, switches mappings, mutates and
  rereads the old mapping to prove stale range release, and passes zero-byte
  cache and invalid-map rejection behavior. `nm` confirms 29 exported `ds4_gpu_*`
  symbols; local library tests pass with 94 tests and B300 release-feature
  tests pass with 96 tests. The basic model-control checker passes with 104
  checks, and unified parity passes with 180 passed, 45 skipped, and no
  failures. The required non-interactive Claude adversarial review timed out
  after 60 seconds without a completed result; self-review fixed a model-map
  replacement/cache cleanup lock-order inversion by taking the backend lock
  before clearing ranges. The fd symbol currently provides ABI acceptance,
  while fd-backed staging, registered/HMM/prefetch, environment-selected
  residency policy, q8/f16 cache hooks, whole-archive retention, the
  executable-stack warning, and remaining graph compute remain active before
  route promotion.
...
- M14.6b2b2b2a Plain RMS Norm ABI Export adds the Rust-owned
  ...
  `abi_rms_norm_plain_kernel` in the reusable embedded CUDA module. A
  C-linked static-library consumer on `NVIDIA B300 SXM6 AC` passes
  single-row, two-row, output/input alias, undersized-output, zero-row,
  current-C zero-width, and null-input checks; successful module load removes
  temporary libdevice link artifacts and `nm` confirms 23 exported
  `ds4_gpu_*` symbols. Local library tests pass with 92 tests and B300
  release-feature tests pass with 94 tests. Weighted RMS remains pending
  because its public C surface reads weights through the not-yet-exported
  model-map range boundary; whole-archive retention, the generated embedded
  object's executable-stack warning, and the shared-module non-release
  SwiGLU codegen blocker remain active before route promotion. The plain-RMS
  ABI checker passes with 94 checks, and unified parity passes with 178
  passed, 45 skipped, and no failures. The required non-interactive Claude
  adversarial review timed out after 60 seconds without a completed result;
  self-review preserved the current-C zero-width boundary and kept
  model-backed RMS pending.
...
- M14.5c2f Generic And Sorted Qwarp Quantized Routed MoE adds opt-in
  ...
  pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 86
  tests; live cargo-oxide execution found eleven kernels, emitted portable
  `sm_80` PTX through libdevice, and matched generic and sorted gate/down
  plus summed output on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.5c2f/routed-moe-qwarp-fallback-smoke.json`
  and `ds4-parity/check_routed_moe_qwarp_fallback_smoke.py --negative-test`.
  The local CUDA-feature build is blocked because
  `/usr/local/cuda/include/cuda.h` is absent on this Mac. Hyperconnection,
  runtime route activation, and C CUDA removal remain unclaimed. Local
  formatting, diff and library tests, the M14.5c2f comparator, retained
  selector checks, and unified parity passed with 170 passed, 45 skipped,
  and 0 failed.
...
- M14.5c2d Single-Token Q4_K Routed MoE adds an opt-in
  ...
  gate/up and direct sum-six down kernels. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 84
  tests; live cargo-oxide execution found five kernels, emitted portable
  `sm_80` PTX through libdevice, and matched gate/up and down output on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  ...
  expert-tile specialization, hyperconnection, runtime route activation,
  and C CUDA removal remain unclaimed. Local formatting, diff and library
  tests, the M14.5c2d comparator, retained selector run, and unified parity
  passed with 168 passed, 45 skipped, and 0 failed.
...
- M14.5c2c6 Gate Tile8 Rowspan Projection adds an executable-local Rust
  ...
  projection. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda`
  tests passed with 82 tests; live cargo-oxide execution found seven kernels,
  emitted portable `sm_80` PTX through libdevice, and matched each widened
  gate output on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.5c2c6/routed-moe-gate-rowspan-smoke.json`
  and `ds4-parity/check_routed_moe_gate_rowspan_smoke.py --negative-test`.
  Widened down scheduling, shared-cache specialization, Q4_K,
  hyperconnection, runtime route activation, and C CUDA removal remain
  unclaimed. Local formatting, diff, library tests, the M14.5c2c6
  comparator, retained routed-MoE checks, and unified parity passed with 161
  passed, 50 skipped, and 0 failed.
...
- M14.5c2c4 Atomic Expert-Tile Down Output adds executable-local Rust
  ...
  row32 expert schedules. On B300 pod `ds4-rust-port-b300`, feature-enabled
  `ds4-cuda` tests passed with 80 tests; live cargo-oxide execution emitted
  portable `sm_80` PTX, lowered the float atomic operation, and matched both
  atomic schedule outputs on `NVIDIA B300 SXM6 AC`. Its fixture and checker
  are `ds4-parity/baselines/backend/m14.5c2c4/routed-moe-atomic-down-smoke.json`
  and `ds4-parity/check_routed_moe_atomic_down_smoke.py --negative-test`.
  Tile16/rowspan dispatch, shared-cache specialization, Q4_K, hyperconnection,
  runtime route activation, and C CUDA removal remain unclaimed. Local
  formatting, diff, library tests, the M14.5c2c4 comparator, retained M14
  checks, and unified parity passed with 159 passed, 50 skipped, and 0 failed.
- M14.5c2c3 Tile4 Row32 Projection adds executable-local Rust cuda-oxide
  functional tile4 row32 IQ2-XXS/Q8_K gate/up and Q2_K/Q8_K non-atomic down
  kernels selected through `DS4_CUDA_MOE_TILE4` over validated expert-tile
  descriptors. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda`
  tests passed with 79 tests; live cargo-oxide execution emitted portable
  `sm_80` PTX through libdevice and matched a three-tile same-expert group,
  partial-tile output, and negative-expert fallback on `NVIDIA B300 SXM6 AC`.
  Its fixture and checker are
  `ds4-parity/baselines/backend/m14.5c2c3/routed-moe-tile4-row32-smoke.json`
  and `ds4-parity/check_routed_moe_tile4_row32_smoke.py --negative-test`.
  Shared-cache specialization, atomic-down/tile16/rowspan execution, Q4_K,
  hyperconnection, runtime route activation, and C CUDA removal remain
  unclaimed. Local formatting, diff, library tests, the M14.5c2c3 comparator,
  retained M14 checks, and unified parity passed with 158 passed, 50 skipped,
  and 0 failed.
...
- M14.5c2c1 Expert-Tile Descriptor Metadata adds executable-local Rust
  ...
  grouping. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda`
  tests passed with 77 tests; live cargo-oxide execution emitted portable
  `sm_80` PTX and matched tile totals, expert identifiers, starts, partial
  tiles, and expert-zero fallback on `NVIDIA B300 SXM6 AC`. Its fixture and
  checker are
  `ds4-parity/baselines/backend/m14.5c2c1/routed-moe-expert-tiles-smoke.json`
  and `ds4-parity/check_routed_moe_expert_tiles_smoke.py --negative-test`.
  Tile-local projection, atomic-down/rowspan execution, Q4_K, hyperconnection,
  runtime route activation, and C CUDA removal remain unclaimed. Local
  formatting, diff, library tests, the M14.5c2c1 comparator, retained M14
  checks, and unified parity passed with 156 passed, 50 skipped, and 0 failed.
- M14.5c2b2 Sorted-Pair P2 Quantized Projection adds executable-local Rust
  cuda-oxide no-expert-tiles/default-P2 batched IQ2-XXS/Q8_K gate/up and
  Q2_K/Q8_K down kernels, composed with sorted metadata, Q8_K quantization,
  and final per-token summation. On B300 pod `ds4-rust-port-b300`,
  feature-enabled `ds4-cuda` tests passed with 76 tests; live cargo-oxide
  execution emitted portable `sm_80` PTX through libdevice and matched sorted
  P2 projection and summed output behavior on `NVIDIA B300 SXM6 AC`. Its
  fixture and checker are
  `ds4-parity/baselines/backend/m14.5c2b2/routed-moe-sorted-p2-smoke.json`
  and `ds4-parity/check_routed_moe_sorted_p2_smoke.py --negative-test`.
  Expert-tile or atomic-down scheduling, Q4_K, hyperconnection, runtime route
  activation, and C CUDA removal remain unclaimed. Local formatting, diff,
  library tests, the M14.5c2b2 comparator, retained M14 checks, and unified
  parity passed with 155 passed, 50 skipped, and 0 failed.
- M14.5c2b1 Batched Sorted-Pair Metadata adds executable-local Rust
  cuda-oxide device-atomic expert counting and scattering plus prefix
  offsets/cursors for the current-C batched routed-MoE ordering surface. On
  B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with
  75 tests; live cargo-oxide execution emitted portable `sm_80` PTX and
  matched histogram, prefix, grouped-scatter, duplicate, and negative-expert
  metadata behavior on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.5c2b1/routed-moe-sorted-pairs-smoke.json`
  and `ds4-parity/check_routed_moe_sorted_pairs_smoke.py --negative-test`.
  Sorted projection, expert-tile/atomic-down execution, Q4_K, hyperconnection,
  runtime route activation, and C CUDA removal remain unclaimed. Local
...
- M14.5c1 Packed F32-Activation Routed MoE Fallback adds executable-local
  ...
  gate/down kernels, and expert summation for single-token and batched
  layouts. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests
  passed with 73 tests; live cargo-oxide execution emitted portable `sm_80`
  PTX through libdevice and matched routed fallback outputs on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.5c1/routed-moe-f32-smoke.json` and
  `ds4-parity/check_routed_moe_f32_smoke.py --negative-test`. Q8
  activation/optimized dispatch, Q4_K, hyperconnection, runtime route
  activation, and C CUDA removal remain unclaimed. Local formatting, diff,
  library tests, the M14.5c1 comparator, retained M14 checks, and unified
  parity passed with 152 passed, 50 skipped, and 0 failed.
- M14.5b Parallel And Warp Router Dispatch adds executable-local Rust
  cuda-oxide parallel shared-memory router selection and warp-shuffle top-k
  selection with deterministic equal-score ordering, partial four-row blocks,
  hash fallback, and current-C-equivalent optimized path priority. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 72 tests;
  live cargo-oxide execution emitted portable `sm_80` PTX through libdevice
  and matched optimized router outputs on `NVIDIA B300 SXM6 AC`. Its fixture
  and checker are
  `ds4-parity/baselines/backend/m14.5b/router-optimized-smoke.json` and
  `ds4-parity/check_router_optimized_smoke.py --negative-test`. Routed MoE,
  hyperconnection, runtime route activation, and C CUDA removal remain
...
- M14.4d8b CUBLAS Attention Output A Dispatch adds executable-local Rust
  ...
  and current-C-equivalent branch selection. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 69
  tests; live cargo-oxide execution emitted portable `sm_80` PTX with
  libdevice linkage and matched packed, grouped, and unpacked output on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d8b/attention-output-q8-cublas-smoke.json`
  and `ds4-parity/check_attention_output_q8_cublas_smoke.py --negative-test`.
  The safe cuda-oxide API exposes SGEMM rather than current-C's
  `CUDA_R_16F` GemmEx entry point; runtime route activation and C CUDA
...
- M14.4d8a Native Q8 Attention Output Projection Surfaces adds
  executable-local Rust cuda-oxide grouped A and B projection orchestration
  over Q8 prequantized inputs, covering low-only output, batched low/output,
  and partial blocks. On B300 pod `ds4-rust-port-b300`, feature-enabled
  `ds4-cuda` tests passed with 67 tests; live cargo-oxide execution emitted
  portable `sm_80` PTX with libdevice linkage and matched native Q8 output
  on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d8a/attention-output-q8-native-smoke.json`
  and `ds4-parity/check_attention_output_q8_native_smoke.py --negative-test`.
  F16/cuBLAS attention-output-A dispatch, runtime route activation, and C
  CUDA removal remain unclaimed. Local formatting, diff, library tests, the
  63-check d8a comparator, retained attention checks, and unified parity
  passed with 148 passed, 50 skipped, and 0 failed.
- M14.4d7 Optimized Indexed Sort And Heads8 Attention Kernels adds
  executable-local Rust cuda-oxide integration of the already owned indexed
  ascending-sort policy with heads8-online attention, plus an rb4
  filtered/duplicate indexed path and `select_attention_indexed_path`.
  On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed
  with 66 tests; live cargo-oxide execution emitted portable `sm_80` PTX
  with libdevice linkage and matched sorted-online and rb4 indexed output on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d7/attention-indexed-optimized-smoke.json`
  and `ds4-parity/check_attention_indexed_optimized_smoke.py --negative-test`.
  Output-Q8 attention, runtime route activation, and C CUDA removal remain
  unclaimed. Local formatting, diff, library tests, the 68-check d7
  comparator, retained attention checks, and unified parity passed with 147
  passed, 50 skipped, and 0 failed.
- M14.4d6 Generic Indexed Mixed Attention Surface adds executable-local Rust
  cuda-oxide generic indexed mixed attention execution with ordered and
  duplicate top-k row preservation, invalid/out-of-visible row filtering,
  ratio-zero all-compressed visibility, causal windows, ring-wrapped raw
  rows, and learned-sink softmax behavior. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 64
  tests; live cargo-oxide execution emitted portable `sm_80` PTX with
  libdevice linkage and matched indexed output on `NVIDIA B300 SXM6 AC`.
  Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d6/attention-indexed-generic-smoke.json`
  and `ds4-parity/check_attention_indexed_generic_smoke.py --negative-test`.
  Indexed sort/heads8 dispatch, output-Q8 attention, runtime route
  activation, and C CUDA removal remain unclaimed. Local formatting, diff,
  library tests, the 64-check d6 comparator, retained attention checks, and
  unified parity passed with 146 passed, 50 skipped, and 0 failed.
- M14.4d5 Static Heads8 Online And CUBLAS Attention Prefill Dispatch adds
  executable-local Rust cuda-oxide grouped-head online prefill behavior,
  live `cuda-core` strided-batched SGEMM raw and masked-mixed prefill
  pipelines, and `select_attention_prefill_path` branch-priority matching.
  On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed
  ...
  mixed cuBLAS output on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d5/attention-prefill-optimized-smoke.json`
  and `ds4-parity/check_attention_prefill_optimized_smoke.py --negative-test`.
  Indexed/output-Q8 attention, runtime route activation, and C CUDA removal
  remain unclaimed. Local formatting, diff, library tests, the 70-check d5
  comparator, retained attention checks, and unified parity passed with 150
  passed, 45 skipped, and 0 failed.
...
- M14.4d3 Heads8 Online Attention Decode Kernels adds executable-local Rust
  ...
  enabled-window selection predicates. On B300 pod `ds4-rust-port-b300`,
  feature-enabled `ds4-cuda` tests passed with 60 tests; live cargo-oxide
  execution emitted portable `sm_80` PTX with libdevice linkage and matched
  grouped/partial-group, batched causal-window, single-all, raw-ring,
  compressed-visibility, and learned-sink output on `NVIDIA B300 SXM6 AC`.
  Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d3/attention-decode-heads8-online-smoke.json`
  and `ds4-parity/check_attention_decode_heads8_online_smoke.py --negative-test`.
...
- M14.4d2 Generic Batched Mixed Attention Decode Surfaces adds
  executable-local Rust cuda-oxide attention for the generic
  `ds4_gpu_attention_decode_raw_batch_heads_tensor` and
  `ds4_gpu_attention_decode_mixed_batch_heads_tensor` surfaces. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 58
  tests; live cargo-oxide execution emitted portable `sm_80` PTX with
  libdevice linkage and matched mixed/raw batched output, causal raw-window
  selection, wrapped raw rows, per-token compressed visibility/masking, and
  learned-sink softmax on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4d2/attention-decode-batch-mixed-smoke.json`
  and `ds4-parity/check_attention_decode_batch_mixed_smoke.py --negative-test`.
...
- M14.4c3b Compressor Prefill And Replay Orchestration adds executable-local
  ...
  general prefill, ratio-4 prefill/replay, and ratio-4 state-only cases. On
  B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with
  56 tests; live cargo-oxide execution emitted portable `sm_80` PTX with
  libdevice linkage and matched state placement, replay ordering, and
  optional FP8 outputs on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4c3b/compressor-prefill-orchestration-smoke.json`
  and `ds4-parity/check_compressor_prefill_orchestration_smoke.py --negative-test`.
  ...
  checks, and unified parity passed with 140 passed, 50 skipped, and 0
  failed.
- M14.4c3a Compressor Update Orchestration adds executable-local Rust
  cuda-oxide composition of compressor row storage, update pooling, weighted
  RMS normalization, YARN RoPE, and ratio-4 post-emission shift. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 55
  tests; live cargo-oxide execution emitted portable `sm_80` PTX with
  libdevice linkage and matched non-emitting, ratio-4 emitting with F16 APE
  and a nonzero compressed-row offset, and general-ratio emitting outputs on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.4c3a/compressor-update-orchestration-smoke.json`
  and `ds4-parity/check_compressor_update_orchestration_smoke.py --negative-test`.
  Prefill/replay orchestration, FP8 compressed-cache composition, attention,
  runtime route activation, and C CUDA removal remain unclaimed. Local
  formatting, diff, library tests, the c3a comparator, retained M14 checks,
  and unified parity passed with 139 passed, 50 skipped, and 0 failed.
- M14.4c2 Compressor Pooling And Ratio-4 Shift Kernels adds executable-local
  Rust cuda-oxide `compressor_prefill_pool_kernel`,
  `compressor_update_pool_kernel`, and `compressor_shift_ratio4_kernel`
  execution with F16 APE input coverage. On B300 pod `ds4-rust-port-b300`,
  feature-enabled `ds4-cuda` tests passed with 54 tests; live cargo-oxide
  execution emitted portable `sm_80` PTX with libdevice linkage and matched
  ...
  and `ds4-parity/check_compressor_pool_shift_kernel_smoke.py --negative-test`.
  Update/prefill wrapper orchestration, normalization/RoPE/FP8 composition,
  attention, runtime route activation, and C CUDA removal remain unclaimed.
  ...
  0 failed.
- M14.4c1 Composed FP8 Raw Storage And Compressor Row Stores adds
  executable-local Rust cuda-oxide composed FP8 quantization plus raw-store
  execution and ratio-4 `compressor_store_kernel`/`compressor_set_rows_kernel`
  execution. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda`
  tests passed with 53 tests; live cargo-oxide execution emitted portable
  `sm_80` PTX with libdevice linkage and matched composed storage, ratio-4
  row geometry, and F32/F16 APE outputs on `NVIDIA B300 SXM6 AC`. Its fixture
  and checker are
  ...
  Compressor pooling/shift, wrapper orchestration, normalization/RoPE
  composition, attention, runtime route activation, and C CUDA removal remain
  unclaimed.
...
- M14.3d3 Paired And HC-Expansion Q8 Matmul Kernels adds executable-local
  ...
  execution. On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda`
  tests passed with 48 tests and live cargo-oxide execution emitted portable
  `sm_80` PTX and proved unequal-width paired output, HC block output,
  HC-expansion output with and without the optional block add, partial-block
  behavior, and invalid-shape rejection on `NVIDIA B300 SXM6 AC`. Its fixture
  and checker are
  `ds4-parity/baselines/backend/m14.3d3/q8-specialized-matmul-kernel-smoke.json`
  and `ds4-parity/check_q8_specialized_matmul_kernel_smoke.py --negative-test`.
  DP4A acceleration, Q8 dispatch, route activation, and C CUDA removal remain
  unclaimed.
  ...
  non-claims.
- M14.3d2 Base And Prequantized Q8 Matmul Kernels adds executable-local Rust
  cuda-oxide `matmul_q8_0_kernel`, `matmul_q8_0_preq_kernel`,
  `matmul_q8_0_preq_warp8_kernel`, and
  `matmul_q8_0_preq_batch_warp8_kernel` using the current-C packed Q8
  representation and scalar integer-dot semantics. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 47
  tests and live cargo-oxide execution emitted portable `sm_80` PTX through
  libdevice and proved direct quantizing, generic prequantized,
  single-token warp8, batched warp8, partial-block, and invalid-shape
  behavior on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.3d2/q8-matmul-kernel-smoke.json`
  and `ds4-parity/check_q8_matmul_kernel_smoke.py --negative-test`.
...
- M14.3d1 Q8 Dequantization And Activation Quantization Kernels adds
  executable-local Rust cuda-oxide `dequant_q8_0_to_f16_kernel`,
  `dequant_q8_0_to_f32_kernel`, and `quantize_q8_0_f32_kernel` using the
  current-C 34-byte packed Q8 block layout. Activation quantization encodes
  `lrintf`-compatible ties-to-even behavior explicitly and retains zero
  padding for partial blocks. On B300 pod `ds4-rust-port-b300`,
  feature-enabled `ds4-cuda` tests passed with 46 tests and live
  cargo-oxide execution emitted portable `sm_80` PTX through libdevice and
  proved packed F16/F32 dequantization, activation quantization,
  ties-to-even results, partial-block padding, and invalid-shape rejection
  on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.3d1/q8-conversion-kernel-smoke.json`
  and `ds4-parity/check_q8_conversion_kernel_smoke.py --negative-test`.
  Quantized matmul, Q8 dispatch, route activation, and C CUDA removal remain
  unclaimed.
  ...
  removal non-claims.
- M14.3c3 F16 And F32 BLAS Dispatch And Activation Conversion advances the
  `cuda-oxide` dependency to `d8ccb4174e0a92b1b80424c1c7258b29a07e4bb7`
  after adding mixed-precision `cublasGemmEx` and DS4-layout projection
  wrappers upstream. It adds executable-local Rust cuda-oxide
  `f32_to_f16_kernel`, current-C-compatible F16/F32 projection dispatch
  selection, and live `cuda-core` BLAS execution. On B300 pod
  `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed with 45
  tests and live cargo-oxide execution emitted portable `sm_80` PTX and
  proved activation conversion, mixed F16/F32 BLAS projection, F32 BLAS
  projection, dispatch priority, pair dispatch, and invalid-shape rejection
  on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.3c3/blas-projection-kernel-smoke.json`
  and `ds4-parity/check_blas_projection_kernel_smoke.py --negative-test`.
...
- M14.3c1 Base F16 And F32 Projection Kernels adds executable-local Rust
  ...
  On B300 pod `ds4-rust-port-b300`, feature-enabled `ds4-cuda` tests passed
  with 42 tests and live cargo-oxide execution emitted portable `sm_80` PTX
  and proved F16 output, F32 output, multi-token output stride, and invalid
  shape rejection on `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.3c1/dense-projection-kernel-smoke.json`
  and `ds4-parity/check_dense_projection_kernel_smoke.py --negative-test`.
  Ordered/paired/serial F16 variants, cuBLAS dispatch, Q8 kernels, route
  activation, and C CUDA removal remain unclaimed.
  Local formatting, diff, workspace tests, the 74-check comparator, and
  ...
  route non-claims.
- M14.3b2 Head RMS Norm Rope Tail Kernel adds executable-local Rust
  cuda-oxide `head_rms_norm_rope_tail_kernel`, retaining current-C's
  per-head reduction, tail-only rotation, YARN correction/ramp path, and
  inverse sign behavior. On B300 pod `ds4-rust-port-b300`, feature-enabled
  `ds4-cuda` tests passed with 41 tests and live cargo-oxide execution
  emitted portable `sm_80` PTX and proved interpolated rotation, YARN
  forward rotation, inverse rotation, and invalid-shape rejection on
  `NVIDIA B300 SXM6 AC`. Its fixture and checker are
  `ds4-parity/baselines/backend/m14.3b2/head-rms-rope-tail-kernel-smoke.json`
  and `ds4-parity/check_head_rms_rope_tail_kernel_smoke.py --negative-test`.
  Standalone RoPE, projection, Q8 kernels, route activation, and C CUDA
  removal remain unclaimed.
  Local formatting, diff, workspace tests, the 74-check comparator, and
