...
- Evidence: Rust exports both public raw KV wrappers through one embedded
  FP16-rounding ring-storage kernel. A C-linked B300 witness proves batched
  and single-row storage, `uint32_t` wraparound ring positioning on distinct
  destination rows, untouched rows, zero-grid rejection, invalid-shape
  rejection, and null rejection. Local tests pass with 140 tests; B300
  feature tests pass with 147 tests; the static library exposes 56 symbols
  and embeds 33 kernels; all 50 preceding linked ABI consumers pass against
  the rebuilt archive with the known executable-stack warning. All 54 CUDA
  ABI comparators pass, and the unified parity report passes with 226 passed,
  45 skipped, and 0 failed. Overlapping same-launch ring writes and composed
  FP8/raw storage remain unclaimed. The pre-implementation and final
  pass-end non-interactive Claude review attempts each returned
  `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Goal: Rust-own `ds4_gpu_compressor_prefill_state_ratio4_tensor` through
  state initialization and one public set-rows kernel without claiming
  compressor update/full prefill, attention, routed MoE, remaining graph
  compute, or route ownership.
- Fixture:
...
- Evidence: Rust exports the public ratio-4 state-only wrapper through one
  embedded set-rows kernel after driver-backed zero and negative-infinity
  state initialization. A C-linked B300 witness proves FP32 and FP16 APE
  output, lower-bank row placement, untouched initialized rows, invalid
  model-range no-mutation ordering, invalid-shape rejection, checked-overflow
  rejection, and null rejection. Ratio-4 modulo cannot distinguish wrapped
  from widened position addition, so this leaf does not claim that separate
  witness. Local tests pass with 143 tests; B300 feature tests pass with 150
  tests; the static library exposes 59 symbols and embeds 35 kernels; all 53
  preceding linked ABI consumers pass against the rebuilt archive with the
  known executable-stack warning. All 57 CUDA ABI comparators pass, and the
  unified report passes with 229 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. Compressor update/full prefill,
  attention, routed MoE, remaining graph compute, route promotion, and C
  CUDA removal remain unclaimed.
...
- Evidence: Rust exports the public update wrapper through embedded
  update-pool and ratio-4 shift kernels composed with the already-owned store,
  weighted RMS, and stride-1 RoPE paths. A C-linked B300 witness proves
  non-emitting state-only mutation, ratio-4 FP16 emission and duplication,
  general-ratio FP32 emission without shifting, wrapped-position emission,
  the emitted zero-RoPE partial-failure boundary, invalid-range preservation,
  invalid-shape rejection, checked-overflow rejection, and null rejection.
  Local tests pass with 145 tests; B300 feature tests pass with 152 tests; the
  static library exposes 61 symbols and embeds 38 kernels; all 55 preceding
  linked ABI consumers pass against the rebuilt archive with the known
  executable-stack warning. All 59 CUDA ABI comparators pass, and the unified
  report passes with 231 passed, 45 skipped, and 0 failed. The
  pre-implementation and final pass-end non-interactive Claude review
  attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`. General prefill,
  attention, routed MoE, remaining graph compute, route promotion, and C CUDA
...
- Status: done
- Goal: Rust-own `ds4_gpu_attention_decode_heads_tensor` through its generic
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because the public prefill raw/static/masked wrappers share one current-C
  dispatch family and together close the public attention ABI.
- Goal: connect remaining graph compute, whole-archive retention policy, and
  production route-promotion work without claiming C CUDA removal before
  those gates pass.
...
- Goal: Rust-own `ds4_gpu_attention_prefill_raw_heads_tensor`,
  `ds4_gpu_attention_prefill_static_mixed_heads_tensor`, and
  `ds4_gpu_attention_prefill_masked_mixed_heads_tensor` through generic,
  online-heads8, and safe-SGEMM current-C dispatch without claiming routed
  MoE, remaining graph compute, or route ownership.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports all three checked prefill wrappers,
    preserves dispatcher priority, retains safe-SGEMM scratch, and clears it
    on public cleanup.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds generic, online, and SGEMM
    adapter kernels promoted from the measured M14.4d4 and M14.4d5 work.
  - A C-linked B300 consumer proves generic raw/static/masked output,
    ...
    masked cuBLAS behavior, invalid-range preservation, invalid-shape
    rejection, and null rejection.
  ...
  - The unified report passes with 238 passed, 45 skipped, and 0 failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Public attention ownership is complete; routed MoE, remaining graph
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because single-token routed MoE has a bounded public CUDA contract while
  the batched public `mid_is_f16` result is not implemented by current CUDA.
- Goal: connect remaining graph compute, whole-archive retention policy, and
  production route-promotion work without claiming C CUDA removal before
  those gates pass.
...
- Status: done
- Goal: Rust-own `ds4_gpu_routed_moe_one_tensor` across its reachable F32,
  IQ2/Q2, Q4_K, and environment-selected single-token paths while deferring
  the batched contract gap and route ownership.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the checked wrapper, retains device IQ2
    tables, resolves cached model ranges, and keeps public packed Q8_K scratch
    aliasing.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds the single-token F32,
    quantized, Q4_K, generic, direct-six, and sum kernels.
  - A C-linked B300 witness proves F32 fallback, default quantized and Q4_K
    output, forced generic routes, Q8 scratch visibility, auxiliary writes,
    negative fallback, and rejection controls.
  - Local tests pass with 154 tests; B300 feature tests pass with 161 tests;
    the static library exports 74 symbols and embeds 72 kernels.
  - All 64 preceding linked ABI consumers pass with the known executable-stack
    warning.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because the tile4/tile8 row32 down entries include the atomic-output
  branch and require its output-zeroing prerequisite.
- Goal: retain row32 tiled and atomic-down kernels, then integrate tile16
  and row-span routed-MoE batch dispatch before connecting remaining
  graph compute, whole-archive retention policy, and production
  route-promotion work without claiming C CUDA removal before those gates
  pass.
...
- Status: done
- Goal: retain tile4/tile8 row32 routed-MoE gate/down kernels and their
  row32 atomic-output zeroing prerequisite in the Rust CUDA ABI module
  without claiming public batched routed-MoE ownership.
- Fixture:
...
- Evidence:
  - Five new retained entries embed tile4/tile8 row32 gate/down and
    atomic-output zeroing behavior already executed in prior B300 proofs.
  - The B300 rebuilt-staticlib regression consumer loads 86 embedded kernels
    while the archive continues to expose 74 Rust ABI symbols.
  - Local tests pass with 157 tests; B300 feature tests pass with 164 tests;
    all 71 CUDA ABI comparators pass; and the unified report passes with 244
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Tile16, row-span, shared-cache specialization, and public batched
    routed-MoE ownership remain pending.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because tile16 row32 atomic down is already B300-proved and can be
  retained independently of row-span and shared-cache dispatch.
- Goal: retain tile16 row32 atomic down, then integrate or further scope
  row-span and shared-cache routed-MoE batch dispatch before connecting
  remaining graph compute, whole-archive retention policy, and production
  route-promotion work without claiming C CUDA removal before those gates
  pass.
...
- Status: done
- Goal: retain the tile16 row32 routed-MoE atomic-down kernel in the Rust
  CUDA ABI module without claiming public batched routed-MoE ownership.
- Fixture:
...
- Evidence:
  - One new retained entry embeds tile16 row32 atomic down with its
    current-C tile-start alignment guard while reusing retained descriptor
    construction and zeroing prerequisites.
  - The B300 rebuilt-staticlib regression consumer loads 87 embedded kernels
    while the archive continues to expose 74 Rust ABI symbols.
  - Local tests pass with 158 tests; B300 feature tests pass with 165 tests;
    all 72 CUDA ABI comparators pass; and the unified report passes with 245
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Row-span, shared-cache specialization, and public batched routed-MoE
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because gate row-span functional projection is already B300-proved and
  can be retained independently of down row-span and shared-cache dispatch.
- Goal: retain gate row-span projection, then integrate or further scope
  tile16 down row-span and shared-cache routed-MoE batch dispatch before
...
- Status: done
- Goal: retain the tile8 gate row-span functional projection kernel in the
  Rust CUDA ABI module without claiming down row-span, shared-cache
  optimization, or public batched routed-MoE ownership.
- Fixture:
...
- Evidence:
  - One new retained entry embeds tile8 gate row-span functional projection
    for the current row512, row1024, and row2048 gate route surface.
  - The B300 rebuilt-staticlib regression consumer loads 88 embedded kernels
    while the archive continues to expose 74 Rust ABI symbols.
  - Local tests pass with 159 tests; B300 feature tests pass with 166 tests;
    all 73 CUDA ABI comparators pass; and the unified report passes with 246
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Tile16 down row-span, shared-cache specialization, and public batched
    routed-MoE ownership remain pending.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because tile16 down row-span functional projection is already B300-proved
  and can be retained independently of shared-cache dispatch.
- Goal: retain tile16 down row-span projection, then integrate or further
  scope shared-cache routed-MoE batch dispatch before connecting remaining
  graph compute, whole-archive retention policy, and production
  route-promotion work without claiming C CUDA removal before those gates
  pass.
...
- Status: done
- Goal: retain the tile16 atomic down row-span functional projection kernel
  in the Rust CUDA ABI module without claiming shared-cache optimization or
  public batched routed-MoE ownership.
- Fixture:
...
- Evidence:
  - One new retained entry embeds tile16 atomic down row-span functional
    projection for the current row512, row1024, and row2048 route surface.
  - The B300 rebuilt-staticlib regression consumer loads 89 embedded kernels
    while the archive continues to expose 74 Rust ABI symbols.
  - Local tests pass with 160 tests; B300 feature tests pass with 167 tests;
    all 74 CUDA ABI comparators pass; and the unified report passes with 247
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Shared-cache specialization and public batched routed-MoE ownership
    remain pending.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because the retained F32 fallback and sum entries need batched-layout
  shaping before public batched-wrapper orchestration.
- Goal: widen retained fallback entries, then implement host-side batched
...
- Status: done
- Goal: widen already-retained F32 fallback and sum ABI entries for the
  previously proved batched layout without claiming public batched
  routed-MoE ownership.
- Fixture:
...
- Evidence:
  - The retained F32 gate, down, and sum entries and their launch methods now
    accept `n_tokens` over the prior-proved flattened batched layout.
  - The existing public single-token routed-MoE wrapper passes `n_tokens = 1`;
    it still does not export `ds4_gpu_routed_moe_batch_tensor`.
  - The B300 rebuilt-staticlib regression consumer loads 91 embedded kernels
    while the archive continues to expose 74 Rust ABI symbols.
  - Local tests pass with 162 tests; B300 feature tests pass with 169 tests;
    all 76 CUDA ABI comparators pass; and the unified report passes with 249
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Evidence:
  - Rust exports the public DSV4 top-k mask wrapper and preserves current-C
    null, zero-dimension, and span validation without adding a
    `top_k <= n_comp` restriction.
  - A linked B300 consumer validates selected `0.0` and excluded negative
    infinity mask output over two tokens plus short-span/null/zero-dimension
    rejection; the public fused QKV RMS linked predecessor remains passing.
  - The rebuilt staticlib exposes 77 Rust ABI symbols and embeds 93 kernels.
    Local tests pass with 167 tests; B300 feature tests pass with 174 tests;
    all 81 CUDA ABI comparators pass; and the unified report passes with 254
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Four indexer score/top-k selection public exports, route promotion,
    whole-archive policy, and C CUDA removal remain pending.
...
- Evidence:
  - Rust exports the public top-k wrapper through current-C-compatible scalar,
    specialized, packed-key, and chunked-tree selection with reusable
    chunked-tree scratch storage.
  - A linked B300 consumer validates scalar, topk1024, topk2048, packed-key,
    chunked-tree, disabled-specialization fallback, and rejection controls;
    the public indexer score linked predecessor remains passing.
  - The rebuilt staticlib exposes 81 Rust ABI symbols and embeds 108 kernels.
    Local tests pass with 169 tests; B300 feature tests pass with 176 tests;
    all 83 CUDA ABI comparators pass; and the unified report passes with 256
    passed, 45 skipped, and 0 failed. The pre-implementation and final
    pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  boundary while Rust used division by `sqrt`.
...
- Status: done
- Goal: align the Rust CUDA RMS and attention scale primitive with current C
  and close the known short official-vector mismatch.
- Fixture:
...
- Evidence:
  - The Rust kernels now use device `__nv_rsqrtf` at C-equivalent RMS and
    attention scale sites, with a host DSO resolution stub for the unused
    cuda-oxide device-extern host edge.
  - On B300, the formerly failing `short_code_completion` step `1` now
    selects expected hex `63`; the three-short-case slice passes all `9`
    selected tokens.
  - Five affected ABI consumers pass against the rebuilt DSO and B300 feature
    tests pass with `176` tests. The full official-vector route remains at
    100% GPU use beyond `935` seconds and is terminated cleanly; route
    promotion and C CUDA removal remain blocked.
  - Local `ds4-cuda` library tests pass with `169` tests; the unified report
    passes with `260` passed, `45` skipped, and `0` failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Status: done
- Goal: reduce cached Rust CUDA routed-MoE down overhead by expanding the
  four fixed Q2 group steps inside each existing ordered chunk.
- Fixture:
...
- Evidence:
  - The retained kernel introduces `abi_moe_down_accumulate_q2_group!` only
    for cached down; its outer two-chunk order, packed-load expansion,
    staged-pair arithmetic, gate/up, dispatch, and default routing remain
    unchanged.
  - Emitted down PTX moves from `8` to `32` DP4A sites while local
    stores/loads increase from `51/6` to `54/9`; the sole remaining call
...
- Evidence:
  - The cached gate/up kernel uses `abi_moe_cached_weight_load_u16!` at ten
    packed IQ2 weight read sites while retaining DP4A topology, cached down,
    row-span policy, and default current-C routing.
  - Row32 fallback probes regress gate/up to `15046.777 ms` and down to
    `10690.146 ms`; a fixed-row512 codegen probe also regresses gate/up to
    `1483.618 ms`, so none are retained.
  - Adjacent B300 parent/candidate measurements lower gate/up from
    `1482.196 ms` to `1461.528 ms` and total routed-MoE from `2337.688 ms`
    to `2316.378 ms`; a second candidate records `1462.521 ms` gate/up.
  - The candidate DSO SHA-256 is
