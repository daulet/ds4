...
static void usage(FILE *fp) {
    fputs("usage: ds4-layer0-attn-output-oracle-dump -m MODEL -o OUTPUT.json [--token N]\n", fp);
}

static void die(const char *msg) {
    fprintf(stderr, "ds4-layer0-attn-output-oracle-dump: %s\n", msg);
    exit(1);
}

static const char *need_arg(int *i, int argc, char **argv, const char *flag) {
    if (*i + 1 >= argc) {
        fprintf(stderr, "ds4-layer0-attn-output-oracle-dump: missing value for %s\n", flag);
        exit(2);
    }
    return argv[++*i];
}
...
int main(int argc, char **argv) {
    ...
    if (!fp) {
        fprintf(stderr,
                "ds4-layer0-attn-output-oracle-dump: failed to open %s: %s\n",
                opt.output_path,
                strerror(errno));
        return 1;
    }
    const int rc = ds4_dump_layer0_attn_output_oracle_json(opt.model_path, opt.token, fp);
    if (fclose(fp) != 0) {
        fprintf(stderr,
                "ds4-layer0-attn-output-oracle-dump: failed to close %s: %s\n",
                opt.output_path,
                strerror(errno));
        return 1;
    }
