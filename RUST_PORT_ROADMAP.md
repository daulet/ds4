...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the in-place public wrapper with
    checked tensor-span and nonzero-dimension validation.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds the 256-thread head RMS
    reduction already proved in the opt-in kernel campaign.
  - A C-linked B300 consumer proves multi-row in-place normalization, short
    tensor rejection, zero-dimension rejection, and null rejection.
  - Local library tests pass with 136 tests; B300 release-feature tests pass
    with 143 tests; the static library exposes 51 Rust ABI symbols; all 46
    preceding linked ABI consumers pass against the rebuilt archive with the
    known generated embedded-object executable-stack warning.
  - All 50 CUDA ABI comparators pass, and the unified parity report passes
    with 222 passed, 45 skipped, and 0 failed.
  - Rust rejects zero-dimensional and oversized launch grids rather than
    retaining current-C zero-grid or arithmetic behavior.
  - The pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the in-place public wrapper with
    checked tensor-span, prefix-boundary, and nonzero-token validation.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds the 64-thread E4M3FN
    quantization kernel already proved in the opt-in kernel campaign.
  - A C-linked B300 consumer proves prefix output, partial-chunk handling,
    untouched RoPE tail, empty-prefix and zero-width no-op behavior, and
    invalid-input rejection.
  - Local library tests pass with 137 tests; B300 release-feature tests pass
    with 144 tests; the static library exposes 52 Rust ABI symbols; all 47
    preceding linked ABI consumers pass against the rebuilt archive with the
    known generated embedded-object executable-stack warning.
  - All 51 CUDA ABI comparators pass, and the unified parity report passes
    with 223 passed, 45 skipped, and 0 failed.
  - Rust rejects zero-token launches before current-C's invalid zero-grid
    submission while preserving its no-op dimension behavior.
  - The pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports one ordered composition wrapper that
    calls public FP8 quantization before public raw storage, preserving the
    current-C partial-mutation failure order.
  - The wrapper reuses `abi_fp8_kv_quantize_kernel` and
    `abi_store_raw_kv_batch_kernel`; it does not add or claim a fused embedded
    kernel.
  - A C-linked B300 consumer proves FP8 prefix output, untouched rotary tail,
    FP16 raw-row storage, `UINT32_MAX` raw-row modulo, invalid-shape/null
    rejection, and quantization retained after a short raw destination fails.
  - Local library tests pass with 141 tests; B300 release-feature tests pass
    with 148 tests; the static library exposes 57 Rust ABI symbols and embeds
    33 kernels; all 51 preceding linked ABI consumers pass against the rebuilt
    archive with the known generated embedded-object executable-stack warning.
  - All 55 CUDA ABI comparators pass, and the unified parity report passes
    with 227 passed, 45 skipped, and 0 failed.
  - Compressor, attention, routed MoE, production route promotion, and C CUDA
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the public wrapper with checked source,
    state, selected APE model-range, and launch-grid validation.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds one compressor-store kernel
    preserving current-C ratio-4 upper-bank placement, FP32/FP16 APE reads,
    and `uint32_t` phase wrap before modulo.
  - A C-linked B300 consumer proves ratio-4 FP32 and FP16 outputs,
    non-power-of-two `UINT32_MAX` position wrap, untouched rows,
    invalid-range/shape rejection, checked-overflow rejection, and null
    rejection.
  - Local library tests pass with 142 tests; B300 release-feature tests pass
    with 149 tests; the static library exposes 58 Rust ABI symbols and embeds
    34 kernels.
  - All 52 preceding linked ABI consumers pass against the rebuilt archive
    with the known generated embedded-object executable-stack warning.
  - All 56 CUDA ABI comparators pass, and the unified parity report passes
    with 228 passed, 45 skipped, and 0 failed.
  - The pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Compressor update/prefill, attention, routed MoE, production route
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the replay wrapper with checked
    fixed-ratio geometry, cached APE/norm reads, output post-processing, and
    state rebuild ordering.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds one ratio-4 replay-pool kernel
    and reuses set-rows, RMS, stride-aware RoPE, and FP8 launch paths.
  - A C-linked B300 consumer proves FP32 and FP16 output, stride-4 RoPE,
    optional FP8 composition, state rebuild after output, zero-RoPE handling,
    invalid-range preservation, invalid-shape/overflow rejection, and null
    rejection.
  - Ratio-4 phase modulo cannot distinguish wrapped from widened position
    addition; this fixed-ratio leaf therefore does not claim a discriminating
    wrap witness.
  - Local library tests pass with 144 tests; B300 release-feature tests pass
    with 151 tests; the static library exposes 60 Rust ABI symbols and embeds
    36 kernels.
  - All 54 preceding linked ABI consumers pass against the rebuilt archive
    with the known executable-stack warning, and all 58 CUDA ABI comparators
    pass.
  - The unified report passes with 230 passed, 45 skipped, and 0 failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Compressor update/general prefill, attention, routed MoE, production
    route promotion, and C CUDA removal remain unclaimed.

################################################### M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb: Remaining Graph Compute And Route Promotion Policy

- Status: active; split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  and M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
  because the public compressor update wrapper is independently comparable
  before general prefill, attention, routed MoE, and route work.
- Goal: connect remaining graph compute, whole-archive retention policy, and
  production route-promotion work without claiming C CUDA removal before
  those gates pass.
...
- Status: done.
- Goal: Rust-own `ds4_gpu_compressor_update_tensor` through its store-first
  emission boundary, update pooling, post-processing, and ratio-4 state
  duplication without claiming general prefill, attention, routed MoE,
  remaining graph compute, or route ownership.
- Fixture:
  `ds4-parity/baselines/backend/m14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba/abi-compressor-update-smoke.json`.
- Comparator:
  `ds4-parity/check_cuda_abi_compressor_update_smoke.py --negative-test`.
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the update wrapper with checked
    ratio-dependent geometry and preserves current-C store-before-emit order.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds update-pool and ratio-4 shift
    kernels while reusing the public store, weighted RMS, and stride-1 RoPE
    launch paths.
  - A C-linked B300 consumer proves non-emitting state-only mutation,
    ratio-4 FP16 emission and post-output duplication, general-ratio FP32
    emission without shifting, wrapped-position emission, the emitted
    zero-RoPE partial-failure boundary, invalid-range preservation,
    invalid-shape/overflow rejection, and null rejection.
  - Local library tests pass with 145 tests; B300 release-feature tests pass
    with 152 tests; the static library exposes 61 Rust ABI symbols and embeds
    38 kernels.
  - All 55 preceding linked ABI consumers pass against the rebuilt archive
    with the known executable-stack warning, and all 59 CUDA ABI comparators
    pass.
  - The unified report passes with 231 passed, 45 skipped, and 0 failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - General prefill, attention, routed MoE, production route promotion, and C
    CUDA removal remain unclaimed.

################################################### M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb: Remaining Graph Compute And Route Promotion Policy

- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  and M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
  because the public general compressor prefill wrapper is independently
  comparable before attention, routed MoE, and route work.
- Goal: connect remaining graph compute, whole-archive retention policy, and
  production route-promotion work without claiming C CUDA removal before
  those gates pass.
...
- Status: done.
- Goal: Rust-own `ds4_gpu_compressor_prefill_tensor` through state
  reconstruction, non-replay pooling, and optional post-processing without
  claiming attention, routed MoE, remaining graph compute, or route
  ownership.
- Fixture:
  `ds4-parity/baselines/backend/m14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba/abi-compressor-prefill-smoke.json`.
- Comparator:
  `ds4-parity/check_cuda_abi_compressor_prefill_smoke.py --negative-test`.
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports the wrapper with checked
    ratio-dependent geometry, APE-before-mutation ordering, and state-only
    success when no complete compressed rows exist.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds one non-replay pool kernel while
    reusing state-row placement, weighted RMS, stride-by-ratio RoPE, and
    optional FP8 paths.
  - A C-linked B300 consumer proves ratio-4 FP16 output and both state banks,
    optional FP8 composition, general-ratio FP32 output/remainder state,
    state-only `n_comp == 0`, successful zero-RoPE output, wrapped positions,
    invalid-range preservation, invalid-shape/overflow rejection, and null
    rejection.
  - Local library tests pass with 146 tests; B300 release-feature tests pass
    with 153 tests; the static library exposes 62 Rust ABI symbols and embeds
    39 kernels.
  - All 56 preceding linked ABI consumers pass against the rebuilt archive
    with the known executable-stack warning, and all 60 CUDA ABI comparators
    pass.
  - The unified report passes with 232 passed, 45 skipped, and 0 failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Attention, routed MoE, production route promotion, and C CUDA removal
    remain unclaimed.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because the public single and batched router-selection wrappers share
  scalar, parallel, and warp current-C dispatch with one measurable
  bias/hash contract.
- Goal: connect remaining graph compute, whole-archive retention policy, and
  production route-promotion work without claiming C CUDA removal before
  those gates pass.
...
- Goal: Rust-own `ds4_gpu_router_select_tensor` and
  `ds4_gpu_router_select_batch_tensor` through current-C scalar, parallel,
  and warp dispatch while preserving biased top-k, hash routing, token
  fallback, and normalized weight behavior without claiming routed MoE or
  route ownership.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports both checked public wrappers, resolves
    optional bias/hash model ranges, applies current-C environment dispatch,
    and rejects zero hash rows and undersized single-token spans before
    launch.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds scalar, parallel, and warp
    router kernels promoted from the measured M14.5a and M14.5b proofs.
  - A C-linked B300 consumer proves default warp bias output, single and
    batched hash fallback, forced parallel/scalar output, partial warp-block
    batching, stable tie order, and reject-before-write controls.
  ...
  - The unified report passes with 239 passed, 45 skipped, and 0 failed. The
    pre-implementation and final pass-end non-interactive Claude review
    attempts each returned `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Router selection ABI ownership is complete; routed MoE, remaining graph
    compute, production route promotion, whole-archive retention policy, and
    C CUDA removal remain unclaimed.
...
- Evidence:
  ...
  - At this leaf, batched routed-MoE ownership remained blocked by the
    `mid_is_f16` public result gap; the following CUDA contract-repair leaf
    resolves that prerequisite without claiming Rust batch execution.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because CUDA's missing batched `mid_is_f16` result can be repaired and
  validated independently of Rust ownership of its full scheduling matrix.
...
- Status: done.
- Goal: repair the retained CUDA `ds4_gpu_routed_moe_batch_tensor` public
  result contract by reporting its actual F32 `mid` representation without
  claiming Rust batch execution or route ownership.
- Fixture:
...
- Evidence:
  - `ds4_cuda.cu` now accepts the already-declared `bool *mid_is_f16`
    result on the public batch wrapper and writes `false` only after a
    successful CUDA `routed_moe_launch`.
  - A C-linked B300 consumer compiled the retained original CUDA backend for
    `sm_103` and proved two-token nonzero output, successful F32 reporting,
    optional null result handling, and rejection without result mutation.
  - Local `ds4-cuda` library tests pass with 154 tests; all 68 Rust CUDA ABI
    ...
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Rust still does not export `ds4_gpu_routed_moe_batch_tensor`; promotion of
    the sorted, tiled, atomic, and row-span batch scheduling matrix, route
    promotion, whole-archive retention policy, and C CUDA removal remain
    unclaimed.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  and M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
  because the row32 tile4/tile8 down entries contain their atomic-output
  branch and must be retained with its output-zeroing prerequisite.
- Goal: retain row32 tiled and atomic-down routed-MoE kernels, then integrate
  tile16 and row-span batch routes before connecting remaining graph
  compute, whole-archive retention policy, and production route-promotion
  work without claiming C CUDA removal before those gates pass.
...
- Status: done.
- Goal: retain the B300-proved tile4/tile8 row32 routed-MoE gate/down
  kernels and row32 atomic-output zeroing prerequisite in the Rust CUDA ABI
  module without exporting the incomplete public batched wrapper.
- Fixture:
  `ds4-parity/baselines/backend/m14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba/abi-routed-moe-row32-tiled-atomic-module-smoke.json`.
- Comparator:
  `ds4-parity/check_cuda_abi_routed_moe_row32_tiled_atomic_module_smoke.py --negative-test`.
- Evidence:
  - `rust/ds4-cuda/src/abi_kernels.rs` now retains row32 tile4/tile8
    gate/up/mid projection kernels, tile4/tile8 down kernels with their
    `atomic_out` branch, and the atomic-output zeroing prerequisite.
  - `rust/ds4-cuda/src/lib.rs` records 86 embedded kernels while preserving
    74 public Rust ABI symbols and expressly not claiming tile16, row-span,
    or public batch ownership.
  - B300 `sm_80` compilation lowered the retained float atomic path and the
    rebuilt static library links/runs the existing C-linked public
    single-token routed-MoE consumer while loading the 86-entry module.
  - Local library tests pass with 157 tests; B300 release-feature tests pass
    with 164 tests; all 71 CUDA ABI comparators pass; and the unified report
    passes with 244 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Tile16, row-span, shared-cache specialization, public batched routed-MoE
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  and M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb
  because the prior B300 tile16 down row-span proof is an isolated
  functional module prerequisite for later shared-cache dispatch.
- Goal: retain tile16 down row-span projection, then integrate or further
  scope shared-cache routed-MoE batch routes before connecting remaining
  graph compute, whole-archive retention policy, and production
  route-promotion work without claiming C CUDA removal before those gates
  pass.
...
- Status: done.
- Goal: retain the B300-proved tile16 atomic down row-span functional
  projection kernel in the Rust CUDA ABI module without claiming
  shared-cache optimization or public batched-wrapper ownership.
- Fixture:
  `ds4-parity/baselines/backend/m14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba/abi-routed-moe-down-rowspan-module-smoke.json`.
- Comparator:
  `ds4-parity/check_cuda_abi_routed_moe_down_rowspan_module_smoke.py --negative-test`.
- Evidence:
  - `rust/ds4-cuda/src/abi_kernels.rs` now retains one tile16 down row-span
    entry spanning the functional row512, row1024, and row2048 route surface
    while preserving the tile-start alignment guard and token-indexed atomic
    accumulation branch.
  - `rust/ds4-cuda/src/lib.rs` records 89 embedded kernels while preserving
    74 public Rust ABI symbols and expressly not claiming shared-cache
    dispatch or public batch ownership.
  - B300 `sm_80` compilation generated the down row-span entry, and the
    rebuilt static library links/runs the existing C-linked public
    single-token routed-MoE consumer while loading the 89-entry module.
  - Local library tests pass with 160 tests; B300 release-feature tests pass
    with 167 tests; all 74 CUDA ABI comparators pass; and the unified report
    passes with 247 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Shared-cache specialization, public batched routed-MoE export, route
    promotion, and C CUDA removal remain open.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because the prior B300 shared-cache specialization proof is an isolated
  embedded-kernel prerequisite for later public batch orchestration.
...
- Goal: retain the B300-proved cached tile8 gate and tile16 atomic down
  row-span kernels in the Rust CUDA ABI module without exporting the
  incomplete public batched-wrapper surface.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi_kernels.rs` now retains cached gate and down
    row-span entries with synchronized packed Q8 staging and synchronized
    IQ2 lookup/sign staging for gate. The retained capacities remain bounded
    to `xq_blocks <= 16` and `midq_blocks <= 8`.
  - `rust/ds4-cuda/src/lib.rs` records 91 embedded kernels while preserving
    74 public Rust ABI symbols and expressly not claiming the public batched
    routed-MoE wrapper or default-route ownership.
  - B300 `sm_80` compilation generated both cached entries, and the rebuilt
    static library links/runs the existing C-linked public single-token
    routed-MoE consumer while loading the 91-entry module.
  - Local library tests pass with 161 tests; B300 release-feature tests pass
    with 168 tests; all 75 CUDA ABI comparators pass; and the unified report
    passes with 248 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Public batched routed-MoE export, route promotion, whole-archive closure,
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  because sorted-pair metadata and P2/fallback submission are an isolated
...
- Goal: add internal sorted launch adapters, then implement expert-tile,
  row-span, cached, and atomic-down host orchestration before defining the
  public batch wrapper or claiming C CUDA removal.
...
- Status: done.
- Goal: add internal Rust ABI-module host launch methods for already embedded
  sorted metadata and sorted P2 or fallback projection kernels without
  exporting the incomplete public batch wrapper.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi_kernels.rs` now exposes internal launch adapters
    for sorted count/prefix/scatter metadata plus sorted P2 and no-P2
    gate/down projection entries, with caller-provided validated scratch
    spans and pair-count-sized result spans.
  - This leaf compiles the host launch adapters only: no Rust public batch
    route invokes them yet, and no expert-tile, row-span, cached, or
    atomic-down host orchestration is claimed.
  - `rust/ds4-cuda/src/lib.rs` records this sorted host-launch boundary while
    preserving 74 public Rust ABI symbols and 91 embedded kernels.
  ...
  - Local library tests pass with 163 tests; B300 release-feature tests pass
    with 170 tests; all 77 CUDA ABI comparators pass; and the unified report
    passes with 250 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` now exports the public batch wrapper, delegates
    single-token calls to the proved public wrapper, and retains
    mutex-guarded sorted/tile device scratch whose counts are cleared on the
    active stream before each sorted invocation.
  - Multi-token IQ2/Q2 execution follows current-C environment dispatch
    through sorted P2, sorted no-P2, tiled row32, tile4 atomic, and the
    128-token tile16 row-span atomic path; insufficient packed-Q8 scratch
    retains the batched F32 fallback. Successful calls report
    `mid_is_f16 = false`.
  - The B300 `sm_80` rebuilt static library exposes 75 Rust ABI symbols,
    embeds 91 kernels, preserves the single-token linked regression, and
    runs the new public-batch linked harness across all recorded routes.
  - Local library tests pass with 165 tests; B300 release-feature tests pass
    ...
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Public routed-MoE ABI ownership is complete. Graph-wide route promotion,
    remaining compute ownership, whole-archive policy, and C CUDA removal
    remain open.
...
- Status: split into M14.6b2b2b2b2b2b2b2b2b2b2b2b2b2bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbba
  ...
  executable and comparable before the remaining indexer graph wrappers and
  route promotion.
...
- Goal: export `ds4_gpu_dsv4_qkv_rms_norm_rows_tensor` from the Rust CUDA ABI
  and execute the fused and disabled-fusion weighted-row paths through a
  linked B300 consumer.
- Fixture:
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` now exports the public fused Q/KV
    weighted-RMS rows wrapper and delegates `DS4_CUDA_DISABLE_QKV_RMS_FUSED`
    to the existing public weighted-row wrappers.
  - `rust/ds4-cuda/src/abi_kernels.rs` retains
    `abi_dsv4_qkv_rms_norm_rows_kernel`; the linked B300 harness passes fused
    and fallback output checks with asymmetric Q/KV widths plus rejection
    controls.
  - The B300 `sm_80` rebuilt static library exposes 76 Rust ABI symbols and
    embeds 92 kernels; the public batched routed-MoE linked regression
    remains passing.
  - Local library tests pass with 166 tests; B300 release-feature tests pass
    with 173 tests; all 80 CUDA ABI comparators pass; and the unified report
    passes with 253 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Five indexer/top-k public exports, graph-wide route promotion,
    whole-archive production policy, and C CUDA removal remain open.
...
- Evidence:
  - `rust/ds4-cuda/src/abi.rs` exports all three public score wrappers through
    shared validation and current-C-compatible selection of scalar,
    direct-one, and WMMA128/64/32/base paths.
  - `rust/ds4-cuda/src/abi_kernels.rs` embeds the six corresponding score
    entries; the linked B300 harness verifies each route, quality-mode scalar
    fallback, decode-batch output, and rejection controls.
  - The B300 `sm_80` rebuilt static library exposes 80 Rust ABI symbols and
    embeds 99 kernels; the public top-k mask linked regression remains
    passing.
  - Local library tests pass with 168 tests; B300 release-feature tests pass
    with 175 tests; all 82 CUDA ABI comparators pass; and the unified report
    passes with 255 passed, 45 skipped, and 0 failed. The pre-implementation
    and final pass-end non-interactive Claude review attempts each returned
    `CLAUDE_REVIEW_TIMEOUT_AFTER_60S`.
  - Indexer top-k selection, graph-wide route promotion, whole-archive
    production policy, and C CUDA removal remain open.
...
- Status: done.
- Goal: reduce corrected cached Rust CUDA routed-MoE gate/up overhead by
  selecting the measured row-256 policy on the opt-in Rust route.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_gate_rowspan_256_default_performance_repair.py --negative-test`.
- Evidence:
  - Only `gate_row_span` host selection changes: no-override cached gate
    execution now selects `256`, existing row-256 and row-128 controls become
    effective, and explicit row-512/row-2048 controls remain selectable.
    Corrected down arithmetic, emitted PTX, the benchmark executable, and
    default current-C routing remain unchanged.
  - Measurement-only probes on one DSO select row-512 at `1462.051 ms`
    gate/up, row-256 twice at `1448.505 ms` and `1448.424 ms`, and row-128
    at `1449.764 ms`, so row-256 is the retained default.
  - On the retained DSO, explicit row-512 records `1462.508 ms` gate/up and
    ...
    `2324.995 ms`, reductions of `1.01%` and `0.64%`. A separate inlined
    quarter-warp-reduction probe is rejected because it improves corrected
    total routed-MoE by only `1.365 ms`.
  - The retained DSO SHA-256 is
    `0f835118466070058b1aaf1488262ba8121af707e272ac61ecbc5c8adffa509f`;
    PTX remains identical to the corrected parent at
...
- Comparator:
  `ds4-parity/check_cuda_rust_gate_staged_pair_scalar_expansion_performance_repair.py --negative-test`.
- Evidence:
  - Only `abi_moe_gate_up_mid_expert_tile8_rowspan_cached_kernel` changes:
    its fixed staged-pair entry work now uses named scalar gate/up and block
    accumulators. Corrected down arithmetic, row-span policy, the benchmark
    executable, and default current-C routing remain unchanged.
  - Emitted PTX exposes `128` DP4A sites instead of `16` and removes local
    stores/loads from the kernel (`36/8` to `0/0`).
  - An adjacent B300 parent/candidate comparison lowers gate/up from
    `1448.003 ms` to `1256.146 ms` (`1.153x`) and total routed-MoE from
    `2325.307 ms` to `2133.521 ms` (`1.090x`); prefill rises from
    `216.02` to `222.92 tok/s`.
  - The repaired DSO SHA-256 is
...
- Status: done.
- Goal: reduce cached Rust CUDA routed-MoE overhead by loading naturally
  aligned staged Q8 scale and DP4A words as `u32` values.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_cached_q8_aligned_word_load_performance_repair.py --negative-test`.
- Evidence:
  - Cached `SXQ` and `SMIDQ` staging retains byte writes but carries explicit
    four-byte alignment. Q8 scale and DP4A word consumers use one aligned
    `u32` helper; Q8 bsum access, corrected down arithmetic, row-span policy,
    DP4A topology, the benchmark executable, and default current-C routing
    remain unchanged.
  - Gate/up PTX retains `128` DP4A sites while replacing shared byte-load
    sites from `584` to `8` with `128` shared `u32` loads. Down PTX retains
    `32` DP4A sites while replacing shared byte-load sites from `134` to `2`
    with `32` shared `u32` loads.
  - An adjacent B300 parent/candidate comparison lowers gate/up from
    `1255.901 ms` to `752.829 ms` (`1.668x`), down from `860.766 ms` to
    `632.945 ms` (`1.360x`), and total routed-MoE from `2132.947 ms` to
    `1402.260 ms` (`1.521x`); prefill rises from `222.88` to `230.07 tok/s`.
    An isolated fast-SwiGLU probe is rejected because its `1256.183 ms`
    gate/up timing does not improve the retained parent.
  - The repaired DSO SHA-256 is
...
- Status: done.
- Goal: reduce cached Rust CUDA routed-MoE down overhead by processing
  sequential eight-entry scalar halves instead of dynamic staged-entry
  arrays.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_down_sequential_half_tile_scalar_expansion_performance_repair.py --negative-test`.
- Evidence:
  - Only `abi_moe_down_expert_tile16_rowspan_cached_kernel` per-entry
    accumulation scheduling changes: it executes sequential eight-entry
    scalar halves. The aligned cached-Q8 loads, corrected Q2 arithmetic,
    gate/up codegen, row-span policy, benchmark executable, and default
    current-C routing remain unchanged.
  - Down PTX exposes `256` DP4A sites instead of `32` and removes local
    stores/loads from `54/9` to `0/0`; gate/up PTX is byte-identical to the
    parent. A full sixteen-entry scalar probe is rejected because its larger
    code/register footprint regresses down to `912.140 ms`.
  - An interleaved B300 parent/candidate comparison leaves gate/up
    effectively unchanged (`752.737 ms` to `752.848 ms`) while reducing
    down from `632.769 ms` to `518.360 ms` (`1.221x`) and total routed-MoE
    from `1401.795 ms` to `1287.615 ms` (`1.089x`); resident prefill rises
    from `241.19` to `244.16 tok/s`.
  - The repaired DSO SHA-256 is
...
- Goal: reduce cached Rust CUDA routed-MoE reduction overhead by expanding
  the retained fixed-order quarter-warp shuffle tree inside hot emit sites.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_in_kernel_fixed_order_reduction_performance_repair.py --negative-test`.
- Evidence:
  - Only the cached gate/up and down emit sites switch from calls to the
    branch-free fixed-order helper to a macro-expanded identical `4`, `2`,
    `1` shuffle sequence. Aligned Q8 staging, DP4A topology, row-span
    policy, benchmark executable, and default current-C route remain
    unchanged.
  - Gate/up PTX replaces `16` reduction calls with `48` in-kernel shuffle
    instructions, retaining only `8` exponential calls; down replaces its
    `8` reduction calls with `24` shuffle instructions. Both preserve DP4A
    counts and remain free of local loads and stores.
  - An adjacent B300 parent/candidate comparison lowers gate/up from
    `618.611 ms` to `604.403 ms`, down from `457.720 ms` to `443.172 ms`,
    and total routed-MoE from `1092.625 ms` to `1063.907 ms`, a `2.63%`
  ...
  - The repaired DSO SHA-256 is
    ...
    Official vectors pass `1958` checks plus `8` negative checks and B300
    feature tests pass `176` tests. The unified report passes with `281`
    passed, `50` skipped, and `0` failed. Both Claude review attempts
    returned `CLAUDE_REVIEW_UNAVAILABLE_NOT_LOGGED_IN`. Total routed-MoE
...
- Goal: reduce cached Rust CUDA routed-MoE shared-staging overhead by storing
  the already aligned private Q8 tail as paired `u64` words.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_cached_q8_aligned_pair_staging_performance_repair.py --negative-test`.
- Evidence:
  - The private cache retains its `296`-byte, eight-byte-aligned slot and the
    external Q8_K ABI remains `292` bytes. Word `0` remains an aligned `u32`
    scale copy; the aligned `72`-word tail is staged through `36` assembled
    `u64` shared writes without changing paired consumption or arithmetic.
  - Gate/up PTX adds one `st.shared.u64` site (`1` to `2`) and down adds one
    (`0` to `1`), while paired loads remain `72`/`128`, DP4A counts remain
    `128`/`256`, and neither hot kernel introduces local loads or stores.
  - An adjacent B300 parent/candidate comparison lowers gate/up from
    `583.532 ms` to `570.157 ms`, down from `430.267 ms` to `423.871 ms`,
    and total routed-MoE from `1030.111 ms` to `1010.385 ms`, a `1.91%`
    total phase reduction. Variable one-shot prefill throughput is not the
    retention criterion.
  - The repaired DSO SHA-256 is
    ...
    Official vectors pass `1958` checks plus `8` negative checks and B300
    feature tests pass `176` tests. The unified report passes `283`
    comparators with `50` skipped and `0` failed. Both requested Claude
    review attempts timed out after `60` seconds. Total routed-MoE remains
    `1.09x` current-C, so default routing and promotion blockers remain in
    force.
...
- Status: done.
- Goal: reduce cached Rust CUDA gate/up global-read overhead using explicit
  global-address-space loads where adjacent B300 measurement supports them.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_gate_global_load_address_space_performance_repair.py --negative-test`.
- Evidence:
  - The pinned `cuda-oxide` revision
    `ae721dc95912a918f182d13b7ca55281aa29d8f9` adds aligned `u16` and
    `u32` global-load intrinsics, and cached gate/up uses them for known
    global model/Q8 source pointers without changing arithmetic, cached
    shared layout, reductions, or the external ABI.
  - Gate/up PTX changes from zero explicit global loads to `10`
    `ld.global.u16` and `3` `ld.global.u32` sites while retaining `2`
    paired shared store sites and `128` DP4A sites. A broad down-path probe
    regressed down to `536.311 ms`, so the retained down path stays at zero
    explicit global loads with `1` paired shared store site and `256` DP4A
    sites.
  - The repaired DSO SHA-256 is
    ...
    total routed-MoE from `1005.846 ms` to `957.414 ms`, a `4.82%` phase
    reduction. Official vectors pass `1958` checks plus `8` negative checks
    and B300 feature tests pass `176` tests. The unified report passes `284`
    comparators with `50` skipped and `0` failed. Both requested Claude
    review attempts timed out after `60` seconds. Total routed-MoE remains
...
- Status: done.
- Goal: reduce cached Rust CUDA down-path scale-byte read overhead through a
  narrow paired-load address-space change while leaving the rejected packed
  Q2 payload globalization scope out of the retained implementation.
- Fixture:
...
- Comparator:
  `ds4-parity/check_cuda_rust_down_paired_scale_global_load_address_space_performance_repair.py --negative-test`.
- Evidence:
  - Cached down now consumes each aligned pair of low-nibble Q2 scale bytes
    through one `memory::load_global_u16` call, while retaining the existing
    explicit metadata loads, scalar packed-Q2 payload access, private Q8
    staging, fixed reduction order, and route policy.
  - Down PTX changes from `2` to `6` `ld.global.u16` sites and from `41` to
    `33` byte-load sites while retaining `152` shared loads, `2` shared
    stores, `256` DP4A operations, and zero local-memory operations.
  - The repaired DSO SHA-256 is
    ...
    Order-reversed adjacent B300 comparisons lower down from `420.440 ms`
    to `401.116 ms` and from `420.188 ms` to `401.120 ms`, with total
    reductions from `958.863 ms` to `939.293 ms` and from `958.364 ms` to
    ...
    review attempts timed out after `60` seconds. Rust total is `1.016x`
