...
static void usage(FILE *fp) {
    fputs("usage: ds4-layer0-output-head-oracle-dump -m MODEL -o OUTPUT.json [--token N]\n", fp);
}
...
int main(int argc, char **argv) {
    ...
    if (!fp) {
        fprintf(stderr, "ds4-layer0-output-head-oracle-dump: open '%s': %s\n", opt.output_path, strerror(errno));
        return 1;
    }
    const int rc = ds4_dump_layer0_output_head_oracle_json(opt.model_path, opt.token, fp);
    if (fclose(fp) != 0 && rc == 0) {
        fprintf(stderr, "ds4-layer0-output-head-oracle-dump: close '%s': %s\n", opt.output_path, strerror(errno));
        return 1;
    }
    return rc;
}
